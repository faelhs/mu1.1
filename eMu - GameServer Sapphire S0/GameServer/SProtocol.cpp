...
//	GS-CS	1.00.18	JPN	0x0042DC70	-	Completed
...
void JGServerLoginResult( SDHP_RESULT * lpMsg)	
{
	...
	{
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 210)));
		return;
	}
...
void JGPAccountRequest(SDHP_IDPASSRESULT * lpMsg)
{
	...
	switch ( lpMsg->result  )
	{
		...
		case 15:
			...
			{	
				LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 211)), gObj[aIndex].Ip_addr, szId, lpMsg->UserNumber , lpMsg->DBNumber );
			}

			if ( lpMsg->UserNumber  == 0 && lpMsg->DBNumber  == 0)
			{
				LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 212)), szId, lpMsg->UserNumber , lpMsg->DBNumber );
			}
	}
...
void AllNoticeRecv( SDHP_NOTICE * lpMsg)
{
	...
	LogAdd(LOG_BLACK, "[Mu Manager][Notice Log][Type:ALL] %s", szNotice);
...
void UserNoticeRecv( SDHP_USER_NOTICE * lpMsg)
{
	...
	LogAdd(LOG_BLACK, "[Mu Manager][Notice Log][Type:USER] (%s) %s", szId, szNotice);
...
void GJReqMapSvrMove(int iIndex, WORD wDesMapSvrCode, WORD wMapNumber, BYTE btX, BYTE btY)
{
	...
	{
		LogAdd(LOG_RED,  "[MapServerMng] Packet Error GJ [0x7A] - User not Connected : %d", iIndex);
	}
	else
	{
...
void JGAnsMapSvrMove(PMSG_ANS_MAPSVRMOVE * lpMsg)
{
	...
	{
		LogAdd(LOG_RED,  "[MapServerMng] Packet Error JG [0x7A] - User not Connected : %d", lpMsg->iIndex);
		return;
	}
	...
	{
		LogAdd(LOG_RED,  "[MapServerMng] Packet Error JG [0x7A] - User Info Mismatch [%s][%s]-[%s][%s] (%d)", lpMsg->szAccountID, lpMsg->szCharName, lpObj->AccountID, lpObj->Name, lpObj->m_Index);
		return;
	}
	...
	if ( lpMsg->iResult == 0	 )
	{
		...
		if ( bRESULT == FALSE )
		{
			LogAdd(LOG_RED,  "[MapServerMng] Packet Error JG [0x7A] - Failed to find Map Info [%s]-[%s] (%d)",
		...
		LogAdd(LOG_BLACK, "[MapServerMng] [%s][%s] JG [0x7A] - Move Map Request OK : MAPSVR_D:%d",
			lpObj->AccountID, lpObj->Name, lpMsg->wDstMapSvrCode);
	}
	else
	{
		...
		LogAdd(LOG_RED,  "[MapServerMng] Packet Error JG [0x7A] - Failed to Authorize User [%s][%s] Result:%d",
...
void GJReqMapSvrAuth(int iIndex, LPSTR lpszAccountID, LPSTR lpszCharName, int iJA1, int iJA2, int iJA3, int iJA4)
{
	...
	{
		LogAdd(LOG_RED,  "[MapServerMng] Packet Error GJ [0x7B] - Index out of bound : %d", iIndex);
		return;
	}
...
void JGAnsMapSvrAuth(PMSG_ANS_MAPSVRAUTH * lpMsg)
{
	...
	{
		LogAdd(LOG_RED,  "[MapServerMng] Packet Error JG [0x7B] - Index out of bound : %d",
			lpMsg->iIndex);
	...
	{
		LogAdd(LOG_RED,  "[MapServerMng] MapSvr MoveReq Error JG [0x7B] - No Request [%s] (%d)",
	...
	switch ( lpMsg->iResult )
	{
		case 0:
			...
			{
				LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 211)), gObj[iIndex].Ip_addr, szId, lpMsg->iUserNumber, lpMsg->iDBNumber);
				iSendResult = 1;
				bAuthOK = TRUE;
			}
			...
			{
				LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 212)), szId, lpMsg->iUserNumber, lpMsg->iDBNumber);
			}
	...
	if ( bAuthOK == FALSE )
	{
		LogAdd(LOG_RED,  "[MapServerMng] [%s][%s] JG [0x7B] - Join Result Failed : RST:%d, BLK:%c",
	...
	LogAdd(LOG_BLACK, "[MapServerMng] [%s][%s] JG [0x7B] - Join Result OK : RST:%d, BLK:%c",
		szId, lpMsg->szCharName, lpMsg->iResult, lpMsg->btBlockCode);
