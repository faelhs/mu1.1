...
//	GS-CS	1.00.18	JPN	0x0053D380	-	Completed
...
void CChaosCastle::CheckSync(int iChaosCastleIndex)
{
	...
	{
		LogAdd(LOG_RED,  "[Chaos Castle] (%d) No Schedule Time Data - Chaos Castle Terminated (m_vtChaosCastleOpenTime.empty())", iChaosCastleIndex);
	...
	switch ( bTIME_CHANGED )
	{
		...
		default:
			LogAdd(LOG_RED,  "[Chaos Castle] (%d) No Schedule Time Data - Chaos Castle Terminated (bTIME_CHANGED=%d)",
	...
	LogAdd(LOG_BLACK, "[Chaos Castle] (%d) Sync Start Time. [%d] min remain (START HOUR:%d, MIN:%d)",
...
void CChaosCastle::ProcState_Playing(int iChaosCastleIndex)
{
	...
	if ( iTICK_MSEC >= 1000 )
	{
			...
			this->m_stChaosCastleData[iChaosCastleIndex].m_bCC_PLAY_START == false )
		{
			...
			LogAdd(LOG_BLACK, "[Chaos Castle] (%d) Chaos Castle Quest Start (UserCount:%d)",
			 ...
			 this->m_stChaosCastleData[iChaosCastleIndex].m_bCC_CAN_ENTER == false )
		{
			...
			{
				this->SetState(iChaosCastleIndex, CC_STATE_CLOSED);
				LogAdd(LOG_BLACK, "[Chaos Castle] (%d) Chaos Castle Quest Closed - No User",
					iChaosCastleIndex+1);
		...
		if ( this->m_stChaosCastleData[iChaosCastleIndex].m_bCC_PLAY_START == true )
		{
			...
			if ( bPlayEnded == TRUE )
			{
				...
				if ( iMonsterCount <= 0 )
				{
					...
					if ( iWinnerIndex != -1 )
					{
						LogAdd(LOG_BLACK, "[Chaos Castle] (%d) [%s][%s] is Winner. [CharInfoSave : Class=%d, Level=%d, LvPoint=%d, Exp=%d, Str=%d, Dex=%d, Vit=%d, Energy=%d, LeaderShip=%d, Map=%d, Pk=%d]",
					...
					{
						LogAdd(LOG_BLACK, "[Chaos Castle] (%d) Has No Winner.", iChaosCastleIndex+1);
					}
				}
				else
				{
					LogAdd(LOG_BLACK, "[Chaos Castle] (%d) Has No Winner : Monster Left (%d)",
	...
	if ( this->m_stChaosCastleData[iChaosCastleIndex].m_iCC_REMAIN_MSEC <= 0 )
	{
		LogAdd(LOG_BLACK, "[Chaos Castle] (%d) is Over : TIME-OUT (Left User:%d, Monster:%d)",
...
void CChaosCastle::SetState_Closed(int iChaosCastleIndex)
{
	...
	LogAdd(LOG_BLACK, "[Chaos Castle] (%d) SetState CLOSED", iChaosCastleIndex+1);
}
...
void CChaosCastle::SetState_Playing(int iChaosCastleIndex)
{
	...
	if ( this->CheckCanStartPlay(iChaosCastleIndex) == false )
	{
		...
		LogAdd(LOG_BLACK, "[Chaos Castle] (%d) Failed to Start Chaos Castle : Lack of User", iChaosCastleIndex +1 );
		this->SetState_Closed(iChaosCastleIndex);
	}
	else
	{
		...
		LogAdd(LOG_BLACK, "[Chaos Castle] (%d) SetState PLAYING", iChaosCastleIndex + 1 );
	}
}
...
void CChaosCastle::SetState_PlayEnd(int iChaosCastleIndex)
{
	...
	LogAdd(LOG_BLACK, "[Chaos Castle] (%d) SetState PLAYEND", iChaosCastleIndex+1);
...
void CChaosCastle::DelMonsterList(int iChaosCastleIndex, int iMonsterIndex)
{
	...
	LogAdd(LOG_BLACK, "[Chaos Castle][Bug Tracer] MONSTER DEL (%d) Count(%d) : ENTER - Index(%d)",
	...
	LogAdd(LOG_BLACK, "[Chaos Castle][Bug Tracer] MONSTER DEL (%d) Count(%d) : OUT - Index(%d)",
		iChaosCastleIndex+1, this->m_stChaosCastleData[iChaosCastleIndex].m_lCC_CURRENT_MONSTER_COUNT, iMonsterIndex);
}
...
void CChaosCastle::SearchNDropMonsterItem(int iChaosCastleIndex, int iMonsterIndex, int iMaxHitUserIndex)
{
	...
	if ( OBJMAX_RANGE(iMaxHitUserIndex) != FALSE )
	{
		LogAdd(LOG_BLACK, "[Chaos Castle] (%d) Monster Dropped ChaosCastle Item To [%s][%s] (Item:%s)",
...
BOOL CChaosCastle::LevelUp(int iUserIndex, int iAddExp, int iEventType)
{
	...
	else
	{
		...
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(2, 8)), gObj[iUserIndex].AccountID, gObj[iUserIndex].Name, gObj[iUserIndex].Level);
	}
...
bool CChaosCastle::CheckCanStartPlay(int iChaosCastleIndex)
{
	...
	{
		LogAdd(LOG_BLACK, "[Chaos Castle] (%d) GetCurPlayUser() FAILED (UserCount:%d)", iChaosCastleIndex+1, iEnteredUser);
...
BOOL CChaosCastle::ObjSetPosition(int iIndex, int iX, int iY)
{
	...
	PHeadSetB((LPBYTE)&pMove2, PROTOCOL_MOVE, sizeof(pMove2));
		...
		if ( gObj[iIndex].MapNumber == iMapNumber && gObj[iIndex].Connected > PLAYER_LOGGED )
		{
				 ...
				 gObj[iIndex].Y >= iMIN_Y && gObj[iIndex].Y <= iMAX_Y )
			{
				...
				if ( btMapAttr == 8 )
				{
					...
					LogAdd(LOG_BLACK, "[Chaos Castle] (%d) [%s][%s] User Dead In Chaos Castle : Fall from Castle (X:%d, Y:%d)",
	...
	for ( int i=0;i<MAX_CHAOSCASTLE_USER;i++)
	{
		if ( this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex != -1 )
		{
				 ...
				 gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].MapNumber == (iChaosCastleIndex + MAP_INDEX_CHAOSCASTLE1) )
			{
				...
				LogAdd(LOG_BLACK, "[Chaos Castle] (%d) [%s][%s] Payback to User ChaosCastle Money (FIRST:%d, PAYBACK:%d, FINAL:%d)",
					iChaosCastleIndex+1, gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].AccountID,
					gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].Name,
...
void CChaosCastle::RewardUserEXP(int iChaosCastleIndex, int iChaosCastleSubIndex, BOOL bWinner)
{
	...
	if ( this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex != -1 )
	{
		if ( gObjIsConnected(this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[iChaosCastleSubIndex].m_iIndex) ==TRUE )
		{
			...
			LogAdd(LOG_BLACK, "[Chaos Castle] (%d) [%s][%s] Reward User EXP (USER_KILL:%d, MON_KILL:%d, TOT_EXP:%d, TOT_REWARD_EXP:%d)",
					   ...
					   iSY > g_iChaosCastle_DamageAxis[iTRAP_STEP][3] ) )
				{
					...
					if ( gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].m_ColdBeattackCount <= 0 )
					{
						...
						GCStateInfoSend(&gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex], 1,
							gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].m_ViewSkillState);
					}
				}
			}
		}
	...
	if ( iItemDropRate < 50 )
	{
		switch ( iChaosCastleIndex )
		{
			case 0:
				...
				LogAdd(LOG_BLACK, "[Chaos Castle] (%d) [%s][%s] Winner Item - Gem of Soul", iChaosCastleIndex+1,
					gObj[iWinnerIndex].AccountID, gObj[iWinnerIndex].Name);
				break;
			case 1: case 2: case 3: case 4: case 5:
				iItemType = ITEMGET(14,22);
				LogAdd(LOG_BLACK, "[Chaos Castle] (%d) [%s][%s] Winner Item - Gem of Creative", iChaosCastleIndex+1,
					gObj[iWinnerIndex].AccountID, gObj[iWinnerIndex].Name);
				break;
		}
	...
	else if ( iItemDropRate <95 )
	{
		switch ( iChaosCastleIndex )
		{
			case 0:
				...
				LogAdd(LOG_BLACK, "[Chaos Castle] (%d) [%s][%s] Winner Item - Gem of Bless", iChaosCastleIndex+1,
					gObj[iWinnerIndex].AccountID, gObj[iWinnerIndex].Name);
				break;
			case 1: case 2: case 3: case 4: case 5:
				iItemType = ITEMGET(14,16);
				LogAdd(LOG_BLACK, "[Chaos Castle] (%d) [%s][%s] Winner Item - Gem of Life", iChaosCastleIndex+1,
					gObj[iWinnerIndex].AccountID, gObj[iWinnerIndex].Name);
				break;
		}
	...
	else
	{
		switch ( iChaosCastleIndex )
		{
			case 0:
				...
				LogAdd(LOG_BLACK, "[Chaos Castle] (%d) [%s][%s] Winner Item - Lucky Box Item", iChaosCastleIndex+1,
					gObj[iWinnerIndex].AccountID, gObj[iWinnerIndex].Name);
				break;
			case 1: case 2: case 3: case 4: case 5:
				MakeRandomSetItem(iWinnerIndex);
				LogAdd(LOG_BLACK, "[Chaos Castle] (%d) [%s][%s] Winner Item - Set Item", iChaosCastleIndex+1,
					gObj[iWinnerIndex].AccountID, gObj[iWinnerIndex].Name);
				break;
		}
...
int CChaosCastle::RewardZettoMoney(int iChaosCastleIndex)
{
	...
	if ( iZettoMoney <= 0 )
	{
		LogAdd(LOG_BLACK, "[Chaos Castle] (%d) ERROR : REWARD ZEN <= 0 (%d)",
	...
	for (int i=0;i<MAX_CHAOSCASTLE_USER;i++)
	{
		if ( this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex != -1 )
		{
			if ( gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].Connected == PLAYER_PLAYING )
			{
				...
				LogAdd(LOG_BLACK, "[Chaos Castle][Bug Tracer] (%d) [%s][%s] Reword ZettoMoney(Map:%d) Send Message",
					iChaosCastleIndex+1, gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].AccountID,
					gObj[this->m_stChaosCastleData[iChaosCastleIndex].m_UserData[i].m_iIndex].Name,
	...
	{
		LogAdd(LOG_BLACK, "[Chaos Castle] (%d) [%s][%s] Win Zen Lotto (RewardZen:%d, UserZen:%d)",
