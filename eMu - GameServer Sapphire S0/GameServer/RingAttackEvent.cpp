...
// GS-CS	1.00.18	JPN	0x0046CF70	-	Completed
...
BOOL CRingMonsterHerd::MonsterHerdItemDrop(LPOBJ lpObj)
{
	...
	if ( lpObj->Class == 135 )
	{
		...
		LogAdd(LOG_BLACK, "[Ring Event] White Wizard Killed by [%s][%s], MapNumber:%d",
...
void CRingMonsterHerd::SendEventGiftWinner(int iIndex, int iGiftKind)
{
	...
	LogAdd(LOG_BLACK, "[Ring Event] [%s][%s] Request to Register Gift - Gift Kind (%d)",
...
void CRingAttackEvent::StartEvent()
{
	...
	for ( int i=0;i<MAX_RINGMONSTER_MAP;i++)
	{
		...
		else
		{
			...
			{
				LogAdd(LOG_BLACK, "[Ring Event] - Error : No Monster Data Exist");
				continue;
			}

			LogAdd(LOG_BLACK, "[Ring Event] - Monster Start Position MapNumber:%d, X:%d, Y:%d",
...
BOOL CRingAttackEvent::Load(char * lpszFileName)
{
	...
	fclose(SMDFile);
	LogAdd(LOG_BLACK, "%s file load!", lpszFileName);
...
void CRingAttackEvent::CheckSync()
{
	...
	{
		LogAdd(LOG_RED,  "[Ring Event] No Schedule Time Data");
		this->SetState(0);
	...
	LogAdd(LOG_BLACK, "[Ring Event] Sync Start Time. [%d] min remain (START HOUR:%d, MIN:%d)",
...
void CRingAttackEvent::ProcState_Closed()
{
		...
		if ( this->m_bDoEvent )
		{
				 ...
				 (this->m_iTIME_MSEC_REMAIN / 60000 ) != this->m_iTIME_NOTIFY_COUNT )
			{
				...
				if ( (this->m_iTIME_NOTIFY_COUNT+1) == 3 )
				{
					...
					LogAdd(LOG_BLACK, "[Ring Event] - Before 3 minutes - Advertise");
				}
			}
		}
	}
	...
	if ( this->m_iTIME_MSEC_REMAIN <= 0 )
	{
		...
		LogAdd(LOG_BLACK, "[Ring Event] - Event Started");
	}
}
...
void CRingAttackEvent::ProcState_Playing()
{
	...
	if ( ((this->m_iTIME_MSEC_REMAIN / 1000 ) % 30 ) == 0 && ((this->m_iTIME_MSEC_REMAIN / 1000) / 30) != s_iTIME_REMAIN  )
	{
		...
		for ( int i=0;i<MAX_RINGMONSTER_MAP;i++)
		{
			...
			LogAdd(LOG_BLACK, "[Ring Event] - per 30'S Inform MapNumber:%d, X:%d, Y:%d, Boss Live:%d",
	...
	if ( this->m_iTIME_MSEC_REMAIN <= (this->m_iTIME_MONSTER_TO_DEST * 60 * 1000) && this->m_bMonsterToDest == FALSE )
	{
		...
		for ( int i=0;i<MAX_RINGMONSTER_MAP;i++)
		{
			...
			{
				LogAdd(LOG_BLACK, "[Ring Event] - Monster Herd Move MapNumber:%d, X:%d, Y:%d",
					g_RingEventMapNum[i], iRandX, iRandY);
			}
		}
	...
	if ( this->m_iTIME_MSEC_REMAIN <= 0 )
	{
		...
		LogAdd(LOG_BLACK, "[Ring Event] - Event Ended");
	}
}
...
void CRingAttackEvent::Start_Menual()
{
	...
	LogAdd(LOG_BLACK, "¡Û¡Ü[Event Management] [Start] RingAttack Event!");
