...
// GS-CS	1.00.18	JPN	0x004667C0	-	Completed
		...
		if ( this->m_DevilSquareBossMonsterInfo[i].m_Type != (WORD)-1 )
		{
			if ( currtime == this->m_DevilSquareBossMonsterInfo[i].m_RegenStartTime )
			{
				...
				if ( iMonIndex >= 0 )
				{
					...
					LogAdd(LOG_BLACK, "[DevilSquare] Boss %d %d %d create ", map, x, y);
...
void CDevilSquareGround::SendScore()
{
	...
	LogAdd(LOG_BLACK, "[DevilSquare] Rank [%d]", this->m_iIndex);
	...
	for ( ; Itor != this->m_DevilSquareRankList.end(); Itor++ )
	{
		...
		LogAdd(LOG_BLACK, "Rank :[%d] : [%s][%s][%d][%d][%d]", count, (*(Itor))->AccountID, (*(Itor))->Name, (*(Itor))->m_nEventMoney, (*(Itor))->m_nEventExp, (*(Itor))->m_nEventScore);
