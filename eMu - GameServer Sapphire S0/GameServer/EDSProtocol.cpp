...
void ExDataServerLoginResult(SDHP_RESULT * lpMsg)
{
	...
	{
		LogAdd(LOG_BLACK, "ExDB Server Login Fail: %d", lpMsg->Result);
...
void GDGuildCreateRecv(SDHP_GUILDCREATE_RESULT * lpMsg)
{
	...
	if ( lpMsg->Result == 1 )
	{
		...
		if ( aIndex >= 0 && lpNode != NULL )
		{
			...
			LogAdd(LOG_BLACK, "[U.System] Guild is Created - Guild : %s / [%s][%s]",
				szGuildName, gObj[aIndex].AccountID, gObj[aIndex].Name);

			LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 177)), lpNode->WarState, lpNode->WarDeclareState, szGuildName);
	...
	if ( lpMsg->Result == 1 || lpMsg->Result == 4)
	{
		...
		for ( int n=0;n<MAX_USER_GUILD;n++)
		{
			...
			if ( lpGuild->Use[n] > 0 && iGuildMemberIndex != -1 )
			{
				...
				LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 178)), gObj[n].Name);
			}
		}
		...
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 179)), szGuildName, szMaster);
	}
}
...
void GDGuildMemberDelResult(SDHP_GUILDMEMBERDEL_RESULT * lpMsg)
{
	...
	if ( lpMsg->Flag == 1 )
	{
		...
		if ( gObjIsConnected(aIndex) == TRUE )
		{
			...
			//{
				...
				{
					GCResultSend(aIndex, 0x53, lpMsg->Result);
					LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 180)), gObj[aIndex].Name);
				}
				else
				{
		...
		{
			LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 181)), gObj[aIndex].Name);
		}
	}
				...
				if ( gObj[iMember].GuildNumber > 0 )
				{
					if ( gObj[iMember].Name[0] == szMember[0] && gObj[iMember].Name[1] == szMember[1] )
					{
						if ( strcmp(gObj[iMember].Name, szMember) == 0 )
						{
							...
							LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 182)), aIndex, gObj[iMember].Name);
							break;
						}
					}
				}
	...
	{
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 183)), szGuildName, lpMsg->Result);
	}
}
			...
			if ( gObj[n].Name[0] == lpMsg->MemberID[0] )
			{
				if ( strcmp(lpMsg->MemberID, gObj[n].Name ) == 0 )
				{
					...
					if ( gObj[n].lpGuild != NULL )
					{
						...
						LogAdd(LOG_BLACK, "[U.System][Guild Status][Info] Guild : %s / [%s][%s] / %d",
							szGuildName, gObj[n].AccountID, szName, gObj[n].GuildStatus);

						LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 184)), gObj[n].AccountID, szName, szGuildName);
...
#endif
						LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 185)));
					}
...
void DGGuildScoreUpdate(LPSTR guildname, int score)
{
	...
	LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 186)), guildname, score);
}
...
void GDGuildScoreUpdateRecv(SDHP_GUILDSCOREUPDATE * lpMsg)
{
	...
	LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 187)), lpMsg->GuildName, lpMsg->Score);
}
...
void DGGuildNoticeRecv(SDHP_GUILDNOTICE * lpMsg)
{
	...
	LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 188)), lpMsg->GuildName, lpMsg->szGuildNotice);
...
void DGGuildAnsAssignStatus(EXSDHP_GUILD_ASSIGN_STATUS_RESULT * lpMsg)
{
	if ( lpMsg->btFlag == 1 )
	{
		...
		LogAdd(LOG_BLACK, "[U.System][Guild Status][Assign] Guild : ( %s ) Member : ( %s ) Status : ( %d )",
...
void DGRelationShipAnsJoin(EXSDHP_RELATIONSHIP_JOIN_RESULT * lpMsg)
{
	...
	if (lpMsg->btRelationShipType == G_RELATIONSHIP_UNION )
	{
		...
		LogAdd(LOG_BLACK, "[U.System][Union][Join] ( Union Master Guild: %s ), (Union Member Guild: %s)",
	...
	else if ( lpMsg->btRelationShipType == G_RELATIONSHIP_RIVAL )
	{
		...
		LogAdd(LOG_BLACK, "[U.System][Rival][Join] (  %s ) vs ( %s )",
...
void DGUnionBreakOff(EXSDHP_RELATIONSHIP_BREAKOFF_RESULT * lpMsg)
{
	...
	LogAdd(LOG_BLACK, "[GS][U.System][Union][BreakOff] Request Union Member Guild: %s)",
		lpReqGuild->Name);
	...
	LogAdd(LOG_BLACK, "[GS][U.System][Union][BreakOff] ( Union Master Guild: %s ), (Union Member Guild: %s)",
		lpTargetGuild->Name, lpReqGuild->Name);
...
void DGRelationShipAnsBreakOff(EXSDHP_RELATIONSHIP_BREAKOFF_RESULT * lpMsg)
{
	...
	LogAdd(LOG_BLACK, "[U.System][Rival][BreakOff] (  %s ) vs ( %s )",
		lpReqGuild->Name, lpTargetGuild->Name);
...
void DGRelationShipNotificationRecv(EXSDHP_NOTIFICATION_RELATIONSHIP * lpMsg)
{
	if ( lpMsg->btUpdateFlag == EX_RESULT_FAIL_FOR_CASTLE )
	{
		...
		if ( lpGuildInfo )
		{
			...
			LogAdd(LOG_BLACK, "[U.System][Union][Auto BreakOff] ( Union Master Guild: %s ), (Union Member Guild: %s)",
...
void DGRelationShipAnsKickOutUnionMember(EXSDHP_KICKOUT_UNIONMEMBER_RESULT * lpMsg)
{
	...
	LogAdd(LOG_BLACK, "[GS][U.System][Union][KickOut] ( Union Master Guild: %s ), (Union Member Guild: %s)",
...
void FriendListRequest(int aIndex)
{
	...
	{
		LogAdd(LOG_BLACK, "error-L3 : [%s][%s] FriendServer Online Request ", gObj[aIndex].AccountID, gObj[aIndex].Name);
		return;
	}

	if (gObj[aIndex].m_FriendServerOnline == FRIEND_SERVER_STATE_OFFLINE )
	{
		LogAdd(LOG_BLACK, "error-L3 : [%s][%s] FriendServer Offline", gObj[aIndex].AccountID, gObj[aIndex].Name);
		return;
	}
	...
	LogAdd(LOG_BLACK, "[%s] Friend List Request", gObj[aIndex].Name);
...
void FriendListResult(LPBYTE lpMsg)
{
	...
	LogAdd(LOG_BLACK, "[%s] Friend List Count (%d) Send",
...
void WaitFriendListResult(FHP_WAITFRIENDLIST_COUNT * lpMsg)
{
	...
	{
		LogAdd(LOG_BLACK, "error-L2 : (%s) Index %s %d ",
	...
	LogAdd(LOG_BLACK, "[%s] WaitFriend List [%s]", gObj[lpMsg->Number].Name, Name.GetBuffer());
}
...
void FriendStateRecv(FHP_FRIEND_STATE * lpMsg)
{
	...
	if ( !gObjIsConnectedGP(lpMsg->Number, Name.GetBuffer()))
	{
		LogAdd(LOG_BLACK, "error-L2 : (%s) Index %s %d ", Name.GetBuffer(), __FILE__, __LINE__);
		return;
	}
	...
	LogAdd(LOG_BLACK, "[%s] Friend State (%d)", gObj[lpMsg->Number].Name, lpMsg->State);
}
...
void FriendAddRequest(PMSG_FRIEND_ADD_REQ * lpMsg, int aIndex)
{
	...
	LogAdd(LOG_BLACK, "[%s] Friend add request Name: [%s]", gObj[aIndex].Name,		szFriendName.GetBuffer());

	if ( !strcmp(gObj[aIndex].Name, szFriendName.GetBuffer()))
	{
		LogAdd(LOG_BLACK, "error-L3 : [%s] Friend add : Not self", gObj[aIndex].Name);
...
void FriendAddResult(FHP_FRIEND_ADD_RESULT * lpMsg)
{
	...
	LogAdd(LOG_BLACK, "[%s] Friend add result : (%d) friend Name : (%s)",
...
void WaitFriendAddRequest(PMSG_FRIEND_ADD_SIN_RESULT * lpMsg, int aIndex)
{
...

	{
		...
		LogAdd(LOG_BLACK, "[%s] WaitFriend add request [%s]", gObj[aIndex].Name, fname.GetBuffer());
	}
}
...
void WaitFriendAddResult(FHP_WAITFRIEND_ADD_RESULT * lpMsg)
{
	...
	LogAdd(LOG_BLACK, "[%s] WaitFriend add result (%d) [%s]",
		Name.GetBuffer(), lpMsg->Result, szFriendName.GetBuffer());
...
void FriendDelRequest(PMSG_FRIEND_DEL_REQ * lpMsg, int aIndex)
{
	...
	LogAdd(LOG_BLACK, "[%s] Friend del request [%s]", gObj[aIndex].Name, Name.GetBuffer());
}
...
void FriendDelResult(FHP_FRIEND_DEL_RESULT * lpMsg)
{
	...
	LogAdd(LOG_BLACK, "[%s] Friend del result (%d) [%s]",
...
void FriendChatRoomCreateReq(PMSG_FRIEND_ROOMCREATE_REQ * lpMsg, int aIndex)
{
	...
	LogAdd(LOG_BLACK, "[%s] Chatroom create request [%s]", gObj[aIndex].Name, szName.GetBuffer());
}
...
void FriendChatRoomCreateResult(FHP_FRIEND_CHATROOM_CREATE_RESULT * lpMsg)
{
	...
	if ( !gObjIsConnectedGP(lpMsg->Number, Name.GetBuffer()))
	{
		LogAdd(LOG_BLACK, "error-L2 : Index %s %d (%s)", __FILE__, __LINE__, Name.GetBuffer());
		return;
	}
	...
	LogAdd(LOG_BLACK, "[%s] Chatroom create result (%d) ticket:(%d)",
...
void FriendMemoSend(PMSG_FRIEND_MEMO * lpMsg, int aIndex)
{
	...
	LogAdd(LOG_BLACK, "[%s] Friend mail send %s (Size:%d)", gObj[aIndex].Name, Name.GetBuffer(), bsize);
}
...
void MngFriendMemoSend(PMSG_JG_MEMO_SEND * lpMsg)
{
	...
	LogAdd(LOG_BLACK, "JoinServer mail send %s (Size:%d)", Name.GetBuffer(), bsize);
}
...
BOOL WithdrawUserMoney(LPSTR Type, LPOBJ lpObj, int Withdraw_Money)
{
	...
	if ( (lpObj->Money - Withdraw_Money) >= 0 )
	{
		...
		LogAdd(LOG_BLACK, "[%s][%s] (%s) Pay Money(In Inventory) : %d - %d = %d",
...
void FriendMemoSendResult(FHP_FRIEND_MEMO_SEND_RESULT * lpMsg)
{
	...
	{
		LogAdd(LOG_BLACK, "JoinServer Send Mail result : (%d)", lpMsg->Result);
		return;
	}
	...
	LogAdd(LOG_BLACK, "[%s] Friend mail Send result : (%d) guid:(%d)",
...
void FriendMemoListReq(int aIndex)
{
	...
	LogAdd(LOG_BLACK, "[%s] Friend mail list request", gObj[aIndex].Name);
}
...
void FriendMemoList(FHP_FRIEND_MEMO_LIST * lpMsg)
{
	...
	LogAdd(LOG_BLACK, "[%s] Friend Mail list : (%d:%10s:%s:%s)",
...
void FriendMemoReadReq(PMSG_FRIEND_READ_MEMO_REQ * lpMsg, int aIndex)
{
	...
	LogAdd(LOG_BLACK, "[%s] Friend Mail read Index:%d", 
...
void FriendMemoRead(FHP_FRIEND_MEMO_RECV * lpMsg)
{
	...
	{
		LogAdd(LOG_BLACK, "error-L2 : Friend Memo Max %s %d", __FILE__, __LINE__);
		return;
	}
	...
	LogAdd(LOG_BLACK, "[%s] Friend Mail read (%d)",
...
void FriendMemoDelReq(PMSG_FRIEND_MEMO_DEL_REQ * lpMsg, int aIndex)
{
	...
	LogAdd(LOG_BLACK, "[%s] Friend mail delete request Index:(%d)",
		gObj[aIndex].Name, lpMsg->MemoIndex);
}
...
void FriendMemoDelResult(FHP_FRIEND_MEMO_DEL_RESULT * lpMsg)
{
	...
	LogAdd(LOG_BLACK, "[%s] Friend mail delete request (%d) index:(%d)",
...
void FriendRoomInvitationReq(PMSG_ROOM_INVITATION * lpMsg, int aIndex)
{
	...
	LogAdd(LOG_BLACK, "[%s] Friend Invistation request room:%d winguid:%d",
...
void FriendRoomInvitationRecv(FHP_FRIEND_INVITATION_RET* lpMsg)
{
	...
	LogAdd(LOG_BLACK, "[%s] Friend Invitation result :%d",
