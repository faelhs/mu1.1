...
void MsgAllUser(int iType, char * szMsg, ...)
{
	...
	for(int i = 0; i < OBJMAX; i++)
	{
		...
		{
			GCServerMsgStringSend(szBuffer, gObj[i].m_Index, iType);
		}
	}
}
...
void ChatMsgSend(char * Msg, ...)
{
	...
	va_end(pArguments);
	// ---
	memcpy(&pMsg.chatmsg, &szBuffer, strlen(szBuffer) + 1);	
	// ---
	PHeadSetB((LPBYTE)&pMsg, 0x00, sizeof(szBuffer) + 13);
	// ---
...
void ChatWisperServer(char * Msg, ...)
{
	...
	memcpy(&pMsg.chatmsg, &szBuffer, strlen(szBuffer) + 1);
	// ---
	PHeadSetB((LPBYTE)&pMsg, 0x02, sizeof(szBuffer) + 13);
	// ---
	DataSendAll((LPBYTE)&pMsg, pMsg.h.size);
}