...
//	GS-CS	1.00.18	JPN	0x0050A790	-	Completed
#include "stdafx.h"
...
BOOL CJewelMixSystem::MixJewel( int iIndex, int iJewelType, int iMixType)
{
	...
	if ( gObjCheckInventorySerial0Item(&gObj[iIndex]))
	{
		...
		LogAdd(LOG_BLACK, "[ANTI-HACK][Serial 0 Item] [Mix Jewel] (%s)(%s)",
	...
	{
		LogAdd(LOG_BLACK, "[JewelMix] [%s][%s] Chaos Mix is already working",
			gObj[iIndex].AccountID, gObj[iIndex].Name);
	...
	{
		LogAdd(LOG_BLACK, "[JewelMix] [%s][%s] Mix iJewelType is out of bound : %d",
	...
	{
		LogAdd(LOG_BLACK, "[JewelMix] [%s][%s] iMixType is out of bound : %d",
	...
	{
		LogAdd(LOG_BLACK, "[JewelMix] [%s][%s] lack of jewel to mix : %d / %d",
			gObj[iIndex].AccountID, gObj[iIndex].Name,
	...
	{
		LogAdd(LOG_BLACK, "[JewelMix] [%s][%s] lack of money to mix : %d / %d",
			gObj[iIndex].AccountID, gObj[iIndex].Name,
	...
	for ( int x= INVETORY_WEAR_SIZE ; x< MAIN_INVENTORY_SIZE; x++)
	{
		if ( gObj[iIndex].pInventory[x].IsItem() == TRUE )
		{
			...
			{
				LogAdd(LOG_BLACK, "[JewelMix] [%s][%s] Mix - Delete Jewel, Type:%d, Level:%d, Serial:%d",
					gObj[iIndex].AccountID, gObj[iIndex].Name,
	...
	if ( bItemDelOK == FALSE )
	{
		LogAdd(LOG_BLACK, "[JewelMix] [%s][%s] lack of jewel to mix (in deleting) : %d / %d",
			gObj[iIndex].AccountID, gObj[iIndex].Name,
	...
	LogAdd(LOG_BLACK, "[JewelMix] [%s][%s] jewel mix succeed : ItemType:%d, JewelCount:%d",
		gObj[iIndex].AccountID, gObj[iIndex].Name,
...
BOOL CJewelMixSystem::UnMixJewel(int iIndex, int iJewelType, int iJewelLevel, int iInventoryPos)
{
	...
	if ( gObjCheckInventorySerial0Item(&gObj[iIndex]))
	{
		...
		LogAdd(LOG_BLACK, "[ANTI-HACK][Serial 0 Item] [UnMix Jewel] (%s)(%s)",
			gObj[iIndex].AccountID, gObj[iIndex].Name);
	...
	if ( !CHECK_LIMIT(iJewelType, 2) )
	{
		LogAdd(LOG_BLACK, "[JewelMix] [%s][%s] UnMix iJewelType is out of bound : %d",
			gObj[iIndex].AccountID, gObj[iIndex].Name, iJewelType);
	...
	{
		LogAdd(LOG_BLACK, "[JewelMix] [%s][%s] iInventoryPos is out of bound : %d",
	...
	if ( gObj[iIndex].pInventory[iInventoryPos].IsItem() == TRUE )
	{
		...
		{
			LogAdd(LOG_BLACK, "[JewelMix] [%s][%s] iJewelLevel is different from request : %d / %d",
				gObj[iIndex].AccountID, gObj[iIndex].Name,
	...
	{
		LogAdd(LOG_BLACK, "[JewelMix] [%s][%s] Item to unmix is not exist",
			gObj[iIndex].AccountID, gObj[iIndex].Name);
	...
	{
		LogAdd(LOG_BLACK, "[JewelMix] [%s][%s] iInventoryItemLevel is out of bound : %d",
	...
	{
		LogAdd(LOG_BLACK, "[JewelMix] [%s][%s] iItemType is different from request : %d / %d",
			gObj[iIndex].AccountID, gObj[iIndex].Name,
	...
	if ( iMixMoney > gObj[iIndex].Money )
	{
		LogAdd(LOG_BLACK, "[JewelMix] [%s][%s] lack of money to unmix : %d / %d",
			gObj[iIndex].AccountID, gObj[iIndex].Name,
			gObj[iIndex].Money, iMixMoney);
	...
	{
		LogAdd(LOG_BLACK, "[JewelMix] [%s][%s] lack of empty slot to unmix : %d / %d",
			gObj[iIndex].AccountID, gObj[iIndex].Name,
	...
	LogAdd(LOG_BLACK, "[JewelMix] [%s][%s] UnMix - Delete Jewel, Type:%d, Level:%d, Serial:%d",
		gObj[iIndex].AccountID, gObj[iIndex].Name,
	...
	if ( bItemCrtOK == TRUE )
	{
		LogAdd(LOG_BLACK, "[JewelMix] [%s][%s] jewel unmix succeed : ItemType:%d, JewelCount:%d",
			gObj[iIndex].AccountID, gObj[iIndex].Name,
			iItemType, iJewelCount);
	...
	{
		LogAdd(LOG_BLACK, "[JewelMix] [%s][%s] jewel unmix failed : ItemType:%d, JewelCount:%d",
			gObj[iIndex].AccountID, gObj[iIndex].Name,
			iItemType, iJewelCount);
