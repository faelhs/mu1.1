...
// GS-CS	1.00.18	JPN	0x00424550	-	Completed
		...
		case 0xFF:	// Test Packet
			{
				...
				LogAdd(LOG_RED,  "error-L2DataServer RECV : %d (%d)", aLen, pMsg->count);
			}
			break;
	}
}
...
void TestDSSend()
{
	...
	LogAdd(LOG_BLACK, "%s", pMsg.testbuf );
	...
	LogAdd(LOG_BLACK, "Test Data Send");
}
...
void JGPGetCharList(unsigned char * lpRecv)
{
	...
	{
		LogAdd(LOG_RED, lMsg.Get(MSGGET(1, 165)), gObj[aIndex].AccountID, szId);
		CloseClient(aIndex);
	...
	if (pCLCount.Count > 0)
	{
		for (int n = 0; n < pCLCount.Count; n++)
		{
			...
			else	// Case DB 3
			{
				...
				if ( TempInventory[7] >= 100 && TempInventory[7] <= 130 )
				{
					pCList.CharSet[5] |= CS_SET_WING1(3);
					pCList.CharSet[17] |= (BYTE)(TempInventory[7] - 100 + 1) << 2;
				}
			}
...
void DataServerGetCharListRequest(short aIndex)
{
	...
	if ( AccountId == 0 )
	{
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 166)), __FILE__, __LINE__);
		CloseClient(aIndex);
		return;
	}
	if ( strlen(AccountId) < 1 )
	{
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 167)), __FILE__, __LINE__);
		CloseClient(aIndex);
		return;
	}
...
void JGCharacterCreateRequest(SDHP_CREATECHARRESULT* lpMsg)
{
	...
	{
		LogAdd(LOG_RED,  lMsg.Get(MSGGET(1, 168)), szAccountID);
		CloseClient(aIndex);
		return;
	}
...
void JGCharDelRequest(SDHP_CHARDELETERESULT * lpMsg)
{
	...
	if ( ::gObjIsAccontConnect(aIndex, szAccountId) == FALSE )
	{
		LogAdd(LOG_RED,  lMsg.Get(MSGGET(1, 169)), szAccountId);
		CloseClient(aIndex);
...
void JGGetCharacterInfo( SDHP_DBCHAR_INFORESULT * lpMsg)
{
	...
	if ( gObjIsAccontConnect(aIndex, szAccountId) == FALSE )
	{
		LogAdd(LOG_RED,  lMsg.Get(MSGGET(1, 170)), szAccountId);
		CloseClient(aIndex);
	...
	if ( lpMsg->result == false )
	{
		LogAdd(LOG_RED,  lMsg.Get(MSGGET(1, 171)), szName, gObj[aIndex].AccountID);
		CloseClient(aIndex);
	...
	{
		LogAdd(LOG_RED,  "error-L1 : Load Block Character infomation", szName, gObj[aIndex].AccountID);	// Deathway Fix HERE
		CloseClient(aIndex);
	...
	for (int i=OBJ_STARTUSERINDEX;i<OBJMAX;i++)
	{
		if ( gObj[i].Connected == PLAYER_PLAYING )
		{
			if ( gObj[i].Type == OBJ_USER )
			{
				...
				{
					LogAdd(LOG_BLACK, "[Anti-HACK][JGGetCharacterInfo] Attempted Character-Copy by double logging [%s][%s]",
	...
	{
		LogAdd(LOG_RED,  lMsg.Get(MSGGET(1, 172)), szName);
		CloseClient(aIndex);
	...
	{
		LogAdd(LOG_RED,  lMsg.Get(MSGGET(1, 173)), __FILE__, __LINE__);
		CloseClient(aIndex);
	...
	if ( lpObj->Money < 0 )
	{
		LogAdd(LOG_RED,  "★☆ ZEN MODIFY ☆★ [%s][%s] %d -> 0", szName, gObj[aIndex].AccountID, lpObj->Money);
		lpObj->Money = 0;
	}
	...
	if ( lpObj->m_bMapSvrMoveReq == false )
	{
		...
		if ( wGameServerCode != gGameServerCode )
		{
			...
			{
				LogAdd(LOG_RED, "[MapServerMng] Map Server Move Fail : CheckMoveMapSvr() == -1 [%s][%s] (%d)",
			...
			LogAdd(LOG_BLACK, "[MapServerMng] Request to Move Map Server : (%d) - [%s][%s] (%d)",
	...
	pjMsg.iUpPoint = lpObj->LevelUpPoint;
	...
	LogAdd(LOG_BLACK, "[FRUIT System] [%s][%s] (MinusPoint:%d/PlusPoint:%d) (MaxMinus:%d/MaxPlus:%d)",
	...
	{
		LogAdd(LOG_BLACK, "[FRUIT System] Character Stat Error [%s][%s] (MinusPoint:%d/PlusPoint:%d) (MaxMinus:%d/MaxPlus:%d)",
			lpObj->AccountID, lpObj->Name, MinusPoint, AddPoint, MaxMinusPoint, MaxAddPoint);
	}
...
void GJSetCharacterInfo(LPOBJ lpObj, int aIndex, BOOL bMapServerMove)
{
	...
	{
		LogAdd(LOG_RED, "[MapServerMng] GJSetCharacterInfo() - Inventory Already Saved [%s][%s] (%d)",
			lpObj->AccountID, lpObj->Name, lpObj->m_Index);
	...
	if ( bMapServerMove == TRUE )
	{
		...
		LogAdd(LOG_BLUE, "[MapServerMng][%s][%s] GJSetCharacterInfo() - MapServerMove Character Save Start (%d)",
			lpObj->AccountID, lpObj->Name, lpObj->m_Index);
	}
...
/*#if (FOREIGN_GAMESERVER==1)
	...
	{
		LogAdd(LOG_RED,  lMsg.Get(MSGGET(1, 174)), lpObj->Name);
		return;
	}
...
void DGGetWarehouseList(SDHP_GETWAREHOUSEDB_SAVE * lpMsg)
{
	...
	{
		LogAdd(LOG_RED,  lMsg.Get(MSGGET(1, 175)), szId, aIndex);
		return;
	}
	// ---
	LPOBJ lpObj = &gObj[aIndex];
	...
	for(int n = 0; n < WAREHOUSE_SIZE; n++)
	{
		...
		if(itype != -1)
		{
			...
			{
				item.Convert(itype, item.m_Option1, item.m_Option2, item.m_Option3, DBI_GET_NOPTION(lpMsg->dbItems[n*dbsize+DBI_NOPTION_DATA]), lpMsg->dbItems[n*dbsize+DBI_SOPTION_DATA], 0, 1);
			}
			else
			{
				item.Convert(itype, item.m_Option1, item.m_Option2, item.m_Option3, DBI_GET_NOPTION(lpMsg->dbItems[n*dbsize+DBI_NOPTION_DATA]), lpMsg->dbItems[n*dbsize+DBI_SOPTION_DATA], 0, 3);
			}
			// ---
...
void GDGetWarehouseNoItem( SDHP_GETWAREHOUSEDB_RESULT * lpMsg)
{
	...
	if ( lpObj->m_IfState.type == 6 )
	{
		if ( lpObj->m_IfState.state == 0 )
		{
			...
			DataSend(lpObj->m_Index, (LPBYTE)&pResult, pResult.h.size);
			LogAdd(LOG_RED,  lMsg.Get(MSGGET(1, 176)), lpObj->Name);
		}
	}
...
void GDSetWarehouseList(int aIndex)
{
	...
	{
		LogAdd(LOG_BLACK, "[%s][%s] WareHouse Save Fail : Not Open",
...
void ItemSerialCreateSend(int aIndex, BYTE MapNumber, BYTE x, BYTE y, int type, BYTE level, BYTE dur, BYTE Op1, BYTE Op2, BYTE Op3, int LootIndex, BYTE NewOption, BYTE SetOption)
{
	...
	if ( OBJMAX_RANGE(aIndex) != FALSE  && gObj[aIndex].Type != OBJ_USER )
	{
		...
		LogAdd(LOG_BLACK, "Monster Item Drop Request [%s] [%d][%d][%d] : [%s][%d][%d][%d][%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set:[%d]",
...
void ItemSerialCreateRecv(SDHP_ITEMCREATERECV * lpMsg)
{
	...
	if ( lpMsg->MapNumber == (BYTE)-1 || lpMsg->MapNumber == (BYTE)-2 )
	{
		...
		if ( lpMsg->MapNumber == (BYTE)-2 )
		{
			...
			{
				LogAdd(LOG_BLACK, "error-L2: DarkTrainerBox not used.");
				return;
			}
		}
		else if ( lpObj->m_IfState.type != 7 )
		{
			LogAdd(LOG_BLACK, "error-L2: ChaosBox not used.");
			return;
		}
		...
		LogAdd(LOG_BLACK, "[%s][%s] CBMix Item Create Item:%s [%d][%d] [%d][%d][%d][%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set:[%d]",
	...
	if ( lpMsg->MapNumber == 235 )
	{
		if ( gObj[lpMsg->aIndex].Connected > PLAYER_CONNECTED )
		{
			...
			if ( iItemPos == (BYTE)-1 )
			{
				LogAdd(LOG_BLACK, "[Mu_2Anv_Event] Error : Failed To Gift Item Type:%d, TypeIndex:%d to [%s][%s]",
	...
	else if ( lpMsg->MapNumber == 236 )
	{
		if ( gObj[lpMsg->aIndex].Connected > PLAYER_CONNECTED )
		{
			...
			if ( btItemPos == (BYTE)-1 )
			{
				LogAdd(LOG_BLACK, "[CashShop] Error : Failed To Insert Item Type:%d, TypeIndex:%d to [%s][%s]",
	...
	else
	{
		...
		LogAdd(LOG_BLACK, "Monster Item Drop [%d][%d][%d] : serial:%d [%s][%d][%d][%d][%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set:[%d]",
...
void DGMoveOtherServer(SDHP_CHARACTER_TRANSFER_RESULT * lpMsg)
{
	...
	if ( lpMsg->Result != 0 )
	{
		...
		LogAdd(LOG_BLACK, "[CharTrasfer] Fail [%s][%s] (%d)",
	...
	LogAdd(LOG_BLACK, "[CharTrasfer] Success [%s][%s] (%d)",
		lpObj->AccountID, lpObj->Name, lpMsg->Result);
...
void DGRecvPetItemInfo(LPBYTE lpData)
{
	...
	{
		LogAdd(LOG_RED,  "Request to receive petitem infomation doesn't match the user %s", szAccountId);
		CloseClient(aIndex);
...
void gObjSavePetItemInfo(int aIndex, int inventype)
{
	...
	if(inventype == 0)
	{
		for(int n = 0; n < INVENTORY_SIZE; n++)
		{
			if(gObj[aIndex].pInventory[n].IsItem())
			{
				if(gObj[aIndex].pInventory[n].m_Type == ITEMGET(13,4) || gObj[aIndex].pInventory[n].m_Type == ITEMGET(13,5))
				{
					if(gObj[aIndex].pInventory[n].m_IsLoadPetItemInfo != FALSE)
					{
						founditemcount++;
						// ---
						petIteminfo.nSerial		= gObj[aIndex].pInventory[n].m_Number;
						petIteminfo.Level		= gObj[aIndex].pInventory[n].m_PetItem_Level;
						petIteminfo.Exp			= gObj[aIndex].pInventory[n].m_PetItem_Exp;
						// ---
						LogAdd(LOG_BLACK, "[%s][%s][PI][%d,%s] Serial:[%d] Level:%d Exp:%d", 
							gObj[aIndex].AccountID, 
							gObj[aIndex].Name, 
							n, 
							gObj[aIndex].pInventory[n].GetName(),
							petIteminfo.nSerial,
							petIteminfo.Level, 
							petIteminfo.Exp);
						// ---
						memcpy(&szbuffer[lofs], &petIteminfo, sizeof(petIteminfo));
						// ---
						lofs += sizeof(petIteminfo);
					}
				}
	...
	else if(inventype == 1)
	{
		for(int n = 0; n < WAREHOUSE_SIZE; n++)
		{
			if(gObj[aIndex].pWarehouse[n].IsItem() )
			{
				if(gObj[aIndex].pWarehouse[n].m_Type == ITEMGET(13,4) || gObj[aIndex].pWarehouse[n].m_Type == ITEMGET(13,5))
				{
					if(gObj[aIndex].pWarehouse[n].m_IsLoadPetItemInfo != FALSE)
					{
						founditemcount++;
						// ---
						petIteminfo.nSerial		= gObj[aIndex].pWarehouse[n].m_Number;
						petIteminfo.Level		= gObj[aIndex].pWarehouse[n].m_PetItem_Level;
						petIteminfo.Exp			= gObj[aIndex].pWarehouse[n].m_PetItem_Exp;
						// ---
						LogAdd(LOG_BLACK, "[%s][%s][PW][%d,%s]serial:[%d] Level:%d Exp:%d",
							gObj[aIndex].AccountID, 
							gObj[aIndex].Name, 
							n, 
							gObj[aIndex].pWarehouse[n].GetName(),
							petIteminfo.nSerial,
							petIteminfo.Level, 
							petIteminfo.Exp);
						// ---
	...
	if(founditemcount)
	{
		...
		pMsg.nCount		= founditemcount;
		// ---
		memcpy(szbuffer, &pMsg, sizeof(pMsg));
		// ---
		cDBSMng.Send(szbuffer, lofs+sizeof(pMsg));
	}
}
...
void GS_DGAnsCastleTotalInfo(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x80][0x00] GS_DGAnsCastleTotalInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
#endif
}
...
void GS_DGAnsOwnerGuildMaster(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x80][0x00] GS_DGAnsCastleTotalInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");	// #error on Msg
		return;
	}
...
void GS_DGAnsCastleNpcBuy(unsigned char * lpRecv )
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x80][0x03] GS_DGAnsCastleNpcBuy() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
	...
	if ( lpMsg->iResult == 1 )
	{
		...
		if ( bRET == TRUE )
		{
			...
			LogAdd(LOG_BLACK, "[CastleSiege] GS_DGAnsCastleNpcBuy() - CCastleSiege::AddDbNPC() OK - Npc:(CLS:%d, IDX:%d)",
				lpMsg->iNpcNumber, lpMsg->iNpcIndex);
		}
		else
		{
			LogAdd(LOG_BLACK, "[CastleSiege] GS_DGAnsCastleNpcBuy() - CCastleSiege::AddDbNPC() FAILED - Npc:(CLS:%d, IDX:%d)",
				lpMsg->iNpcNumber, lpMsg->iNpcIndex);
		}
	}
	else
	{
		LogAdd(LOG_BLACK, "[CastleSiege] GS_DGAnsCastleNpcBuy() FAILED - Result:(%d), Npc:(CLS:%d, IDX:%d)",
...
void GS_DGAnsCastleNpcRepair(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x80][0x04] GS_DGAnsCastleNpcRepair() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
	...
	if ( lpMsg->iResult == 1 )
	{
		...
		if ( bRET == TRUE )
		{
			...
			LogAdd(LOG_BLACK, "[CastleSiege] GS_DGAnsCastleNpcRepair() - CCastleSiege::RepairDbNPC() OK - Npc:(CLS:%d, IDX:%d)",
				lpMsg->iNpcNumber, lpMsg->iNpcIndex);
		}
		else
		{
			LogAdd(LOG_BLACK, "[CastleSiege] GS_DGAnsCastleNpcRepair() - CCastleSiege::RepairDbNPC() FAILED - Npc:(CLS:%d, IDX:%d)",
				lpMsg->iNpcNumber, lpMsg->iNpcIndex);
		}
	}
	else
	{
		LogAdd(LOG_BLACK, "[CastleSiege] GS_DGAnsCastleNpcRepair() FAILED - Result:(%d), Npc:(CLS:%d, IDX:%d)",
			lpMsg->iResult, lpMsg->iNpcNumber, lpMsg->iNpcIndex);
	}
...
void GS_DGAnsCastleNpcUpgrade(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x80][0x05] GS_DGAnsCastleNpcUpgrade() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}

	if ( lpMsg->iResult == 0 )
	{
		LogAdd(LOG_RED,  "[CastleSiege] ERROR - Castle NPC Upgrade Fail() (CLS:%d, IDX:%d, UPTYPE:%d, UPVAL:%d)",
	...
	else
	{
		...
		LogAdd(LOG_BLACK, "[CastleSiege] [0x80][0x06] GS_DGAnsTaxInfo() - Npc Upgrade OK (CLS:%d, IDX:%d, UPTYPE:%d, UPVAL:%d)",
			lpMsg->iNpcNumber, lpMsg->iNpcIndex, lpMsg->iNpcUpType, lpMsg->iNpcUpValue);
	}
...
void GS_DGAnsTaxInfo(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x80][0x06] GS_DGAnsTaxInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsTaxRateChange(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x80][0x07] GS_DGAnsTaxRateChange() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsCastleMoneyChange(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x80][0x08] GS_DGAnsCastleMoneyChange() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
	...
	if ( lpMsg->iResult == 1 )
	{
		if ( gObjIsConnected(lpMsg->iIndex) )
		{
			if ( lpMsg->iMoneyChanged < 0 )
			{
				...
				LogAdd(LOG_BLACK, "[CastleSiege] [0x80][0x08] GS_DGAnsCastleMoneyChange() - Withdraw Request OK [%s][%s] (ReqMoney:%d, TotMoney:%I64d)",
...
void GS_DGAnsSiegeDateChange(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x80][0x09] GS_DGAnsSiegeDateChange() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsGuildMarkRegInfo(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x80][0x0A] GS_DGAnsGuildMarkRegInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsSiegeEndedChange(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x80][0x0B] GS_DGAnsSiegeEndedChange() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
#endif
}
...
void GS_DGAnsCastleOwnerChange(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x80][0x0C] GS_DGAnsCastleOwnerChange() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
#endif
}
...
void GS_DGAnsRegAttackGuild(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x80][0x0D] GS_DGAnsRegAttackGuild() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsRestartCastleState(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x80][0x0E] GS_DGAnsRestartCastleState() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsMapSvrMsgMultiCast(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x80][0x0F] GS_DGAnsMapSvrMsgMultiCast() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
	char szNotice[144] = {0};
...
void GS_DGAnsRegGuildMark(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x80][0x10] GS_DGAnsRegGuildMark() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsGuildMarkReset(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x80][0x11] GS_DGAnsGuildMarkReset() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
#endif
}
...
void GS_DGAnsGuildSetGiveUp(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x80][0x12] GS_DGAnsGuildSetGiveUp() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsNpcRemove(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x80][0x16] GS_DGAnsNpcRemove() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
#endif
}
...
void GS_DGAnsCastleTributeMoney(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x80][0x18] GS_DGAnsCastleTributeMoney() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
	
	LogAdd(LOG_BLACK, "[CastleSiege] [0x80][0x18] GS_DGAnsCastleTributeMoney() - Money Tribute OK (%d)",
...
void GS_DGAnsResetCastleTaxInfo(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x80][0x19] GS_DGAnsResetCastleTaxInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsResetSiegeGuildInfo(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x80][0x1A] GS_DGAnsResetSiegeGuildInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
#endif
}
...
void GS_DGAnsResetRegSiegeInfo(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x80][0x1B] GS_DGAnsResetRegSiegeInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
#endif
}
...
void GS_DGAnsCastleInitData(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x81] GS_DGAnsCastleInitData() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}

	if ( g_CastleSiege.GetDataLoadState() != CASTLESIEGE_DATALOAD_2 )
	{
		LogAdd(LOG_RED,  "[CastleSiege] CASTLE SIEGE DATA SETTING FAILED [0x81] - m_iCastleDataLoadState != CASTLESIEGE_DATALOAD_2 (%d)",
...
void GS_DGAnsCastleNpcInfo(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x82] GS_DGAnsCastleNpcInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
#endif
}
...
void GS_DGAnsAllGuildMarkRegInfo(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x83] GS_DGAnsAllGuildMarkRegInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsFirstCreateNPC(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x84] GS_DGAnsFirstCreateNPC() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsCalcREgGuildList(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x85] GS_DGAnsCalcREgGuildList() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsCsGulidUnionInfo(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x86] GS_DGAnsCsGulidUnionInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsCsSaveTotalGuildInfo(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x87] GS_DGAnsCsSaveTotalGuildInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsCsLoadTotalGuildInfo(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x88] GS_DGAnsCsLoadTotalGuildInfo() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}
...
void GS_DGAnsCastleNpcUpdate(LPBYTE lpRecv)
{
	...
	if ( lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup() )
	{
		LogAdd(LOG_RED,  "[CastleSiege] PACKET-ERROR [0x89] GS_DGAnsCastleNpcUpdate() - lpMsg->wMapSvrNum != g_MapServerManager.GetMapSvrGroup()");
		return;
	}

	LogAdd(LOG_BLACK, "[CastleSiege] [0x89] GS_DGAnsCastleNpcUpdate() - Npc Data Update Result : '%s'", (lpMsg->iResult) ? "SUCCEED" : "FAIL");
#endif
}