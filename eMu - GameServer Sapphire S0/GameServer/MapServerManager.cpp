...
short CMapServerManager::CheckMoveMapSvr(int iIndex, int iMAP_NUM, short sSVR_CODE_BEFORE)
{
	...
	switch ( sMAP_MOVE_INFO )
	{
		case -1:
			{
				...
				{
					LogAdd(LOG_BLACK, "[MapServerMng] CheckMoveMapSvr() - MapServer Check OK [%s][%s] : MAP-%d / SVR-%d(State Map:%d X:%d Y:%d)",
		...
		default:
			...
			{
				LogAdd(LOG_RED,  "[MapServerMng] CheckMoveMapSvr() - Unknown MapMove Info [%s][%s] : MAP-%d / INFO-%d",
