...
void CItem::Convert(int type, BYTE Option1, BYTE Option2, BYTE Option3,  BYTE Attribute2, BYTE SetOption, BYTE ItemEffectEx,  BYTE DbVersion)
{
	...
	{
		LogAdd(LOG_RED,  "error-L1 : ItemIndex error %d", _type);
	}
	...
	if(((_type >= ITEMGET(12,0) ) && (_type <= ITEMGET(12,6) ) ) || (_type == ITEMGET(13,30)) || NEWWINGS(_type))
	{
		Attribute2=0;
	}
	...
	if ( gSetItemOption.IsSetItem(_type) != 0 && SetOption != 255 && SOption != 0)
	{
		...
		// ---
		if(this->m_SetOption != 0 && gServerInfo.m_Data.iAncientAndExcellent == 0)
		{
			this->m_NewOption = 0;
			Attribute2 = 0;
		}
	...
	if ( p->Defense > 0 )	
	{
		...
		else 
		{
			...
			if ( ( _type >= ITEMGET(12,3) && _type <= ITEMGET(12,6)) || _type == ITEMGET(13,30) || _type == ITEMGET(13,4) )
			{
	...
	if ( Option3 != 0 )
	{
		...
		if ( _type == ITEMGET(12, 104) || _type == ITEMGET(12, 106) )
		{
			this->m_Option3 = Option3;
			this->m_Special[this->m_SpecialNum] = 85;
		...
		if ( _type == ITEMGET(12, 111) || _type == ITEMGET(12, 113) )
		{
			this->m_Option3 = Option3;
			this->m_Special[this->m_SpecialNum] = 85;
		...
		if((_type >= ITEMGET(12,114)) && (_type <= ITEMGET(12,130)))
		{
			this->m_Option3 = Option3;
			this->m_Special[this->m_SpecialNum] = 85;
...
BOOL CItem::IsClass(char aClass, int ChangeUP)
{
	...
	{
		LogAdd(LOG_BLACK, "클래스 에러 : %d (%s %d)", aClass, __FILE__, __LINE__); // 클래스 에러 : %d (%s %d)
		return 0;
	}
...
void CItem::SetPetItemInfo(int petlevel, int petexp)
{
	if ( this->m_Type == ITEMGET(13,4) )
	{
		...
		this->Convert(this->m_Type, this->m_Option1, this->m_Option2,
			this->m_Option3, this->m_NewOption, this->m_SetOption, 0, CURRENT_DB_VERSION);
...
int CItem::NormalWeaponDurabilityDown(int defense, int aIndex)
{
	...
	if ( damagemin == 0 )
	{
		LogAdd(LOG_BLACK, "damagemin이 0이다");
		return 0;
	}
	...
	if ( this->m_Durability == 0.0f )
	{
		if ( this->m_CurrentDurabilityState != 1.0f )
		{
			...
			this->Convert(this->m_Type, this->m_Option1, this->m_Option2, this->m_Option3, this->m_NewOption,
				this->m_SetOption, 0, CURRENT_DB_VERSION);
			return true;
		}
	}
	else if ( this->m_Durability < this->m_DurabilityState[2] )
...
int ItemGetNumberMake(int type, int index)
{
	...
	{
		LogAdd(LOG_BLACK, "Error -> Wrong item size: %d", make);
		return -1;
	}
	return make;
}
...
int ItemGetDurability(int index, int itemLevel, int ExcellentItem, int SetItem)
{
	...
	else if ( itemLevel >= 5 )
	{
		...
		{
			dur=ItemAttribute[index].Durability + itemLevel*2+7;
		}
