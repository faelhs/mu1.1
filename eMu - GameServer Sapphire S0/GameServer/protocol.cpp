...
BOOL DataEncryptCheck(int aIndex, BYTE protoNum, BOOL Encrypt)
{
	if ( Encrypt == FALSE )
	{
		LogAdd(LOG_BLACK, "Error-L1 : Not Encrypt %s %d", gObj[aIndex].AccountID, protoNum);
		CloseClient(aIndex);
		return FALSE;
	}
	...
	else
	{
		...
		switch ( protoNum )
		{
			...
			case 0x1E:
				if ( DataEncryptCheck(aIndex, protoNum, Encrypt) != FALSE )
				{
					if(lpObj->pInventory[0].m_Type == OBJ_EMPTY && aRecv[3] == 0x29)
					{
			...
			case 0xFB: // -> Reconnect
				{
					...
					case 12:
						{
							gReconnect.SendLogin(aIndex, (CONNECTEX_LOGIN *)aRecv);
						}
						break;
					}
					// ---
				...
				break;
			default:
				LogAdd(LOG_RED,  "error-L2 : account:%s name:%s HEAD:%x (%s,%d) State:%d",
...
void CGLiveClient(PMSG_CLIENTTIME * lpClientTime, short aIndex)
{
	...
	if ( gObj[aIndex].CheckSpeedHack != false )
	{
		...
		if ( checktime < -7000 )
		{
			LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 214)), gObj[aIndex].AccountID, gObj[aIndex].Name, checktime);
			CloseClient(aIndex);
		}
	...
	if ( gObj[aIndex].Connected == PLAYER_PLAYING )
	{
		...
		{
			/*LogAdd(LOG_RED,  "[%s][%s][%s] ClientHack Detected : Editing AttackSpeed [%d][%d] [%d][%d]",
...
{
	gServerInfo.SetInfo(aIndex);
	// ---
	if(strcmp(gObj[aIndex].Ip_addr, "127.0.0.1") == 0)
	{
	...
	{
		LogAdd(LOG_BLACK, "error-L1 : [%s][%s] CheckSum-Exe error ", gObj[aIndex].AccountID, gObj[aIndex].Name);
		CloseClient(aIndex);
		return;
	}
	...
	{
		LogAdd(LOG_BLACK, "error-L1 : CheckSum-Exe error %d %d %d [%s]", dwgCheckSum[gObj[aIndex].CheckSumTableNum],
...
void PChatProc(PMSG_CHATDATA * lpChat, short aIndex)
{
	...
	{
		LogAdd(LOG_BLACK, "[Anti-HACK][PChatProc][%s][%s] Chat Message Len : %d", lpObj->AccountID, lpObj->Name, slen);
		return;
	}
	...
	gCommands.Manager(lpChat, aIndex);
		...
		case '!':	// Global Announcement
			if ( slen > 2 )
			{
				if ( (lpObj->AuthorityCode&1)==1)
				{
					...
					LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 215)), gObj[aIndex].AccountID, gObj[aIndex].Name, &lpChat->chatmsg[1]);
			...
			if ( lpChat->chatmsg[1] == '>' )
			{
				if ( lpObj->Name[0] == lpObj->lpGuild->Names[0][0] )
				{
					if ( !strcmp(lpObj->Name, lpObj->lpGuild->Names[0] ) )
					{
						...
						LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 216)), lpObj->lpGuild->Name, lpChat->chatmsg);
...
void CSPJoinIdPassRequest(PMSG_IDPASS* lpMsg, int aIndex)
{
	...
	{
		LogAdd(LOG_RED,  "error-L1: Serial error [%s] [%s]", id, serial);
		GCJoinResult(JS_BAD_CLIENT_VERSION, aIndex);
		CloseClient(aIndex);
		return;
	}
	...
	if (PacketCheckTime(lpObj) == FALSE )
	{
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 217)), aIndex, id);
		CloseClient(aIndex);
		return;
	}
	...
	if ( gObj[aIndex].Connected != PLAYER_CONNECTED )
	{
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 218)), aIndex, id);
		CloseClient(aIndex);
		return;
	}
	...
	if ( gObj[aIndex].LoginMsgSnd != FALSE )
	{
		...
		{
			LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 219)), aIndex, id, gObj[aIndex].Ip_addr );
		}
		else
		{
			LogAdd(LOG_BLACK, "error : %d %s %d", gObj[aIndex].LoginMsgSnd, __FILE__, __LINE__);
		}
	...
	LogAdd(LOG_BLACK, "join send : (%d)%s", aIndex, gObj[aIndex].AccountID);
...
void CGClientCloseMsg(PMSG_CLIENTCLOSE * lpMsg, int aIndex)
{
	switch ( lpMsg->Flag )
	{
		case 0:	// Close Game
			...
			LogAdd(LOG_BLACK, "[UserSelClose] Go Exit [%s][%s] Map:[%d][%d/%d]",
		...
		case 2:	// ServerList
			...
			LogAdd(LOG_BLACK, "[UserSelClose] Go ServerList [%s][%s] Map:[%d][%d/%d]",
				gObj[aIndex].AccountID, gObj[aIndex].Name,
				gObj[aIndex].MapNumber, gObj[aIndex].X, gObj[aIndex].Y);
			break;
		...
		case 1:	// Select Character
			...
			LogAdd(LOG_BLACK, "[UserSelClose] Go CharSelect [%s][%s] Map:[%d][%d/%d]",
				gObj[aIndex].AccountID, gObj[aIndex].Name,
				gObj[aIndex].MapNumber, gObj[aIndex].X, gObj[aIndex].Y);
			break;
	}
...
void CGClientMsg(PMSG_CLIENTMSG* lpMsg, int aIndex)
{
	if ( lpMsg->Flag )
	{
		LogAdd(LOG_RED, "error-L1 : HackCheck [%s][%s] 0x%x 0x%x",
			gObj[aIndex].AccountID, gObj[aIndex].Name,
...
void CGPCharacterCreate( PMSG_CHARCREATE * lpMsg, int aIndex)
{
	...
	{
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 220)), aIndex, gObj[aIndex].Ip_addr);
		CloseClient(aIndex);
		return;
	}
	...
	if ( gObj[aIndex].Connected == PLAYER_PLAYING )
	{
		LogAdd(LOG_BLACK, "[Anti-HACK][CGPCharacterCreate] Attempted Character Create during GamePlay. [%s][%s]",
			gObj[aIndex].AccountID, gObj[aIndex].Name);
		CloseClient(aIndex);
		return;
	}
	...
	if ( !gCreateCharacter )
	{
		GCServerMsgStringSend("[ANTI-HACK][CreateCharacter][%s][%s] Attempted Creating Character During GamePlay.", aIndex, 1);
		JGCharacterCreateFailSend(aIndex, lpMsg->Name);
		return;
	}
	...
	if ( lpMsg->ClassSkin == 0x30 ) // MG
	{
		...
		{
			LogAdd(LOG_RED,  "error-L1: Magumsa Character create error [%s]", gObj[aIndex].AccountID);
			JGCharacterCreateFailSend(aIndex, lpMsg->Name);
			return;
		}
	}
	if ( lpMsg->ClassSkin == 0x40 ) // DL
	{
		...
		{
			LogAdd(LOG_RED,  "error-L1: Darklord Character create error [%s]", gObj[aIndex].AccountID);
			JGCharacterCreateFailSend(aIndex, lpMsg->Name);
			return;
		}
	}
	...
	LogAdd(LOG_BLACK, "Character create : %s,%s", szTemp2, szTemp);
...
void CGPCharDel(PMSG_CHARDELETE * lpMsg,int aIndex)
{
	...
	if ( gObj[aIndex].Connected == PLAYER_PLAYING )
	{
		LogAdd(LOG_BLACK, "[Anti-HACK][CGPCharDel] Attempted Character Delete during GamePlay. [%s][%s]",
			gObj[aIndex].AccountID, gObj[aIndex].Name);
		CloseClient(aIndex);
		return;
	}
	...
	{
		LogAdd(LOG_BLACK, "[Anti-HACK][CGPCharDel] Attempted Character Delete during Prevented Time. [%s][%s]",
			gObj[aIndex].AccountID, gObj[aIndex].Name);
		CloseClient(aIndex);
		return;
	}
	...
	LogAdd(LOG_BLACK, "Character delete : %s,%s", szTemp2, szTemp);
...
void GCLevelUpMsgSend(int aIndex, int iSendEffect)
{
	...
	pMsg.iUpPoint = gObj[aIndex].LevelUpPoint;
...
void GCSkillKeyRecv(PMSG_SKILLKEY * lpMsg, int aIndex)
{
	...
	LogAdd(LOG_BLACK, "Option Recv %d %d %d %d", lpMsg->GameOption,
...
void GCSkillKeySend(int aIndex, LPBYTE keybuffer, BYTE GO, BYTE Qk, BYTE Wk, BYTE Ek, BYTE ChatWnd)
{
	...
	DataSend(aIndex, (UCHAR*)&pMsg, pMsg.h.size);
	LogAdd(LOG_BLACK, "Option Send %d %d %d %d", GO, Qk, Wk, Ek);
}
...
void CGItemGetRequest(PMSG_ITEMGETREQUEST * lpMsg, int aIndex)
{
	...
	if ( !::gObjFixInventoryPointer(aIndex))
		LogAdd(LOG_BLACK, "[Fix Inv.Ptr] False Location - %s, %d", __FILE__, __LINE__);

	if ( gObj[aIndex].pTransaction == TRUE )
	{
		LogAdd(LOG_BLACK, "[%s][%s] CGItemGetRequest() Failed : Transaction == 1, IF_TYPE : %d",
	...
	if ( MAX_ITEM_TYPE_RANGE(item_num) == FALSE )
	{
		...
		LogAdd(LOG_BLACK, "error-L3 : %s %d", __FILE__, __LINE__);
		pResult.result = -1;
		pResult.h.size -= sizeof(pResult.Data);
		DataSend(aIndex, (UCHAR*)&pResult, pResult.h.size);
		return;
	...
	if ( lpItem->IsItem() == TRUE && lpItem->Give == false && lpItem->live == true)
	{
		if ( lpItem->m_QuestItem != false )
		{
			...
			if ( ItemCount > 0 )
			{
				LogAdd(LOG_BLACK, "[Quest] Too many have quest item [%s][%s] (%s) %d",
		...
		if ( lpItem->m_Type == ITEMGET(13,20) )	// Wizard Ring
		{
			switch ( lpItem->m_Level )
			{
				case 0:
				{
					...
					if ( iWRCount > 0 )
					{
						LogAdd(LOG_BLACK, "[Ring Event] Too many have Magician's Ring [%s][%s] (Name:%s, Count:%d)",
		...
		if ( lpItem->m_Type == ITEMGET(13,38) )	// MoonStonePendant
		{
			...
			if ( count > 0 )
			{
				LogAdd(LOG_BLACK, "[ MoonStonePendant ] Too many have MoonStonePendant [%s][%s] ( Name:%s, Count:%d )",
		...
		if ( lpItem->m_Type == ITEMGET(13,39) )	// ChangeRing
		{
			...
			if ( count > 0 )
			{
				LogAdd(LOG_BLACK, "[ ChangeRing ] Too many have ChangeRing [%s][%s] ( Name:%s, Count:%d )",
					gObj[aIndex].AccountID, gObj[aIndex].Name, lpItem->GetName(), count);
				...
				for ( int n=0;n<MAIN_INVENTORY_SIZE;n++)
				{
					...
					if ( MAIN_INVENTORY_RANGE(pos) )
					{
						if ( MapC[map_num].ItemGive(aIndex, item_num, true) == TRUE )
						{
							...
							LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 221)), gObj[aIndex].AccountID, gObj[aIndex].Name, map_num,
								...
								NewOption[6], lpItem->m_SetOption, 0,0,0);
							...
							if ( gObj[aIndex].pInventory[pos].m_Durability >= 5.0f )
							{
								...
								LogAdd(LOG_BLACK, "[Kalima] [%s][%s] Make Lost Kalima Map (Left Kundun Mark:%d)",
					...
					else
					{
						...
						if ( BC_MAP_RANGE(map_num) )
						{
							if ( g_BloodCastle.CheckQuestItemSerial(map_num-MAP_INDEX_BLOODCASTLE1, lpItem) )
							{
								...
								LogAdd(LOG_BLACK, "[Blood Castle] (%d) (Account:%s, Name:%s) picked up Angel King's Weapon (%d)",
...
BOOL CGItemDropRequest(PMSG_ITEMTHROW * lpMsg, int aIndex, BOOL drop_type)
{
	...
	{
		LogAdd(LOG_BLACK, "[ANTI-HACK][Item Duplication] - Item Drop during MapServer Move [%s][%s]",
			gObj[aIndex].AccountID, gObj[aIndex].Name);
		pResult.Result = false;
	}
	...
	{
		LogAdd(LOG_BLACK, "[%s][%s] CGItemDropRequest() Failed : Transaction == 1, IF_TYPE : %d",
			gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].m_IfState.type);
		pResult.Result = false;
	...
	{
		LogAdd(LOG_BLACK, "error:%s %d", __FILE__, __LINE__);
		pResult.Result = false;
	}
	...
	if ( (lpObj->pInventory[lpMsg->Ipos].m_Type >= ITEMGET(13,0) && lpObj->pInventory[lpMsg->Ipos].m_Type <= ITEMGET(13,3))
		...
		|| NEWWINGS(lpObj->pInventory[lpMsg->Ipos].m_Type))
	{
	...
	if ( pResult.Result == 1 )
	{
		...
		if ( gObjCheckSerial0ItemList(&lpObj->pInventory[lpMsg->Ipos]) != FALSE )
		{
			...
			LogAdd(LOG_BLACK, "[ANTI-HACK][Serial 0 Item] [ItemDrop] (%s)(%s) Item(%s) Pos(%d)",
		...
		if ( bIsItem == TRUE )
		{
			...
			if ( type == ITEMGET(14,11)) // Box of Luck until Kunduns
			{
				...
				if ( level == 1 )
				{
					...
					LogAdd(LOG_BLACK, "[%s][%s][%d]%d/%d Used box of StarOfXMas Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
				...
				else if ( level == 2 )
				{
					...
					LogAdd(LOG_BLACK, "[%s][%s][%d]%d/%d Used box of FireCracker Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
						lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
						serial, szItemName, type, level, Option1, Option2, Option3);
				}
				else if ( level == 3 )
				{
					...
					LogAdd(LOG_BLACK, "[%s][%s][%d]%d/%d Used box of HeartOfLove Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
						lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
						serial, szItemName, type, level, Option1, Option2, Option3);
				}
				else if ( level == 6 )
				{
					...
					LogAdd(LOG_BLACK, "[%s][%s][%d]%d/%d Used box of GoldMedal Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
						lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
						serial, szItemName, type, level, Option1, Option2, Option3);
				}
				else if ( level == 5 )
				{
					...
					LogAdd(LOG_BLACK, "[%s][%s][%d]%d/%d Used box of SilverMedal Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
						lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
						serial, szItemName, type, level, Option1, Option2, Option3);
				}
				else if ( level == 7 )
				{
					...
					LogAdd(LOG_BLACK, "[%s][%s][%d]%d/%d Used box of EventChip Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
						lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
						serial, szItemName, type, level, Option1, Option2, Option3);
				}
				else if ( level == 8 )
				{
					...
					LogAdd(LOG_BLACK, "[%s][%s][%d]%d/%d Used box of ElradoraBox-8 Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
						lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
						serial, szItemName, type, level, Option1, Option2, Option3);
				}
				else if ( level == 9 )
				{
					...
					LogAdd(LOG_BLACK, "[%s][%s][%d]%d/%d Used box of ElradoraBox-9 Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
						lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
						serial, szItemName, type, level, Option1, Option2, Option3);
				}
				else if ( level == 10 )
				{
					...
					LogAdd(LOG_BLACK, "[%s][%s][%d]%d/%d Used box of ElradoraBox-10 Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
						lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
						serial, szItemName, type, level, Option1, Option2, Option3);
				}
				else if ( level == 11 )
				{
					...
					LogAdd(LOG_BLACK, "[%s][%s][%d]%d/%d Used box of ElradoraBox1-11 Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
						lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
						serial, szItemName, type, level, Option1, Option2, Option3);
				}
				else if ( level == 12 )
				{
					...
					LogAdd(LOG_BLACK, "[%s][%s][%d]%d/%d Used box of ElradoraBox1-12 Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
						lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
						serial, szItemName, type, level, Option1, Option2, Option3);
				}
				else if ( level == 13 )
				{
					...
					LogAdd(LOG_BLACK, "[%s][%s][%d]%d/%d Used box of DarkLordHeart Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)",
						lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
						serial, szItemName, type, level, Option1, Option2, Option3);
				}
				else if ( level == 0 )
				{
					...
					LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 222)),	// [%s][%s][%d]%d/%d Used Box of Luck Serial:%d (%s:%d/level:%d/skill:%d/op2:%d/op3:%d)
						lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y,
						serial, szItemName, type, level, Option1, Option2, Option3);
				}
				else
				...
				LogAdd(LOG_BLACK, "[%s][%s] Used box of luck (level:%d)",
					...
					case 0:
						{
							...
							LogAdd(LOG_BLACK, "[Hidden TreasureBox Event] [%s][%s] Throw Mysterious Bead : Drop (%d) - serial:%d",
					...
					case 1: case 2: case 3:
						{
							...
							LogAdd(LOG_BLACK, "[Hidden TreasureBox Event] [%s][%s] Throw Crystal (%d) - serial:%d",
								lpObj->AccountID, lpObj->Name, level, serial);
						}
						break;
					case 4:
						LogAdd(LOG_BLACK, "[Hidden TreasureBox Event] [%s][%s] Throw Hidden Treasure Box - serial:%d",
						...
						break;
					case 5:
						LogAdd(LOG_BLACK, "[Hidden TreasureBox Event] [%s][%s] Throw Surprise Treasure Box - serial:%d",
							lpObj->AccountID, lpObj->Name,  serial);
						gObjInventoryDeleteItem(aIndex, lpMsg->Ipos);
						pResult.Result = true;
						...
						else
						{
							...
							LogAdd(LOG_BLACK, "[Hidden TreasureBox Event] [%s][%s] Event ZenDrop : %d : (%d)(%d/%d)",
			...
			else if ( type == ITEMGET(14,28) ) // Lost Map
			{
				...
				{
					LogAdd(LOG_BLACK, "[Kalima] [%s][%s] Success to Make Kalima Gate (Lost Kalima Map Serial:%d)",
			...
			else
			{
				...
				if ( MapC[map].ItemDrop( type, level, dur, lpMsg->px, lpMsg->py, Option1, Option2, Option3,
					NOption, SOption, serial2, OwnerIndex, PetLevel, PetExp, 0) == TRUE )
				{
					gObjInventoryDeleteItem(aIndex, lpMsg->Ipos);
					pResult.Result = true;
					...
					if ( drop_type == FALSE )
					{
						LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 223)), lpObj->AccountID, lpObj->Name, lpObj->MapNumber,
							...
							NewOption[5], NewOption[6], SOption, 0, 0, 0);
					}
					else
					{
						LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 224)), lpObj->AccountID, lpObj->Name, lpObj->MapNumber,
							lpObj->X, lpObj->Y, serial, szItemName, type, level, Option1, Option2,
							Option3, (int)dur, NewOption[0], NewOption[1], NewOption[2], NewOption[3], NewOption[4],
							NewOption[5], NewOption[6], SOption, 0, 0, 0 );
					}
		...
		{
			LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 225)));
			pResult.Result = false;
		}
	}
...
void CGInventoryItemMove(PMSG_INVENTORYITEMMOVE * lpMsg, int aIndex)
{
	...
	{
		LogAdd(LOG_RED, "[%s][%d] error-L3", __FILE__, __LINE__);
		::GCItemMoveResultSend(aIndex, -1, 0, ItemInfo);
		return;
	}
	...
	if ( lpMsg->sFlag == 3 || lpMsg->tFlag == 3 )
	{
		...
		{
			::GCItemMoveResultSend(aIndex, -1, 0, (LPBYTE)&ItemInfo);
			LogAdd(LOG_BLACK, "[%s][%s] error-L1 : used not Chaosbox", gObj[aIndex].AccountID, gObj[aIndex].Name);
		...
		{
			LogAdd(LOG_BLACK, "[%s][%s] error-L3 : CBMixing", gObj[aIndex].AccountID, gObj[aIndex].Name);
			::GCItemMoveResultSend(aIndex, -1, 0, (LPBYTE)&ItemInfo);
	...
	if ( lpMsg->sFlag == 5 || lpMsg->tFlag == 5 )
	{
		...
		{
			::GCItemMoveResultSend(aIndex, -1, 0, (LPBYTE)&ItemInfo);
			LogAdd(LOG_BLACK, "[%s][%s] error-L1 : used not DarkTrainerBox", gObj[aIndex].AccountID, gObj[aIndex].Name);
		...
		if ( lpObj->ChaosLock == TRUE )
		{
			LogAdd(LOG_BLACK, "[%s][%s] error-L3 : DarkTrainerBoxMixing", gObj[aIndex].AccountID, gObj[aIndex].Name);
			::GCItemMoveResultSend(aIndex, -1, 0, (LPBYTE)&ItemInfo);
		 ...
		 lpMsg->tFlag == 6 || lpMsg->tFlag == 7 || lpMsg->tFlag == 8 )
	{
		...
		{
			GCItemMoveResultSend(aIndex, -1, 0, (LPBYTE)&ItemInfo);
			LogAdd(LOG_BLACK, "[%s][%s] error-L1 : used not ChaosBox",
				gObj[aIndex].AccountID, gObj[aIndex].Name);
		...
		if ( lpObj->ChaosLock == TRUE )
		{
			LogAdd(LOG_BLACK, "[%s][%s] error-L3 : JewelOfHarmony Mixing",
				gObj[aIndex].AccountID, gObj[aIndex].Name);
			GCItemMoveResultSend(aIndex, -1, 0, (LPBYTE)&ItemInfo);
	...
	if ( lpMsg->sFlag == 2 || lpMsg->tFlag == 2 )
	{
		...
		{
			::GCItemMoveResultSend(aIndex, -1, 0, (LPBYTE)&ItemInfo);
			LogAdd(LOG_BLACK, "[%s][%s] error-L1 : used not Warehouse", gObj[aIndex].AccountID, gObj[aIndex].Name);
	...
	if ( lpMsg->sFlag == 1 && lpMsg->tFlag == 0 )
	{
		...
		{
			::GCItemMoveResultSend(aIndex, -1, 0, (LPBYTE)&ItemInfo);
			LogAdd(LOG_BLACK, "[%s][%s] error-L1 : used not Trade", gObj[aIndex].AccountID, gObj[aIndex].Name);
		 ...
		 (lpMsg->sFlag == 3 && lpMsg->tFlag == 3) )
	{
		...
		if ( result != 0xFF && lpMsg->sFlag == 2 && lpMsg->tFlag == 0)
		{
			...
			if ( (lpObj->Money - money) > 0 )
			{
				...
				LogAdd(LOG_BLACK, "Pay WareHouse Money(In Inventory) : %d - %d = %d",
			...
			else if ( (lpObj->WarehouseMoney - money) > 0 )
			{
				...
				LogAdd(LOG_BLACK, "Pay WareHouse Money(In WareHouse) : %d - %d = %d",
...
void CGTalkRequestRecv(PMSG_TALKREQUEST * lpMsg, int aIndex)
{
	...
	if ( !gObjIsConnectedGP(aIndex) )
	{
		LogAdd(LOG_BLACK, "[ANTI-HACK][NPC Talk] Couldn't talk.. (%s)(%s)",
			gObj[aIndex].AccountID, gObj[aIndex].Name);
		...
		else
		{
			...
			if ( bNoItem != FALSE )
			{
				...
				if ( pResult.Result != 0xFF )
				{
					...
					LogAdd(LOG_BLACK, "[%s][%s] (%d) Shop buy [%d][%d][%d][%s] LEV:%d, DUR:%d, OP:[%d][%d][%d]",
...
void CGSellRequestRecv(PMSG_SELLREQUEST * lpMsg, int aIndex)
{
	...
	if ( gObj[aIndex].pTransaction == 1 )
	{
		LogAdd(LOG_BLACK, "[%s][%s] CGSellRequestRecv() Failed : Transaction == 1, IF_TYPE : %d",
			gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].m_IfState.type);
	...
	{
		DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 227)), lpMsg->Pos);
		return;
	}
	...
	if ( gObjCheckSerial0ItemList(&lpObj->pInventory[lpMsg->Pos]) != FALSE )
	{
		...
		LogAdd(LOG_BLACK, "[ANTI-HACK][Serial 0 Item] [Sell Item] (%s)(%s) Item(%s) Pos(%d)",
	...
	if ( lpObj->pInventory[lpMsg->Pos].IsItem() == FALSE )
	{
		pResult.Result = false;
		pResult.Money = lpObj->Money;
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 228)), __FILE__, __LINE__, lpObj->Name, lpMsg->Pos);
		LogAdd(LOG_BLACK, "ID:%s CharID:%s ", lpObj->AccountID, lpObj->Name);
		DataSend(aIndex, (LPBYTE)&pResult, pResult.h.size);
	...
	else
	{
		...
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 229)), lpObj->AccountID, lpObj->Name, tShop,
...
void ItemDurRepaire(LPOBJ lpObj, CItem * DurItem, int pos, int RequestPos)
{
	...
	if ( itemtype >= ITEMGET(14,0) || ( itemtype >= ITEMGET(13,0) &&itemtype  < ITEMGET(13,4) ) ||itemtype == ITEMGET(13,10) || (itemtype >= ITEMGET(12,7) &&itemtype <ITEMGET(13,0) ) ||itemtype ==ITEMGET(4,7) ||itemtype ==ITEMGET(4,15) && !NEWWINGS(itemtype))
	{
		pResult.Money = 0;
		DataSend(lpObj->m_Index, (LPBYTE)&pResult, pResult.h.size);
		return;
	...
	else
	{
		...
		DurItem->Convert(DurItem->m_Type, DurItem->m_Option1, DurItem->m_Option2, DurItem->m_Option3,DurItem->m_NewOption, DurItem->m_SetOption, 0, CURRENT_DB_VERSION);
...
void CGTradeRequestSend(PMSG_TRADE_REQUEST * lpMsg, int aIndex)
{
	...
	if ( gObj[aIndex].pTransaction == 1 )
	{
		LogAdd(LOG_BLACK, "[%s][%s] CGTradeRequestSend() Failed : Transaction == 1, IF_TYPE : %d",
			gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].m_IfState.type);
		return;
	}
	...
	LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 230)), gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].Ip_addr,
...
void CGTradeResponseRecv(PMSG_TRADE_RESPONSE * lpMsg, int aIndex)
{
	...
	{
		LogAdd(LOG_BLACK, "error-L1 : %d (A_ID:%s) %s %d", number, gObj[aIndex].AccountID, __FILE__, __LINE__);
		Result = FALSE;
	...
	if ( OBJMAX_RANGE(number) == FALSE )
	{
		...
		LogAdd(LOG_BLACK, "error-L1 : target:%d (A_ID:%s) %s %d", number, gObj[aIndex].AccountID, __FILE__, __LINE__);
		Result = FALSE;
		lpMsg->Response = false;
	}
	...
	{
		LogAdd(LOG_BLACK, "(%s)(%s) Trade Interface State Error : use:%d type:%d", gObj[aIndex].AccountID, gObj[aIndex].Name, 
	...
	{
		LogAdd(LOG_BLACK, "(%s)(%s) Trade Interface State Error : use:%d type:%d", gObj[number].AccountID, gObj[number].Name, 
	...
	if ( gObj[aIndex].pTransaction == 1 )
	{
		LogAdd(LOG_BLACK, "[%s][%s] CGTradeResponseRecv() Failed : Transaction == 1, IF_TYPE : %d",
			gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].m_IfState.type);
	...
	else
	{
		...
		if ( Result != TRUE )
		{
			...
			if ( Result == 2 )
			{
				LogAdd(LOG_BLACK, "[%s][%s] Made Trade Error (in Transaction) [%s][%s]",
					gObj[aIndex].AccountID, gObj[aIndex].Name,
		...
		else
		{
			...
			LogAdd(LOG_BLACK, "[%s][%s] Trade Ready [%s][%s]", 
				gObj[number].AccountID, gObj[number].Name,
				gObj[aIndex].AccountID, gObj[aIndex].Name);
		}
	}
	...
	if ( Result == FALSE )
	{
		if ( gObj[aIndex].m_IfState.use != FALSE && gObj[aIndex].m_IfState.type == 1 )
		{
			gObj[aIndex].m_IfState.use = FALSE;
			gObj[aIndex].TargetNumber = -1;
			LogAdd(LOG_BLACK, "Interface State : %d", gObj[aIndex].m_IfState.use);
		}
		...
		if ( gObj[number].TargetNumber == aIndex )
		{
			if ( gObj[number].m_IfState.use != FALSE && gObj[number].m_IfState.type == 1 )
			{
				...
				LogAdd(LOG_BLACK, "Interface State : %d", gObj[number].m_IfState.use);
			}
		}
	}
}
...
void CGTradeMoneyRecv(PMSG_TRADE_GOLD * lpMsg, int aIndex)
{
	...
	if ( !OBJMAX_RANGE(aIndex) )
	{
		LogAdd(LOG_BLACK, "error : %s %d (%d)", __FILE__, __LINE__, aIndex);
		return;
	}

	if ( gObj[aIndex].CloseType != -1 )
	{
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 231)), gObj[aIndex].AccountID, gObj[aIndex].Name);
		return;
	}
	...
	{
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 232)), gObj[aIndex].AccountID, gObj[aIndex].Name, number);
		return;
	}

	if ( gObj[number].CloseType != -1 )
	{
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 233)), gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[number].AccountID, gObj[number].Name);
		return;
	}
...
void CGTradeOkButtonRecv(PMSG_TRADE_OKBUTTON * lpMsg, int aIndex)
{
	...
	if ( lpMsg->Flag == 1 )
	{
		if ( gObj[aIndex].TradeOk == false )
		{
			...
			LogAdd(LOG_BLACK, "[%s][%s] Trade Accept", gObj[aIndex].AccountID, gObj[aIndex].Name);
...
void CGPShopReqSetItemPrice(PMSG_REQ_PSHOP_SETITEMPRICE * lpMsg, int aIndex)
{
	...
	if ( !gObjIsConnected(aIndex))
	{
		LogAdd(LOG_BLACK, "[PShop] ERROR : Index is not CONNECTED : %d", aIndex);
		return;
	}

	if ( gObj[aIndex].Type != OBJ_USER )
	{
		LogAdd(LOG_BLACK, "[PShop] ERROR : Index is not CHARACTER : %d", aIndex);
		return;
	}

	if ( gObj[aIndex].Level <= 5 )
	{
		LogAdd(LOG_BLACK, "[PShop] [%s][%s] ERROR : Level is Under 6 : %d",
	...
	{
		LogAdd(LOG_BLACK, "[PShop] [%s][%s] ERROR : Item Position Out of Bound : %d",
	...
	{
		LogAdd(LOG_BLACK, "[PShop] [%s][%s] ERROR : Item Does Not Exist : %d",
			gObj[aIndex].AccountID, gObj[aIndex].Name, lpMsg->btItemPos);
	...
	{
		MsgOutput(aIndex, lMsg.Get(MSGGET(13, 26)));
		GCReFillSend(aIndex, gObj[aIndex].Life, 0xFD, 1);

		LogAdd(LOG_BLACK, "[ANTI-HACK][Serial 0 Item] [PShop] (%s)(%s) Item(%s) Pos(%d)",
	...
	{
		GCReFillSend(aIndex, gObj[aIndex].Life, 0xFD, 1);
		LogAdd(LOG_BLACK, "error-L2 : CopyItem [%s][%s] return %s %d", gObj[aIndex].AccountID, gObj[aIndex].Name, __FILE__, __LINE__);
	...
	if ( iItemPrice <= 0 )
	{
		LogAdd(LOG_BLACK, "[PShop] [%s][%s] ERROR : Personal Shop Item Price <= 0 : %d",
	...
	{
		LogAdd(LOG_BLACK, "[PShop] [%s][%s] ERROR : Item Price Change - Item Block",gObj[aIndex].AccountID, gObj[aIndex].Name,sitem->m_iPShopValue);	// Deathway fix Here - Last parameter can cause crash
	...
	LogAdd(LOG_BLACK, "[PShop] [%s][%s] Changing Item Price : %d (Type:%d (%s), Lev:%d, Serial:%d, Dur:%d",
...
void CGPShopReqOpen(PMSG_REQ_PSHOP_OPEN * lpMsg, int aIndex)
{
	...
	if ( (gObj[aIndex].Penalty&4) == 4 || (gObj[aIndex].Penalty&8) == 8 )
	{
		LogAdd(LOG_BLACK, "[PShop] [%s][%s] ERROR : Cant't Open Shop - Item Block" ,gObj[aIndex].AccountID, gObj[aIndex].Name);
	...
	else
	{
		if ( gObj[aIndex].m_IfState.use == TRUE )
		{
			...
			if ( !bEnablePShopOpen  )
			{
				LogAdd(LOG_BLACK, "[PShop] [%s][%s] ERROR : m_IfState.type is Using : %d", gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].m_IfState.type);
			}
		}
		else
		{
			LogAdd(LOG_BLACK, "[PShop] [%s][%s] ERROR : Unknown m_IfState.type : %d", gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].m_IfState.type);
			return;
		}
	}
	...
	if ( gObj[aIndex].pTransaction == 1 )
	{
		LogAdd(LOG_BLACK, "[PShop] [%s][%s] ERROR : Transaction == 1, IF_TYPE : %d", gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].m_IfState.type);
	...
	if ( bEnablePShopOpen )
	{
		if ( gObj[aIndex].m_bPShopOpen == false )
		{
			...
			LogAdd(LOG_BLACK, "[PShop] [%s][%s] Personal Shop Opened", gObj[aIndex].AccountID, gObj[aIndex].Name);
		}
		else
		{
			memcpy(gObj[aIndex].m_szPShopText, lpMsg->szPShopText, sizeof(lpMsg->szPShopText));
			...
			LogAdd(LOG_BLACK, "[PShop] [%s][%s] Personal Shop Already Opened - Changing PShop Name",
				gObj[aIndex].AccountID, gObj[aIndex].Name);
		}
	}
	else
...
void CGPShopReqClose(int aIndex)
{
	...
	LogAdd(LOG_BLACK, "[PShop] [%s][%s] Requested to Close PShop", gObj[aIndex].AccountID, gObj[aIndex].Name);
...
{
	LogAdd(LOG_BLACK, "[PShop] [%s][%s] Close PShop", gObj[aIndex].AccountID, gObj[aIndex].Name);
...
void CGPShopReqBuyList(PMSG_REQ_BUYLIST_FROM_PSHOP * lpMsg, int aSourceIndex)
{
	...
	{
		LogAdd(LOG_BLACK, "[PShop] [%s][%s] PShop List Request Failed : Seller did Not CONNECTED (%d)",
	...
	{
		LogAdd(LOG_BLACK, "[PShop] [%s][%s] PShop List Request Failed : Seller is Not CHARACTER (%d)"
			,gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,  MAKE_NUMBERW(lpMsg->NumberH, lpMsg->NumberL));
		::CGPShopAnsBuyList(aSourceIndex, -1, 2, 0);
		return;
	}
	...
	{
		LogAdd(LOG_BLACK, "[PShop] [%s][%s] PShop Item Buy Request Failed : Seller is Closing (%d)"
			,gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,  MAKE_NUMBERW(lpMsg->NumberH, lpMsg->NumberL) );
	...
	{
		LogAdd(LOG_BLACK, "[PShop] [%s][%s] PShop Item Buy Request Failed : Buyer is Closing (%d)",
			gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,  MAKE_NUMBERW(lpMsg->NumberH, lpMsg->NumberL));
		CGPShopAnsBuyItem(aSourceIndex, -1, 0, 2);
		return;
	}
	...
	{
		LogAdd(LOG_BLACK, "[PShop] [%s][%s] PShop List Requested to Him(/Her)Self",
	...
	{
		LogAdd(LOG_BLACK, "[PShop] [%s][%s] PShop List Request Failed : [%s][%s] Did not Open PShop",
			gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
			lpObj->AccountID, lpObj->Name);
	...
	{
		LogAdd(LOG_BLACK, "[PShop] [%s][%s] PShop List Request Failed : [%s][%s] is in Item Block",
			gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
			lpObj->AccountID, lpObj->Name);
	...
	{
		LogAdd(LOG_BLACK, "[PShop] [%s][%s] PShop List Request Failed : Requester Transaction == 1, IF_TYPE : %d",
			gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name, gObj[aSourceIndex].m_IfState.type);
		return;
	}

	LogAdd(LOG_BLACK, "[PShop] [%s][%s] is Receiving PShop List From [%s][%s]",
			gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
			lpObj->AccountID, lpObj->Name);
	...
	else
	{
		...
		for ( int n=MAIN_INVENTORY_SIZE;n<INVENTORY_SIZE;n++)
		{
			if ( lpObjTarget->Inventory1[n].IsItem() == TRUE )
			{
				...
				LogAdd(LOG_BLACK, "ITEM TYPE:%d", lpObjTarget->Inventory1[n].m_Type);
			}
		}
...
void CGPShopReqBuyItem(PMSG_REQ_BUYITEM_FROM_PSHOP * lpMsg, int aSourceIndex)
{
	...
	if ( gObjIsConnected( MAKE_NUMBERW(lpMsg->NumberH, lpMsg->NumberL)) == FALSE )
	{
		LogAdd(LOG_BLACK, "[PShop] [%s][%s] PShop Item Buy Request Failed : Seller did Not CONNECTED (%d)",
			gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name, MAKE_NUMBERW(lpMsg->NumberH, lpMsg->NumberL));
		::CGPShopAnsBuyItem(aSourceIndex, -1, 0, 2);
		return;
	}
	...
	if ( lpObj->Type != OBJ_USER )
	{
		LogAdd(LOG_BLACK, "[PShop] [%s][%s] PShop Item Buy Request Failed : Seller is Not CHARACTER (%d)",
			gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,  MAKE_NUMBERW(lpMsg->NumberH, lpMsg->NumberL));
		::CGPShopAnsBuyItem(aSourceIndex, -1, 0, 2);
		return;
	}
	...
	if ( lpObj->m_bPShopOpen == false )
	{
		LogAdd(LOG_BLACK, "[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Did not Open PShop",
			gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
			lpObj->AccountID, lpObj->Name);
	...
	{
		LogAdd(LOG_BLACK, "[PShop] [%s][%s] PShop Item Buy Request Failed : Name Mismatch [%s] - [%s]",
			gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
			szName, lpObj->Name);
	...
	{
		LogAdd(LOG_BLACK, "[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] is Closing Connection",
			gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
			lpObj->AccountID, lpObj->Name);
	...
	if ( (lpObj->Penalty &4) == 4 || (lpObj->Penalty &8) == 8 )
	{
		LogAdd(LOG_BLACK, "[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] is in Item Block",
			gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
			lpObj->AccountID, lpObj->Name);
	...
	{
		LogAdd(LOG_BLACK, "[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Already Trade With Other",
			gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
			lpObj->AccountID, lpObj->Name);
	...
	else
	{
		...
		{
			MsgOutput(lpObj->m_Index, lMsg.Get(MSGGET(13, 26)));
			GCReFillSend(aSourceIndex, gObj[aSourceIndex].Life, 0xFD, 1);

			LogAdd(LOG_BLACK, "[ANTI-HACK][Serial 0 Item] [PShop Buy] (%s)(%s) Item(%s) Pos(%d)",
		...
		{
			::GCReFillSend(aSourceIndex, gObj[aSourceIndex].Life, -3, 1);
			LogAdd(LOG_BLACK, "error-L2 : CopyItem [%s][%s] return %s %d", gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
				__FILE__, __LINE__);
		...
		{
			GCReFillSend(lpObj->m_Index, lpObj->Life, -3, 1);
			LogAdd(LOG_BLACK, "error-L2 : CopyItem [%s][%s] return %s %d",
				lpObj->AccountID, lpObj->Name,
				__FILE__, __LINE__);
		...
		{
			LogAdd(LOG_RED, "[Fix Inv.Ptr] False Location - %s, %d", 
				__FILE__, __LINE__);
		}

		if ( gObj[aSourceIndex].pTransaction == 1 )
		{
			LogAdd(LOG_BLACK, "[PShop] [%s][%s] PShop Item Buy Request Failed : Requester Transaction == 1, IF_TYPE : %d",
				gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name, gObj[aSourceIndex].m_IfState.type);
			return;
		}
		...
		__try
		{
			...
			if ( lpObj->Inventory1[lpMsg->btItemPos].IsItem() == TRUE )
			{
				...
				{
					LogAdd(LOG_BLACK, "[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Did Not Choose Item Price",
						gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
						lpObj->AccountID, lpObj->Name);
					::CGPShopAnsBuyItem(aSourceIndex, lpObj->m_Index, 0, 6);
				...
				{
					LogAdd(LOG_BLACK, "[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Lack of Zen",
						gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
						lpObj->AccountID, lpObj->Name);
				...
				{
					LogAdd(LOG_BLACK, "[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] Exceeding Zen of the Host",
						gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
						lpObj->AccountID, lpObj->Name);
				...
				{
					LogAdd(LOG_BLACK, "[PShop] [%s][%s] PShop Item Buy Request Failed : [%s][%s] No Room to Buy Item",
						gObj[aSourceIndex].AccountID, gObj[aSourceIndex].Name,
						lpObj->AccountID, lpObj->Name);
					::CGPShopAnsBuyItem(aSourceIndex, lpObj->m_Index, 0, 8);
				...
				LogAdd(LOG_BLACK, "[PShop] [%s][%s][%s] PShop Item Buy Request Succeed : [%s][%s][%s] (Price=%d, ItemType:%d (%s), ItemLevel:%d, ItemDur:%d, Serial:%d",
				...
				{
					LogAdd(LOG_BLACK, "[PShop] [%s][%s] Sold All Items - Auto Closing PShop",
...
void CGPartyRequestRecv(PMSG_PARTYREQUEST * lpMsg, int aIndex)
{
	...
	LogAdd(LOG_BLACK, "Party result : %d %d %d %d", gObj[aIndex].CloseCount, gObj[number].CloseCount, gObj[aIndex].Connected, gObj[number].Connected);
	LogAdd(LOG_BLACK, "%s %s", gObj[aIndex].Name, gObj[number].Name);
...
void CGPartyDelUser(PMSG_PARTYDELUSER * lpMsg, int aIndex)
{
	...
	if ( gObj[aIndex].PartyNumber < 0 )
	{
		LogAdd(LOG_BLACK, "No Party %s %s", gObj[aIndex].AccountID, gObj[aIndex].Name);
		return;
	}

	if ( !gParty.GetIndexUser(gObj[aIndex].PartyNumber, lpMsg->Number, usernumber, dbnumber))
	{
		LogAdd(LOG_BLACK, "User Not found");
		return;
	}
	...
	if ( pnumber < 0 )
	{
		LogAdd(LOG_BLACK, "%s %d", __FILE__, __LINE__);
		return;
	}
...
void CGGuildRequestRecv(PMSG_GUILDJOINQ * lpMsg, int aIndex)
{
	...
	LogAdd(LOG_BLACK, "[%s][%s] Guild Request - Join : %s",
		gObj[aIndex].AccountID, gObj[aIndex].Name,
...
void CGGuildDelUser(PMSG_GUILDDELUSER * lpMsg, int aIndex)
{
	...
	if ( !strcmp(gObj[aIndex].lpGuild->Names[0], gObj[aIndex].Name ))
	{
		...
		if ( gObjJoominCheck(aIndex, joomin) == TRUE )
		{
			...
			LogAdd(LOG_BLACK, "[%s][%s] Guild Request - Dismiss All : %s",
	...
	else
	{
		...
		LogAdd(LOG_BLACK, "[%s][%s] Guild Request - Leave : %s",
			gObj[aIndex].AccountID, gObj[aIndex].Name, guildname);
	}
}
...
void GCGuildWarRequestResult(LPSTR GuildName, int aIndex, int type)
{
	...
	if ( lpMyGuild->WarState == 1 || lpMyGuild->WarDeclareState == 1 )
	{
		...
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 234)), lpMyGuild->WarState,
	...
	if ( gPkLimitFree == FALSE && gObj[aIndex].m_PK_Level >= 6)
	{
		...
		LogAdd(LOG_BLACK, "Error on declaring war : GuildMaster Is Murderer 0x04 %s %s",
	...
	if ( strcmp(lpMyGuild->Names[0], gObj[aIndex].Name ) )
	{
		...
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 235)),
			lpMyGuild->Names[0], gObj[aIndex].Name);
		DataSend(aIndex, (LPBYTE)&pMsg, pMsg.h.size);
	...
	if ( lpNode )
	{
		if ( lpNode->WarState == 1 || lpNode->WarDeclareState == 1 )
		{
			...
			//Error on declaring war : %d %d %s
			LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 234)), lpNode->WarState,
		...
		{
			LogAdd(LOG_BLACK, "[U.System][Guildwar] Can't declare guildwar between Same Union (%s / %d) <-> (%s / %d)",
					...
					if ( gObj[n].Name[0] == lpNode->Names[0][0] )
					{
						if ( !strcmp(gObj[n].Name, lpNode->Names[0]))
						{
							if ( gPkLimitFree == FALSE && gObj[n].m_PK_Level >= 6)
							{
								...
								LogAdd(LOG_BLACK, "Error on declaring war : Target GuildMaster Is Murderer 0x04 %s %s",
									lpMyGuild->Names[0], gObj[aIndex].Name);
		...
		if ( warmaster >= 1 )
		{
			...
			LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 236)), gObj[aIndex].AccountID, gObj[aIndex].Name,
...
void GCGuildWarRequestSend(LPSTR GuildName, int aIndex, int type)
{
	...
	LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 237)), GuildName);
#endif
}
#if GS_CASTLE == 1
						...
						if ( lpMyNode->Use[n] )
						{
							if ( lpMyNode->Index[n] >= 0 )
							{
								...
								if ( g_call )
								{
									...
									LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 238)), lpMyNode->Name);
						...
						if ( lpMyNode->lpTargetGuildNode->Use[n] )
						{
							if ( lpMyNode->lpTargetGuildNode->Index[n] >= 0 )
							{
								...
								if ( g_call )
								{
									...
									// Declared guild war against (%s)
									LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 238)), lpMyNode->lpTargetGuildNode->Name);
...
void GCGuildWarScore(int aIndex)
{
	...
	if ( gObj[aIndex].lpGuild != NULL )
	{
		...
		LogAdd(LOG_BLACK, "Score %s %d", gObj[aIndex].Name, gObj[aIndex].lpGuild->PlayScore);
	}

	if ( gObj[aIndex].lpGuild->lpTargetGuildNode != NULL )
	{
		pMsg.Score2 = gObj[aIndex].lpGuild->lpTargetGuildNode->PlayScore;
		LogAdd(LOG_BLACK, "Target Score %s %d", gObj[aIndex].Name, gObj[aIndex].lpGuild->lpTargetGuildNode->PlayScore);
	}
...
void CGWarehouseMoneyInOut(int aIndex, PMSG_WAREHOUSEMONEYINOUT* lpMsg)
{
	...
	{
		GCWarehouseInventoryMoneySend(aIndex, 0, 0, 0);
		LogAdd(LOG_BLACK, "[%s][%s] error-L1 : used not Warehouse",
			gObj[aIndex].AccountID, gObj[aIndex].Name);
		...
		case 0x01:
			{
				...
				if ( (lpObj->Money-rZen) > 0 )
				{
					...
					LogAdd(LOG_BLACK, "Get WareHouse Money(In Inventory) : %d - %d = %d",
				...
				else if ( (lpObj->WarehouseMoney-rZen) > 0 )
				{
					...
					LogAdd(LOG_BLACK, "Get WareHouse Money(In WareHouse) : %d - %d = %d",
...
void CGChaosBoxItemMixButtonClick(PMSG_CHAOSMIX* aRecv, int aIndex)
{
	...
	if ( lpObj->ChaosLock == TRUE )
	{
		LogAdd(LOG_BLACK, "[%s][%s] Already Used Chaos",
			lpObj->AccountID, lpObj->Name);
	...
	switch ( iMixType )	// TEST - Good
	{
		...
		case CHAOS_TYPE_HT_BOX:
			...
			break;
		default:
			LogAdd(LOG_BLACK, "[%s][%s] Undefine chaosmix type detect %d",
...
void PMoveProc(PMSG_MOVE* lpMove, int aIndex)
{
	if ( !OBJMAX_RANGE(aIndex))
	{
		LogAdd(LOG_BLACK, "error-L3 : move protocol index error %s %d", __FILE__, __LINE__);
		return;
	}
	...
	{
		LogAdd(LOG_BLACK, "error-L3 : Path Count error %d id:%s %s %d",
...
void RecvPositionSetProc(PMSG_POSISTION_SET * lpMove, int aIndex)
{
	...
	if (  OBJMAX_RANGE(aIndex) == FALSE)
	{
		LogAdd(LOG_BLACK, "error : move protocol index error %s %d", __FILE__, __LINE__);
		return;
	}
	...
	PHeadSetB((LPBYTE)&pMove, PROTOCOL_POSITION, sizeof(pMove)); //original PROTOCOL_MOVE
	pMove.NumberH = SET_NUMBERH(aIndex);
	pMove.NumberL = SET_NUMBERL(aIndex);
	pMove.X = lpMove->X;
	pMove.Y = lpMove->Y;
...
void CGAttack(PMSG_ATTACK* lpMsg, int aIndex)
{
	...
	{
		LogAdd(LOG_BLACK, "[CGAttack] [UserIndex Error] :%s %d %d", __FILE__, __LINE__, usernumber);
		return;
	}
	...
	if ( iTimeCalc < lpObj->m_DetectSpeedHackTime && iTimeCalc && lpObj->HaveWeaponInHand )
	{
		...
		if ( lpObj->m_DetectCount > gHackCheckCount )
		{
			...
			if ( gIsKickDetecHackCountLimit )
			{
				...
				{
					LogAdd(LOG_BLACK, "[%s][%s] %s Kick DetecHackCountLimit Over User (%d)",
			...
			LogAdd(LOG_BLACK, "[%s][%s] %s Attack Speed Is Wrong Normal (%d)(%d) Penalty %d",
				lpObj->AccountID, lpObj->Name, lMsg.Get(MSGGET(7, 108)+lpObj->Class),
	...
	if ( bIsIgnorePacketSpeedHackDetect )
	{
		if ( lpObj->m_SpeedHackPenalty > 0 )
		{
			...
			LogAdd(LOG_BLACK, "[%s][%s] %s Apply Attack Speed Penalty (%d left)",
				lpObj->AccountID, lpObj->Name, lMsg.Get(MSGGET(7, 108)+lpObj->Class),
				lpObj->m_SpeedHackPenalty);
...
void CGMagicAttack(PMSG_MAGICATTACK* lpMsg, int aIndex)
{
	...
	if ( usernumber < 0 || usernumber > OBJMAX-1 )
	{
		LogAdd(LOG_BLACK, "error :%s %d %d", __FILE__, __LINE__, usernumber);
		return;
	}
	...
	if ( attackret )
	{
		LogAdd(LOG_BLACK, "[%s][%s] Try Attack In Not Attack Area [Protocol] (%s:%d,%d) errortype = %d",
	...
	if ( lpObj->Type == OBJ_USER )
	{
		...
		{
			LogAdd(LOG_RED,  "[0x19] CGMagicAttack() - Invalid Status");
			return;
		}

		if ( MagicDamageC.CheckKillCount(lpMagic->m_Skill, lpObj->m_btKillCount) < 0 )
		{
			LogAdd(LOG_RED,  "[0x19] CGMagicAttack() - Invalid KillCount");
			return;
		}
	}
	else
	...
	if ( !gCheckSkillDistance(aIndex, usernumber, lpMagic->m_Skill))
	{
		...
		if ( lpObj->m_iSkillDistanceErrorCount > g_iSkillDistanceKickCount )
		{
			...
			{
				LogAdd(LOG_BLACK, "[SKILL DISTANCE CHECK] [%s][%s] Kick Invalid Skill Area User. count(%d)",
...
void CGTeleportRecv(PMSG_TELEPORT* lpMsg, int aIndex)
{
	...
	if ( lpMsg->MoveNumber == 0 )
	{
		...
		{
			LogAdd(LOG_RED,  "[%s][%s] Try Teleport Not Move Area [%d,%d]",
				gObj[aIndex].AccountID, gObj[aIndex].Name,
				x, y);
...
void CGTargetTeleportRecv(PMSG_TARGET_TELEPORT * lpMsg, int aIndex)
{
	...
	{
		LogAdd(LOG_RED,  "[%s][%s] Try Target Teleport Not Move Area [%d,%d]",
...
void CGBeattackRecv(unsigned char* lpRecv, int aIndex, int magic_send)
{
	...
	// ---
	if(lpMagic->m_Skill == 42 || lpMagic->m_Skill == 43 || lpMagic->m_Skill == 41)	
	{
		return;
	}
	// ---
	if(gObj[aIndex].Type == OBJ_USER)
	{
		...
		{
			LogAdd(LOG_RED,  lMsg.Get(MSGGET(1, 239)), gObj[aIndex].AccountID, gObj[aIndex].Name, lpMagic->m_Skill);
			// ---
			gObjUserKill(aIndex);
			return;
		}
		...
		{
			LogAdd(LOG_RED,  "Too long time passed after casting magic. [%s][%s] (%d)(%d)", gObj[aIndex].AccountID, gObj[aIndex].Name, lpMagic->m_Skill, GetTickCount() - gObj[aIndex].UseMagicTime);
			return;
		}
		// ---
	...
	if(lpMagic->m_Skill == 24 || lpMagic->m_Skill == 78)
	{
		...
		if(NSAttackSerial > gObj[aIndex].OSAttackSerial)
		{
			...
			// ---
			int iTimeCalc = GetTickCount() - gObj[aIndex].m_LastAttackTime;
			// ---
			if(iTimeCalc < gObj[aIndex].m_DetectSpeedHackTime && iTimeCalc)
			{
				gObj[aIndex].m_DetectCount++;
				gObj[aIndex].m_SumLastAttackTime += iTimeCalc;
				// ---
				if(gObj[aIndex].m_DetectCount > gHackCheckCount)
				{
					gObj[aIndex].m_DetectedHackKickCount++;
					// ---
					if(gIsKickDetecHackCountLimit)
					{
						if(gObj[aIndex].m_DetectedHackKickCount > gDetectedHackKickCount)
						{
							LogAdd(LOG_BLACK, "[%s][%s] %s Kick DetecHackCountLimit Over User (%d)", gObj[aIndex].AccountID, gObj[aIndex].Name, lMsg.Get(MSGGET(7, 108)+gObj[aIndex].Class), gObj[aIndex].m_DetectedHackKickCount);
							// ---
							CloseClient(aIndex);
							return;
						}
					...
					// ---
					LogAdd(LOG_BLACK, "[%s][%s] %s Attack Speed Is Wrong Magic3 (%d)(%d) Penalty %d",
						gObj[aIndex].AccountID, gObj[aIndex].Name, lMsg.Get(MSGGET(7, 108)+gObj[aIndex].Class),
						gObj[aIndex].m_DetectSpeedHackTime, gObj[aIndex].m_SumLastAttackTime/gObj[aIndex].m_DetectCount,
						gObj[aIndex].m_SpeedHackPenalty);
				}
				// ---
				gObj[aIndex].m_LastAttackTime = GetTickCount();
			}
			else
			{
				gObj[aIndex].m_SumLastAttackTime	= 0;
				gObj[aIndex].m_DetectCount			= 0;
			}
			// ---
			gObj[aIndex].m_LastAttackTime = GetTickCount();
		}
	...
	for (int n=0;n<lpCount->Count;n++)
	{
		...
		if ( lpMsg->MagicKey  && lpMagic->m_Skill == 9 )	// "Evil Spirit"
		{
			...
			{
				LogAdd(LOG_BLACK, "★★★★ InValid DurationTime Key = %d ( Time : %d) [%d][%d]",	// #error BIG_ERROR Deathway Change tp %s %s
			...
			{
				LogAdd(LOG_BLACK, "★★★★ InValid VailidCount = %d ( Count : %d) [%d][%d]",	// #error BIG_ERROR Deathway Change tp %s %s
		...
		if ( gWriteSkillLog )
		{
			LogAdd(LOG_BLACK, "Magic Attack3 : %d, serial = %d, Tgt =  %d, cnt = %d",
		...
		for (int i=0;i<lpCount->Count;i++)
		{
			if ( n != i)
			{
				...
				{
					LogAdd(LOG_BLACK, "[%s][%s] %s Detect Hack : Multi Attack",
						gObj[aIndex].AccountID, gObj[aIndex].Name,
...
void CGDurationMagicRecv(PMSG_DURATION_MAGIC_RECV* lpMsg, int aIndex)
{
	...
	if(gObj[aIndex].Type == OBJ_USER)
	{
		lpMagic = gObjGetMagicSearch(&gObj[aIndex], lpMsg->MagicNumber);
	}
	else
	{
		lpMagic = gObjGetMagic(&gObj[aIndex], lpMsg->MagicNumber);
	}
	// ---
	if(lpMagic == NULL)
	{
	...
	if(gObj[aIndex].Type == OBJ_USER)
	{
		if(MagicDamageC.CheckStatus(lpMagic->m_Skill, gObj[aIndex].GuildStatus) == 0)
		{
			LogAdd(LOG_RED, "[0x1E] CGDurationMagicRecv() - Invalid Status");
			return;
		}
		// ---
		if(MagicDamageC.CheckKillCount(lpMagic->m_Skill, gObj[aIndex].m_btKillCount) < 0)
		{
			LogAdd(LOG_RED,  "[0x1E] CGDurationMagicRecv() - Invalid KillCount");
			return;
		}
	}
	// ---
	gObj[aIndex].UseMagicNumber = 0;
	// ---
	...
	if(lpMsg->MagicKey)
	{
		gObj[aIndex].DurMagicKeyChecker.SetDurationTime(lpMsg->MagicKey, GetTickCount());
	}
}
...
void CGUseItemRecv(PMSG_USEITEM* lpMsg, int aIndex)
{
	...
	if(gObj[aIndex].m_IfState.use && gObj[aIndex].m_IfState.type != 3)
	{
		GCReFillSend(aIndex, gObj[aIndex].Life, 0xFD, 1);
		// ---
		LogAdd(LOG_BLACK, "[%s][%s] CGUseItemRecv()_If return %s %d", gObj[aIndex].AccountID, gObj[aIndex].Name, __FILE__, __LINE__);
		return;
	}
	// ---
	if(gObj[aIndex].CloseType != -1)
	{
		GCReFillSend(aIndex, gObj[aIndex].Life, 0xFD, 1);
		// ---
		LogAdd(LOG_BLACK, "[%s][%s] CGUseItemRecv()_CloseType return %s %d", gObj[aIndex].AccountID, gObj[aIndex].Name, __FILE__, __LINE__);
		return;
	}
	// ---
	if(gObjFixInventoryPointer(aIndex) == false)
	...
	if(gObj[aIndex].pTransaction == 1)
	{
		LogAdd(LOG_BLACK, "[%s][%s] CGUseItemRecv() Failed : Transaction == 1, IF_TYPE : %d", gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].m_IfState.type);
		return;
	}
	// ---
	int iPosition = lpMsg->inventoryPos;
	// ---
	if(iPosition > MAIN_INVENTORY_SIZE-1)
	{
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 241)), __FILE__, __LINE__);
		// ---
		GCReFillSend(aIndex, gObj[aIndex].Life, 0xFD, 1);
		// ---
		LogAdd(LOG_BLACK, "[%s][%s] CGUseItemRecv()_Inventory return %s %d", gObj[aIndex].AccountID, gObj[aIndex].Name, __FILE__, __LINE__);
		return;
	}
	// ---
	if(lpMsg->inventoryPos == lpMsg->invenrotyTarget)
	{
		GCReFillSend(aIndex, gObj[aIndex].Life, 0xFD, 1);
		// ---
		LogAdd(LOG_BLACK, "error-L1 : [%s][%s] CGUseItemRecv()_Pos return %s %d", gObj[aIndex].AccountID, gObj[aIndex].Name, __FILE__, __LINE__);
		return;
	}
	// ---
	citem = &gObj[aIndex].pInventory[iPosition];
	// ---
	if(citem->IsItem())
	{
		...
		// ---
		if(citem->m_Type == ITEMGET(14,0) || citem->m_Type == ITEMGET(14,1) || citem->m_Type == ITEMGET(14,2) || citem->m_Type == ITEMGET(14,3))
		{
			...
			switch(citem->m_Type)
			{
			case ITEMGET(14,0): { nAddRate = gServerInfo.m_Data.iPotionHP[0]; }	break;
			case ITEMGET(14,1): { nAddRate = gServerInfo.m_Data.iPotionHP[1]; }	break;
			case ITEMGET(14,2): { nAddRate = gServerInfo.m_Data.iPotionHP[2]; }	break;
			case ITEMGET(14,3): { nAddRate = gServerInfo.m_Data.iPotionHP[3]; }	break;
			}
			// ---
			if(citem->m_Level == 1)
			{
			...
			// ---
			tLife += ((int)gObj[aIndex].MaxLife * nAddRate) / gServerInfo.m_Data.iPotionHP[4];
			// ---
			...
			if(gObj[aIndex].FillLife > 0.0f)
			{
				...
				// ---
				GCReFillSend(gObj[aIndex].m_Index, gObj[aIndex].Life, 0xFF, FALSE);
			}
			// ---
			...
			// ---
			if(!gObjSearchItemMinus(&gObj[aIndex], iPosition, 1))
			{
				gObjInventoryItemSet(aIndex, iPosition, -1);
				// ---
				gObj[aIndex].pInventory[iPosition].Clear();
				// ---
				GCInventoryItemDeleteSend(aIndex, iPosition, 1);
			}
		}
		else if(citem->m_Type == ITEMGET(14,4) || citem->m_Type == ITEMGET(14,5) || citem->m_Type == ITEMGET(14,6))
		{
			...
			switch ( citem->m_Type )
			{
			case ITEMGET(14,4): { tMana += ((int)(gObj[aIndex].MaxMana + gObj[aIndex].AddMana)) * gServerInfo.m_Data.iPotionMP[0] / gServerInfo.m_Data.iPotionMP[1]; } break;
			case ITEMGET(14,5):	{ tMana += ((int)(gObj[aIndex].MaxMana + gObj[aIndex].AddMana)) * gServerInfo.m_Data.iPotionMP[2] / gServerInfo.m_Data.iPotionMP[3]; } break;
			case ITEMGET(14,6):	{ tMana += ((int)(gObj[aIndex].MaxMana + gObj[aIndex].AddMana)) * gServerInfo.m_Data.iPotionMP[4] / gServerInfo.m_Data.iPotionMP[5]; } break;
			}
			// ---
		...
		else if(citem->m_Type >= ITEMGET(15,0) || citem->m_Type == ITEMGET(12,7) || (citem->m_Type >= ITEMGET(12,8) && citem->m_Type <= ITEMGET(12,24)))
		{
		...
		{
			LogAdd(LOG_BLACK, "[Using Item] [Return Scroll] [%s][%s] - Current Map:[%d]", gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].MapNumber);
			// ---
		...
		else if(citem->m_Type == ITEMGET(13,15))
		{
			if(iItemUseType == 0)
			{
				gObjUsePlusStatFruit(aIndex, iPosition);
			}
			else if(iItemUseType == 1)
			{
				gObjUseMinusStatFruit(aIndex, iPosition);
			}
		}
		else if(citem->m_Type == ITEMGET(14,7))
		{
			...
			{
				LogAdd(LOG_BLACK, "[CastleSiege] Using CastleSiege Potion Lv[%d]/Si[%d]/Dur[%d] - [%s][%s] - (Guild : %s)",
					iLevel, iSerial, iDur, gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].lpGuild->Name);
			}
			else
			{
				LogAdd(LOG_BLACK, "[CastleSiege] Using CastleSiege Potion Lv[%d]/Si[%d]/Dur[%d] - [%s][%s]",
		...
		{
			LogAdd(LOG_BLACK, "error-L3 : %s return %s %d %d %s", gObj[aIndex].Name, __FILE__,__LINE__, iPosition, &CItem::GetName);
			// ---
			GCReFillSend(aIndex, gObj[aIndex].Life, 0xFD, 1);
		}
	}
	else
	{
		GCReFillSend(aIndex, gObj[aIndex].Life, 0xFD, 1);
		// ---
		LogAdd(LOG_BLACK, "error-L3 : %s return %s %d %d", gObj[aIndex].Name, __FILE__,__LINE__, iPosition);
	}
}
...
void GCManaSend(int aIndex, int Mana, BYTE Ipos, BYTE flag, int BP)
{
	...
	if ( aIndex < 0 || aIndex > OBJMAX-1)
	{
		LogAdd(LOG_BLACK, "GCManaSend() return %s %d", __FILE__, __LINE__);
		return;
	}
	...
	if (Ipos == 0xFE)
	{
		pMsg.iBP = gObj[aIndex].MaxBP;
	} 
	else 
	{
		pMsg.iBP = gObj[aIndex].BP;
	}
...
void GCReqmoveDevilSquare(PMSG_REQ_MOVEDEVILSQUARE* lpMsg, int aIndex)
{
	if ( aIndex < 0 || aIndex > OBJMAX-1 )
	{
		LogAdd(LOG_BLACK, "return %s %d", __FILE__, __LINE__);
		return;
	}
	...
	LogAdd(LOG_BLACK, "[DevilSquare] [%s][%s] Request Move DevilSquare [%d][%d][%d]",
	...
	if ( MAIN_INVENTORY_RANGE(cInvitationItemPos) == FALSE )
	{
		...
		LogAdd(LOG_BLACK, "[DevilSquare] [%s][%s] Move DevilSquare Fail [%d][%d][%d]",
			lpObj->AccountID, lpObj->Name, lpObj->Class, lpObj->Level, cSquareNumber+1);
	...
	if ( ItemLevel != 0 )
	{
		if ( ItemLevel != 10 )
		{
			if ( ItemLevel != (cSquareNumber+1) )
			{
				...
				LogAdd(LOG_BLACK, "[DevilSquare] [%s][%s] Move DevilSquare Invitation LevelError [%d][%d][%d][%d]",
	...
	{
		LogAdd(LOG_BLACK, "[DevilSquare] [%s][%s] GetUserLevelToEnter() failed",
			lpObj->AccountID, lpObj->Name);
	...
	if ( bEnterCheck == 1 )
	{
		...
		LogAdd(LOG_BLACK, "[DevilSquare] [%s][%s] Move DevilSquare Fail [%d][%d][%d][%d]",
	...
	LogAdd(LOG_BLACK, "[DevilSquare] [%s][%s] Move DevilSquare success [%d][%d] RemoveItem[%s][%d][%d][%d]",
	...
	switch ( lpMsg->hEventType )
	{
		case 1:
			if ( gObj[aIndex].m_PK_Level >= 4 )
			{
				...
				LogAdd(LOG_BLACK, "[PK User][DevilSquare] [%s][%s] Move Fail [PK Level:%d]",
		...
		case 2:
			if ( gObj[aIndex].m_PK_Level >= 4 )
			{
				...
				LogAdd(LOG_BLACK, "[PK User][BloodCastle] [%s][%s] Move Fail [PK Level:%d]",
					gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].m_PK_Level);
		...
		case 4:
			if ( gObj[aIndex].m_PK_Level >= 4 )
			{
				...
				LogAdd(LOG_BLACK, "[PK User][ChaosCastle] [%s][%s] Move Fail [PK Level:%d]",
					gObj[aIndex].AccountID, gObj[aIndex].Name, gObj[aIndex].m_PK_Level);
...
void GCPingSendRecv(PMSG_PING_RESULT* aRecv, int aIndex)
{
	...
	LogAdd(LOG_BLACK, "[%s][%s] Ping = %d ms",
...
void GCRegEventChipRecv(PMSG_REGEVENTCHIP* lpMsg, int aIndex)
{
	...
	{
		LogAdd(LOG_BLACK, "[EventChip] [%s][%s] Attempted ItemCopy using Trade Window",
			lpObj->AccountID, lpObj->Name);
	...
	if ( lpObj->m_IfState.type == 7 )
	{
		LogAdd(LOG_BLACK, "[EventChip] [%s][%s] Attempted ItemCopy using ChaosBox Window",
			lpObj->AccountID, lpObj->Name);
	...
	if ( lpObj->UseEventServer != FALSE )
	{
		...
		LogAdd(LOG_BLACK, "[EventChip] [%s][%s] Not Found EventChip (RegEventchip) #1 %d",
	...
	if ( !sitem->IsItem() )
	{
		...
		LogAdd(LOG_BLACK, "[EventChip] [%s][%s] Not Found EventChip (RegEventchip) #2 %d",
			lpObj->AccountID, lpObj->Name, lpMsg->ChipPos);
	...
	switch ( lpMsg->Type )
	{
		case 0x00:
			...
			else
			{
				...
				LogAdd(LOG_BLACK, "[EventChip] [%s][%s] Not Found EventChip (RegEventchip) #3 %d",
					lpObj->AccountID, lpObj->Name, lpMsg->ChipPos);
		...
		case 0x01:
			if ( sitem->m_Type == ITEMGET(14,21) && sitem->m_Level == 1 ) // Stone
			{
				...
				LogAdd(LOG_BLACK, "[Stone] [%s][%s] Register Stone (Stone Pos:%d, Serial:%d)",
			...
			else
			{
				...
				LogAdd(LOG_BLACK, "[Stone] [%s][%s] Not Found EventChip (Stone Pos: %d)",
					lpObj->AccountID, lpObj->Name, lpMsg->ChipPos);
...
void GCGetMutoNumRecv(PMSG_GETMUTONUMBER* lpMsg, int aIndex)
{
	...
	LogAdd(LOG_BLACK, "[EventChip] [%s][%s] Request MutoNumber",
		gObj[aIndex].AccountID, gObj[aIndex].Name);
}
...
void GCUseRenaChangeZenRecv(PMSG_EXCHANGE_EVENTCHIP* lpMsg, int aIndex)
{
	...
	if ( lpMsg->btType == 0x01 )
		LogAdd(LOG_BLACK, "[EventChip] [%s][%s] Request Change Stones", gObj[aIndex].AccountID, gObj[aIndex].Name);
	else
		LogAdd(LOG_BLACK, "[EventChip] [%s][%s] Request Change Rena", gObj[aIndex].AccountID, gObj[aIndex].Name);
}
...
void CGRequestEnterBloodCastle(PMSG_REQ_MOVEBLOODCASTLE* lpMsg, int iIndex)
{
	...
	LogAdd(LOG_BLACK, "[Blood Castle] (%d) (Account:%s, Name:%s) Entered Blood Castle (Invisible Cloak Serial:%d)",
...
void CGRequestEnterChaosCastle(PMSG_REQ_MOVECHAOSCASTLE* lpMsg, int iIndex)
{
	...
	LogAdd(LOG_BLACK, "[Chaos Castle] (%d) (Account:%s, Name:%s) Entered Chaos Castle (Guard Mail Set Serial:%d)",
...
void CGRequestLottoRegister(PMSG_REQ_2ANV_LOTTO_EVENT* lpMsg, int aIndex)
{
	...
	LogAdd(LOG_BLACK, "[Mu_2Anv_Event] [%s][%s] Register Lotto Number (Serial: %s-%s-%s)",
...
void CGReqMoveOtherServer(PMSG_REQ_MOVE_OTHERSERVER * lpMsg, int aIndex)
{
	...
	if ( !gObjJoominCheck(aIndex, lpMsg->LastJoominNumber))
	{
		...
		LogAdd(LOG_BLACK, "[CharTrasfer] Fail (JoominNumber) [%s][%s]",
			lpObj->AccountID, lpObj->Name);
	...
	LogAdd(LOG_BLACK, "[CharTrasfer] Request Character Trasfer [%s][%s]",
		lpObj->AccountID, lpObj->Name);
}
...
void GCNPggCheckSumRecv(PMSG_NPROTECTGGCHECKSUM * lpMsg, int aIndex)
{
	...
	LogAdd(LOG_BLACK, "[NPgg] Recv Checksum = %x,AuthValue = %x [%s][%s]",
	...
	if ( dwGGErrCode )
	{
		LogAdd(LOG_BLACK, "[NPgg] Invalid Checksum Detect Checksum = %x,AuthValue = %x [%s][%s]",
			lpMsg->m_ggadCheckSum.dwIndex, lpObj->NPggCSAuth.m_AuthAnswer.dwIndex,
			lpObj->AccountID, lpObj->Name);
	...
	LogAdd(LOG_BLACK, "[NPgg] Checksum Clear %x [%s][%s]",
...
void CGDuelStartRequestRecv(PMSG_REQ_START_DUEL * lpMsg, int aIndex)
{
	...
	{
		LogAdd(LOG_BLACK, "error : %s %d (%d)", __FILE__, __LINE__, iDuelIndex);
		return;
	}
	...
	LogAdd(LOG_BLACK, "[Duel] [%s][%s] Requested to Start Duel to [%s][%s]",
...
void CGDuelEndRequestRecv(PMSG_REQ_END_DUEL * lpMsg, int aIndex)
{
	...
	if ( OBJMAX_RANGE(gObj[iDuelIndex].m_iDuelUser) )
	{
		if ( gObj[iDuelIndex].m_iDuelUser == aIndex )
		{
			...
			LogAdd(LOG_BLACK, "[Duel] [%s][%s] Duel Ended [%s][%s]",
				gObj[aIndex].AccountID, gObj[aIndex].Name,
		...
		if ( gObj[iDuelIndex].m_iDuelUserReserved == aIndex )
		{
			...
			if ( !strcmp(szDuelName, szDuelName2))
			{
				...
				else if ( lpMsg->bDuelOK == true )
				{
					...
					LogAdd(LOG_BLACK, "[Duel] [%s][%s] Duel Started [%s][%s]",
						gObj[aIndex].AccountID, gObj[aIndex].Name,
						gObj[iDuelIndex].AccountID, gObj[iDuelIndex].Name);
				}
			}
...
void CGRequestPetItemCommand(PMSG_REQUEST_PET_ITEM_COMMAND * lpMsg, int aIndex)
{
	if ( !gObjIsConnectedGP(aIndex))
	{
		LogAdd(LOG_BLACK, "error-L2 : Index %s %d", __FILE__, __LINE__);
		return;
	}
...
void CGRequestPetItemInfo(PMSG_REQUEST_PET_ITEMINFO * lpMsg, int aIndex)
{
	...
	if ( lpMsg->InvenType == 0 )	// Inventory
	{
		...
		{
			LogAdd(LOG_BLACK, "[PetItem] [%s][%s] RequestPetItem Invalid Iventory nPos %d",
	...
	else if ( lpMsg->InvenType == 1 )	// Warehouse
	{
		if ( lpObj->LoadWareHouseInfo != false )
		{
			...
			{
				LogAdd(LOG_BLACK, "[PetItem] [%s][%s] RequestPetItem Invalid WareHouse nPos %d",
					lpObj->AccountID, lpObj->Name, lpMsg->nPos);
	...
	else if ( lpMsg->InvenType == 2 )	// Trade
	{
		...
		{
			LogAdd(LOG_BLACK, "[PetItem] [%s][%s] RequestPetItem Invalid Trade nPos %d",
				lpObj->AccountID, lpObj->Name, lpMsg->nPos);
	...
	else if ( lpMsg->InvenType == 3 )	// Target Trade
	{
		...
		if ( !TRADE_BOX_RANGE(lpMsg->nPos))
		{
			LogAdd(LOG_BLACK, "[PetItem] [%s][%s] RequestPetItem Invalid TargetTrade nPos %d",
				lpObj->AccountID, lpObj->Name, lpMsg->nPos);
	...
	else if ( lpMsg->InvenType == 4 )	// Chaos
	{
		...
		{
			LogAdd(LOG_BLACK, "[PetItem] [%s][%s] RequestPetItem Invalid Chaos nPos %d",
				lpObj->AccountID, lpObj->Name, lpMsg->nPos);
...
void GCGuildViewportInfo(PMSG_REQ_GUILDVIEWPORT * aRecv, int aIndex)
{
	...
	{
		LogAdd(LOG_BLACK, "★★☆ 길드 정보 찾을수 없음. 이름 : [%s] 번호 : %d",
...
void CGReqMapSvrAuth(PMSG_REQ_MAPSERVERAUTH * lpMsg, int iIndex)
{
	...
	{
		LogAdd(LOG_RED,  "error-L1: Serial error [%s] [%s]", id, btSerial);
		GCAnsMapSvrAuth(iIndex, 6);
		CloseClient(iIndex);
		return;
	}
	...
	if ( PacketCheckTime(lpObj) == FALSE )
	{
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 217)), iIndex, id);
		CloseClient(iIndex);
		return;
	}
	...
	{
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 218)), iIndex, id);
		CloseClient(iIndex);
		return;
	}
	...
	if ( gObj[iIndex].LoginMsgSnd != 0 )
	{
		...
		{
			LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 219)), iIndex, id, gObj[iIndex].Ip_addr);
		}
		else
		{
			LogAdd(LOG_BLACK, "error : %d %s %d", gObj[iIndex].LoginMsgSnd, __FILE__, __LINE__);
		}
	...
	LogAdd(LOG_BLACK, "[MapServerMng] Map Server Join Send : [%s][%s](%d)",
...
void GCAnsMapSvrAuth(int iIndex, int iResult)
{
	if ( !OBJMAX_RANGE(iIndex) )
	{
		LogAdd(LOG_RED, "[MapServerMng] Packet Error JG [0x7B] - Index out of bound : %d",iIndex);
		return;
	}
...
void GCAnsRegCastleSiege(int iIndex, int iResult, LPSTR lpszGuildName)
{
	...
	{
		LogAdd(LOG_BLACK, "[CastleSiege] [%s][%s] Registered Castle Siege (GUILD:%s) - Result:%d",
...
void GCAnsGiveUpCastleSiege(int iIndex, int iResult, BOOL bGiveUp, int iMarkCount, LPSTR lpszGuildName)
{
	...
	if ( gObjIsConnected(iIndex) )
	{
		LogAdd(LOG_BLACK, "[CastleSiege] [%s][%s] GiveUp Castle Siege (GUILD:%s) - Result:%d",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName, iResult);
	}
#endif
}
...
void CGReqNpcBuy(PMSG_REQ_NPCBUY * lpMsg, int iIndex)
{
	...
	{
		LogAdd(LOG_RED,  "[CastleSiege] CGReqNpcBuy() ERROR - Authority Fail [%s][%s], Guild:(%s)(%d), Npc:(CLS:%d, IDX:%d)",
	...
	if ( !bRET_VAL )
	{
		...
		LogAdd(LOG_RED,  "[CastleSiege] CGReqNpcBuy() ERROR - CCastleSiege::CheckAddDbNPC() [%s][%s], Guild:(%s)(%d), Npc:(CLS:%d, IDX:%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
			gObj[iIndex].GuildStatus, lpMsg->iNpcNumber, lpMsg->iNpcIndex);
		return;
	}

	LogAdd(LOG_RED,  "[CastleSiege] CGReqNpcBuy() OK - [%s][%s], Guild:(%s)(%d), Npc:(CLS:%d, IDX:%d)",
		gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
		gObj[iIndex].GuildStatus, lpMsg->iNpcNumber, lpMsg->iNpcIndex);
#endif
}
...
void CGReqNpcRepair(PMSG_REQ_NPCREPAIR * lpMsg, int iIndex)
{
	...
	if ( g_CastleSiege.CheckGuildOwnCastle(gObj[iIndex].GuildName) == FALSE ||
		 (gObj[iIndex].GuildStatus != 0x80 && gObj[iIndex].GuildStatus != 0x40 ) )
	{
		LogAdd(LOG_RED,  "[CastleSiege] CGReqNpcRepair() ERROR - Authority Fail [%s][%s], Guild:(%s)(%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
	...
	if ( !bRETVAL )
	{
		...
		LogAdd(LOG_RED,  "[CastleSiege] CGReqNpcRepair() ERROR - CL Request Fail [%s][%s], Guild:(%s)(%d), Npc:(CLS:%d, IDX:%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
			gObj[iIndex].GuildStatus, lpMsg->iNpcNumber, lpMsg->iNpcIndex);
	...
	if ( !gObjIsConnected(iNpcIndex) )
	{
		...
		LogAdd(LOG_RED,  "[CastleSiege] CGReqNpcRepair() ERROR - Npc Alive Data Mismatch [%s][%s], Guild:(%s)(%d), Npc:(CLS:%d, IDX:%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
	...
	if ( gObj[iIndex].Money < iRepairCost )
	{
		...
		LogAdd(LOG_RED,  "[CastleSiege] CGReqNpcRepair() ERROR - Money isn't enough [%s][%s], Guild:(%s)(%d), Npc:(CLS:%d, IDX:%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
			gObj[iIndex].GuildStatus, pNpcData.m_iNPC_NUM, pNpcData.m_iNPC_INDEX);
	...
	LogAdd(LOG_RED,  "[CastleSiege] CGReqNpcRepair() OK - [%s][%s], Guild:(%s)(%d), Npc:(CLS:%d, IDX:%d)",
		gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
		gObj[iIndex].GuildStatus, pNpcData.m_iNPC_NUM, pNpcData.m_iNPC_INDEX);
...
void CGReqNpcUpgrade(PMSG_REQ_NPCUPGRADE * lpMsg, int iIndex)
{
	...
	{
		LogAdd(LOG_BLACK, "[CastleSiege] CGReqNpcUpgrade() ERROR - Authority Fail [%s][%s], Guild:(%s)(%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
			gObj[iIndex].GuildStatus);
		return;
	}
		...
		case 283:
			{
				...
				if ( bRETVAL )
				{
					switch ( lpMsg->iNpcUpType )
					{
						...
						default:
							GCAnsNpcUpgrade(iIndex, 0x05, lpMsg->iNpcNumber, lpMsg->iNpcIndex, lpMsg->iNpcUpType, lpMsg->iNpcUpValue);
							LogAdd(LOG_BLACK, "[CastleSiege] CGReqNpcUpgrade() ERROR - UpType doesn't Exist [%s][%s], Guild:(%s)(%d), NPC CLS:%d, IDX:%d, UPTYPE:%d, UPVAL:%d",
								gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
								gObj[iIndex].GuildStatus, lpMsg->iNpcNumber, lpMsg->iNpcIndex,
	...
	if ( gObj[iIndex].Money < iNEED_MONEY )
	{
		...
		LogAdd(LOG_BLACK, "[CastleSiege] CGReqNpcUpgrade() ERROR - Money is Low [%s][%s], Guild:(%s)(%d), NPC CLS:%d, IDX:%d, UPTYPE:%d, UPVAL:%d, NXTLV:%d, Money:%d, Need Money:%d",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
	...
	if ( iGEMOFDEFEND_COUNT < iNEED_GEMOFDEFEND )
	{
		...
		LogAdd(LOG_BLACK, "[CastleSiege] CGReqNpcUpgrade() ERROR - Gem is Low [%s][%s], Guild:(%s)(%d), NPC CLS:%d, IDX:%d, UPTYPE:%d, UPVAL:%d, NXTLV:%d, Gem:%d, Need Gem:%d",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
	...
	LogAdd(LOG_BLACK, "[CastleSiege] [0xB2][0x07] CGReqNpcUpgrade() - Pay For Npc Upgrade (CLS:%d, IDX:%d, UPTYPE:%d, UPVAL:%d)",
	...
	LogAdd(LOG_BLACK, "[CastleSiege] CGReqNpcUpgrade() REQ OK - [%s][%s], Guild:(%s)(%d), NPC CLS:%d, IDX:%d, UPTYPE:%d, UPVAL:%d, NXTLV:%d",
		gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
...
void CGReqTaxMoneyInfo(PMSG_REQ_TAXMONEYINFO * lpMsg, int iIndex)
{
	...
	if ( g_CastleSiege.CheckGuildOwnCastle(gObj[iIndex].GuildName) == FALSE ||
		 (gObj[iIndex].GuildStatus != 0x80 ) )
	{
		LogAdd(LOG_RED,  "[CastleSiege] CGReqTaxMoneyInfo() ERROR - Authority Fail [%s][%s], Guild:(%s)(%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
			gObj[iIndex].GuildStatus);
		return;
	}
	...
	LogAdd(LOG_BLACK, "[CastleSiege] CGReqTaxMoneyInfo() REQ OK - [%s][%s], Guild:(%s)(%d)",
		gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
		gObj[iIndex].GuildStatus);
#endif
}
...
void CGReqTaxRateChange(PMSG_REQ_TAXRATECHANGE * lpMsg, int iIndex)
{
	...
	if ( g_CastleSiege.CheckGuildOwnCastle(gObj[iIndex].GuildName) == FALSE ||
		 (gObj[iIndex].GuildStatus != 0x80 ) )
	{
		LogAdd(LOG_RED,  "[CastleSiege] CGReqTaxRateChange() ERROR - Authority Fail [%s][%s], Guild:(%s)(%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
			gObj[iIndex].GuildStatus);
		return;
	}
	...
	if ( iTaxRate < 0 || iTaxRate > iMaxTaxRate ) 
	{
		...
		LogAdd(LOG_BLACK, "[CastleSiege] CGReqTaxRateChange() ERROR - Tax Rate Out of Range [%s][%s], Guild:(%s)(%d), TaxType:%d, TaxRate:%d",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
	...
	LogAdd(LOG_BLACK, "[CastleSiege] CGReqTaxRateChange() REQ OK - [%s][%s], Guild:(%s)(%d), TaxType:%d, TaxRate:%d",
		gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
		gObj[iIndex].GuildStatus, lpMsg->btTaxType, iTaxRate);
#endif
}
...
void CGReqMoneyDrawOut(PMSG_REQ_MONEYDRAWOUT * lpMsg, int iIndex)
{
	...
	if ( g_CastleSiege.CheckGuildOwnCastle(gObj[iIndex].GuildName) == FALSE ||
		 (gObj[iIndex].GuildStatus != 0x80 ) )
	{
		LogAdd(LOG_RED,  "[CastleSiege] CGReqMoneyDrawOut() ERROR - Authority Fail [%s][%s], Guild:(%s)(%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
			gObj[iIndex].GuildStatus);
		return;
	}
	...
	if ( iMoneyChange <= 0 )
	{
		...
		LogAdd(LOG_BLACK, "[CastleSiege] CGReqMoneyDrawOut() ERROR - Req Money < 0 [%s][%s], Guild:(%s)(%d), Money:%d, TotMoney:%d",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
	...
	if ( bRETVAL == FALSE )
	{
		...
		LogAdd(LOG_BLACK, "[CastleSiege] CGReqMoneyDrawOut() ERROR - Castle Money is Low [%s][%s], Guild:(%s)(%d), Money:%d, TotMoney:%d",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
			gObj[iIndex].GuildStatus, iMoneyChange, gObj[iIndex].Money);
	...
	LogAdd(LOG_BLACK, "[CastleSiege] CGReqMoneyDrawOut() REQ OK - [%s][%s], Guild:(%s)(%d), Money:%d, TotMoney:%d",
		gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
		gObj[iIndex].GuildStatus, iMoneyChange, gObj[iIndex].Money);
#endif
}
...
void CGReqCsGateOperate(PMSG_REQ_CSGATEOPERATE * lpMsg, int iIndex)
{
	...
	if ( g_CastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE )
	{
		...
		{
			LogAdd(LOG_BLACK, "[CastleSiege] CGReqCsGateOperate() ERROR - Authority Fail [%s][%s], Guild:(%s)(%d)",
				gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
				gObj[iIndex].GuildStatus);
			return;
		}
	...
	if ( g_CastleSiege.CheckCsGateAlive(iGateIndex) == TRUE )
	{
		...
		if ( !bRETVAL )
		{
			...
			LogAdd(LOG_BLACK, "[CastleSiege] CGReqCsGateOperate() ERROR - Operate Gate Failed [%s][%s], Guild:(%s)(%d)",
				gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
				gObj[iIndex].GuildStatus);
		...
		else
		{
			...
			LogAdd(LOG_BLACK, "[CastleSiege] CGReqCsGateOperate() OK - [%s][%s], Guild:(%s)(%d), DOOR:(%d)(X:%d,Y:%d)(STATUE:%d)",
				gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
	...
	else
	{
		...
		LogAdd(LOG_BLACK, "[CastleSiege] CGReqCsGateOperate() ERROR - Gate Doesn't Exist [%s][%s], Guild:(%s)(%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
			gObj[iIndex].GuildStatus);
	}
#endif
...
void GCAnsCsAccessCrownState(int iIndex, BYTE btCrownState)
{
	...
	if ( btCrownState == 0 )
	{
		...
		LogAdd(LOG_BLACK, "[CastleSiege] [Reg. Attempt] Accumulated Crown AccessTime : %d [%s](%s)(%s)",
	...
	if ( btCrownState == 1 )
	{
		LogAdd(LOG_BLACK, "[CastleSiege] [Reg. Success] Accumulated Crown AccessTime : %d [%s](%s)(%s)",
			gObj[iIndex].m_iAccumulatedCrownAccessTime, gObj[iIndex].GuildName, gObj[iIndex].AccountID,
			gObj[iIndex].Name);
		gObj[iIndex].m_iAccumulatedCrownAccessTime = 0;
	}
	...
	if ( btCrownState == 2 )
	{
		...
		LogAdd(LOG_BLACK, "[CastleSiege] [Reg. Fail] Accumulated Crown AccessTime : %d [%s](%s)(%s)",
			gObj[iIndex].m_iAccumulatedCrownAccessTime, gObj[iIndex].GuildName, gObj[iIndex].AccountID,
			gObj[iIndex].Name);
	}
...
void CGReqNpcDbList(PMSG_REQ_NPCDBLIST * lpMsg, int iIndex)
{
	...
	if ( g_CastleSiege.CheckGuildOwnCastle(gObj[iIndex].GuildName) == FALSE ||
		 ( gObj[iIndex].GuildStatus != 0x80 && gObj[iIndex].GuildStatus != 0x40 ) )
	{
		LogAdd(LOG_RED,  "[CastleSiege] CGReqNpcDbList() ERROR - Authority Fail [%s][%s], Guild:(%s)(%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName,
			gObj[iIndex].GuildStatus);
		...
		if ( lpObj->Money > iEnterTaxMoney )
		{
			if ( lpObj->MapNumber == MAP_INDEX_CASTLESIEGE )
			{
				if ( gObjMoveGate(lpObj->m_Index, 95) == TRUE )
				{
					...
					LogAdd(LOG_BLACK, "[Castle HuntZone] [%s][%s] - Entrance TAX : %d - %d = %d",
