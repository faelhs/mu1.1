...
void CObjAttack::Load()
{
	//this->m_Lua.DoFile(".\\Customs\\Scripts\\Attack.lua");
}
...
BOOL CObjAttack::Attack(LPOBJ lpObj, LPOBJ lpTargetObj, CMagicInf* lpMagic,  int magicsend, unsigned char MSBFlag, int AttackDamage, BOOL bCombo)
{
	...
	if ( AttackDamage == 0 )
	{
		if ( !this->MissCheck(lpObj, lpTargetObj, skill, skillSuccess, magicsend, bAllMiss) )
		{
			return FALSE;
		}
		...
		if ( gObjSatanSprite(lpObj) == TRUE )
		{
			...
			{
				AttackDamage = AttackDamage * gServerInfo.m_Data.wSatanAddDamage / 10.0f;
			}
		...
		if ( gObjAngelSprite(lpTargetObj) == TRUE )
		{
			if ( AttackDamage > 1 )
			{
				AttackDamage = AttackDamage * gServerInfo.m_Data.wAngelAddDefense / 100;
			}
		}
		...
		if ( gObjDenorantSprite(lpObj ) )
		{
			...
			{
				AttackDamage = AttackDamage * gServerInfo.m_Data.wDinorantAddDamage / 100.0f;
			}
		...
		if ( gObjDarkHorse(lpTargetObj ) )
		{
			...
			int decdamage = gServerInfo.m_Data.wDarkHorseAddDamage - ((Darkhorse->m_PetItem_Level + 30) / 2 );
	...
	else
	{
		...
		if (lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_USER)
		{
			AttackDamage = AttackDamage * gServerInfo.m_Character.PvPDamageRate[lpObj->Class][lpTargetObj->Class] / 100;
		}

		else if (lpObj->Type == OBJ_USER && lpTargetObj->Type == OBJ_MONSTER)
		{
			AttackDamage = AttackDamage * gServerInfo.m_Character.PvMDamageRate[lpObj->Class] / 100;
		}
	...
	{
		LogAdd(LOG_BLACK, "error-Level1 : [%s][%s] Str:%d %d %d %d %d %d %d",
	...
	if ( AttackDamage > 0 )
	{
		int iReflect = lpTargetObj->DamageReflect;

		if (iReflect > gServerInfo.m_Character.ReflectDamage)
		{
			iReflect = gServerInfo.m_Character.ReflectDamage;
		}

		int atd_reflect = AttackDamage * iReflect / 100;
