...
//	GS-CS	1.00.18	JPN	0x0046B3C0	-	Completed
...
void CEledoradoEvent::RegenGoldGoblen()
{
	...
	for (n=0;n<OBJ_MAXMONSTER;n++)
	{
		if ( gObj[n].Class == 78 )
		{
			...
			LogAdd(LOG_BLACK, "Make GoldGoblen : %d, %d,%d",
...
//	int Map[2]={0,3};
	...
	for (n=0;n<OBJ_MAXMONSTER;n++)
	{
		if ( gObj[n].Class == 53 )	// Golden Titan
		{
			...
			LogAdd(LOG_BLACK, "Make GoldTaitan : %d, %d,%d",
				MapNumber, gObj[n].X, gObj[n].Y);
		}
...
void CEledoradoEvent::RegenGoldDercon()
{
	...
	for (n=0;n<OBJ_MAXMONSTER;n++)
	{
		if ( gObj[n].Class == 79 )	// Gold Dercon
		{
			...
			LogAdd(LOG_BLACK, "Make GoldDercon : %d, %d,%d",
				MapNumber, gObj[n].X, gObj[n].Y);
...
//	int Map[2]={0,3};
	...
	for (n=0;n<OBJ_MAXMONSTER;n++)
	{
		if ( gObj[n].Class == 80 )	// Devil Lizard King
		{
			...
			LogAdd(LOG_BLACK, "Make GoldLizarKing : %d, %d,%d",
				MapNumber, gObj[n].X, gObj[n].Y);
		}
...
//	int Map[2]={0,3};
	...
	for (n=0;n<OBJ_MAXMONSTER;n++)
	{
		if ( gObj[n].Class == 82 )	// Gold Tantalos
		{
			...
			LogAdd(LOG_BLACK, "Make GoldTantarus : %d, %d,%d",
				MapNumber, gObj[n].X, gObj[n].Y);
		}
...
void CEledoradoEvent::Start_Menual()
{
	...
	LogAdd(LOG_BLACK, "¡Û¡Ü[Event Management] [Start] EledoradoEvent Event!");
