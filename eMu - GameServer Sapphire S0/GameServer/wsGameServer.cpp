...
//	GS-CS	1.00.18	JPN	0x0048D70	-	Completed
#include "stdafx.h"
...
BOOL CwsGameServer::CreateServer(LPSTR ip_addr, WORD port, DWORD WinServerMsg, DWORD WinClientMsg)
{
	...
	if ( nResult == -1 )
	{
		LogAdd(LOG_BLACK, "Server bind error %d", WSAGetLastError() );
	...
	{
		closesocket(this->m_socket );
		LogAdd(LOG_BLACK, "Server listen error %d", WSAGetLastError() );
		return 0;
	}
	...
	{
		closesocket(this->m_socket);
		LogAdd(LOG_BLACK, "Server WSAAsyncSelect error %d", WSAGetLastError());
		return 0;
	}
...
BOOL CwsGameServer::DataSocketSend(SOCKET socket,  char* buf, int len)
{
	...
	{
		LogAdd(LOG_BLACK, "INVALID_SOCKET %s %d", __FILE__, __LINE__);
		return 0;
	}
