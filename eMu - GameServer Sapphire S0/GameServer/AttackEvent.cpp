...
// GS-CS 1.00.18	0x0046AA80 - Completed
#include "stdafx.h"
	...
	for ( int n = 0; n < OBJ_MAXMONSTER ; n++ )
	{
		if ( gObj[n].Class == 53 || gObj[n].Class == 54 )	// Golden Titan + Golden Soldier
		{
			...
			if ( gObj[n].Class == 53 )
			{
				...
				LogAdd(LOG_BLACK, "event %d %d %d create ", this->m_BossMap53, this->m_BossMapX53, this->m_BossMapY53);
		...
		else if ( gObj[n].Class == 55 || gObj[n].Class == 56 )	// Death Bone + Death King
		{
			...
			if ( gObj[n].Class == 55 )
			{
				...
				LogAdd(LOG_BLACK, "event %d %d %d create ", this->m_BossMap55, this->m_BossMapX55, this->m_BossMapY55 );
			}
			else
			{
...
void CAttackEvent::Start_Menual() 
{
	...
	LogAdd(LOG_BLACK, "¡Û¡Ü[Event Management] [Start] Attack Event!");	// Deathway translation here M-[Event Management] [Start] Attack Event! 
	this->EventState= false;
	this->Start();
	this->Active();
}
