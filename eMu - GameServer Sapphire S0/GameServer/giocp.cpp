...
int CreateListenSocket()
{
	...
	if ( g_Listen == -1 )
	{
		LogAdd(LOG_BLACK, "WSASocket() failed with error %d", WSAGetLastError() );
		return 0;
	}
	else
	{
		...
		else
		{
			...
			if (nRet == -1)
			{
				LogAdd(LOG_BLACK, "listen() failed with error %d", WSAGetLastError());
				return 0;
			}
			else
			{
...
DWORD WINAPI IocpServerWorker(LPVOID p)
{
	...
	__try
	{
		...
		{
			LogAdd(LOG_BLACK, "CreateIoCompletionPort failed with error: %d", GetLastError());
			__leave;
		}
	...
	while(TRUE)
	{
		...
		if( !bSuccess )
		{
			if( lpOverlapped != NULL )
			{
				...
				{
					EnterCriticalSection(&criti);
					LogAdd(LOG_BLACK, "Error Thread : GetQueueCompletionStatus( %d )", GetLastError());
					LeaveCriticalSection(&criti);
					return 0;
				}
			}
		...
		if( dwIoSize == 0 )
		{
			LogAdd(LOG_BLACK, "Connection Closed, dwIoSize == 0 (Index:%d)", lpPerSocketContext->nIndex);
		...
		else if( lpIOContext->IOOperation == RECV_IO )
		{
			...
			{
				LogAdd(LOG_BLACK, "error-L1 : Socket Header error %d, %d", WSAGetLastError(), lpPerSocketContext->nIndex);
				CloseClient(lpPerSocketContext, 0);
				LeaveCriticalSection(&criti);
				continue;
			}
			...
			if( nRet == SOCKET_ERROR && (WSAGetLastError() != ERROR_IO_PENDING) )
			{
				LogAdd(LOG_BLACK, "WSARecv() failed with error %d", WSAGetLastError());
...
BOOL RecvDataParse(LPPER_IO_CONTEXT	lpIOContext, int uIndex)	
{
	...
	while ( 1 )
	{
		...
		{
			LogAdd(LOG_BLACK, "error-L1 : Header error (%s %d)lOfs:%d, size:%d",
		...
		if ( size <= 0 )
		{
			LogAdd(LOG_BLACK, "error-L1 : size %d",size);
			return FALSE;
		}
		...
		if ( size <= lpIOContext->nSentBytes )
		{
			if ( xcode == 0xC3 )
			{
				...
				else
				{
					...
					{
						LogAdd(LOG_RED,  "error-L1 : CStreamPacketEngine Adding Error : ip = %s account:%s name:%s HEAD:%x (%s,%d) State:%d", 
							gObj[uIndex].Ip_addr, 
					...
					{
						LogAdd(LOG_RED,  "error-L1 : CStreamPacketEngine ExtractPacket Error : ip = %s account:%s name:%s HEAD:%x (%s,%d) State:%d", 
							gObj[uIndex].Ip_addr, 
							gObj[uIndex].AccountID, 
							gObj[uIndex].Name, 
							headcode, 
					...
					{
						LogAdd(LOG_BLACK, "암호패킷 : [0x%x]" , headcode);
					}
		...
		else if ( lOfs > 0 )
		{
			...
			{
				LogAdd(LOG_BLACK, "error-L1 : recvbuflen 1 %s %d", __FILE__, __LINE__);
				break;
			}
			...
			if ( lpIOContext->nSentBytes < MAX_IO_BUFFER_SIZE ) 
			{
				...
				LogAdd(LOG_BLACK, "Message copy %d", lpIOContext->nSentBytes);
				//break;
			}
			break;
...
int DataSend(int aIndex, LPBYTE lpMsg, DWORD dwSize)
{
	...
	{
		LogAdd(LOG_BLACK, "error-L2 : Index(%d) %x %x %x ", dwSize, lpMsg[0], lpMsg[1], lpMsg[2]);
		LeaveCriticalSection(&criti);
		return false;
	}
	...
	{
		LogAdd(LOG_BLACK, "Error : Max msg(%d) %s %d", dwSize, __FILE__, __LINE__);
		CloseClient(aIndex);
		LeaveCriticalSection(&criti);
		return false;
	}
	...
	if ( lpIoCtxt->nWaitIO > 0 )
	{
		...
		{
			LogAdd(LOG_BLACK, "(%d)error-L2 MAX BUFFER OVER %d %d %d [%s][%s]", 
	...
	{
		LogAdd(LOG_BLACK, "(%d)error-L2 MAX BUFFER OVER %d %d [%s][%s]", 
	...
	if ( WSASend( gObj[aIndex].m_socket, &lpIoCtxt->wsabuf , 1, &SendBytes, 0, &lpIoCtxt->Overlapped, NULL) == -1 )
	{
		...
		if ( WSAGetLastError() != WSA_IO_PENDING )	
		{
			...
			{
				LogAdd(LOG_BLACK, "(%d)WSASend(%d) failed with error [%x][%x] %d %s ", 
...
int IoSendSecond(LPPER_SOCKET_CONTEXT lpPerSocketContext)
{
	...
	if ( WSASend(gObj[aIndex].m_socket, &lpIoCtxt->wsabuf, 1, &SendBytes, 0, &lpIoCtxt->Overlapped, NULL) == -1 )
	{
		if ( WSAGetLastError() != WSA_IO_PENDING )
		{
			LogAdd(LOG_BLACK, "WSASend(%d) failed with error %d %s ", __LINE__, WSAGetLastError(), gObj[aIndex].Ip_addr);
			CloseClient(aIndex);
			LeaveCriticalSection(&criti);
			return false;
		}
...
BOOL UpdateCompletionPort(SOCKET sd, int ClientIndex, BOOL bAddToList)
{
	...
	if (cp == NULL)
	{
		LogAdd(LOG_BLACK, "CreateIoCompletionPort: %d", GetLastError() );
		return FALSE;
	}
...
void CloseClient(int index)
{
	...
	{
		LogAdd(LOG_BLACK, "error-L1 : CloseClient index error");
		return;
	}

	if ( gObj[index].Connected == PLAYER_EMPTY )
	{
		LogAdd(LOG_BLACK, "error-L1 : CloseClient connect error");
		return;
	}
	...
	{
		LogAdd(LOG_BLACK, "error-L1 : CloseClient INVALID_SOCKET");
	}
