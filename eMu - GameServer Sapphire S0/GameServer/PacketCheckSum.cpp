...
// GS-CS	1.00.18	JPN	0x00461F60	-	Completed
...
void CPacketCheckSum::Check(int aIndex)
{
	...
	for (int i=0 ; i<MAX_PACKET_CHECKSUM_FUNCTION_INDEX ; i++)
	{
		if ( this->PaketChecksum[aIndex].Check[i] == 0 )
		{
			...
			LogAdd(LOG_BLACK, "PacketCheckSum Error [%d][%s][%s] %d",
	...
	LogAdd(LOG_BLACK, "Check PacketCheckSum [%d][%s][%s]", aIndex, gObj[aIndex].AccountID, gObj[aIndex].Name);
	this->ClearCheckSum(aIndex);
}
...
BOOL CPacketCheckSum::Add(int aIndex, int funcindex, DWORD checksum)
{
	...
	else
	{
		...
		LogAdd(LOG_BLACK, "PacketCheckSum : Compare Fail : [%d][%s][%s] (%d,%d)", aIndex,
...
void CPacketCheckSum::AddCheckSum(int aIndex, int funcindex, DWORD checksum)
{
	...
	{
		LogAdd(LOG_BLACK, "PacketCheckSum : Invalid FuncIndex : %d, %d", aIndex, funcindex);
		return;
	}
	...
	LogAdd(LOG_BLACK, "Insert PacketCheckSum %d, %d,%d", funcindex, this->m_ChecksumTableAVGCount[funcindex], checksum);
...
void CPacketCheckSum::SetClearChecksumFunc(int funcindex)
{
	...
	if ( iCount >= MAX_CHECKSUM_TABLE_AVG-1 )
	{
		...
		{
			LogAdd(LOG_BLACK, "Complete Init Checksum");

			for (int i=0;i<MAX_PACKET_CHECKSUM_FUNCTION_INDEX;i++)
			{
				this->m_ChecksumTable[i] = this->m_ChecksumTableAVG[i][0];
				LogAdd(LOG_BLACK, "%d, %d", i, this->m_ChecksumTable[i]);
			}
		}
	}
}