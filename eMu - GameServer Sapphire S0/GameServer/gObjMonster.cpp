...
CLogToFile KUNDUN_EVENT_LOG("KUNDUN_EVENT_LOG", ".\\Log\\KUNDUN_EVENT_LOG", 1);	// line : 68
...
void gObjMonsterSetHitDamage(LPOBJ lpObj, int hit_player, int hit_damage)
{
	...
	{
		LogAdd(LOG_BLACK, "error-L2 : Hit Damage Set error");
	}
}
			...
			if (  BC_MAP_RANGE(lpObj->MapNumber) != FALSE && lpObj->Type >= OBJ_MONSTER)
			{
				...
				if ( g_BloodCastle.CheckMonsterKillCount(lpObj->MapNumber-MAP_INDEX_BLOODCASTLE1) != false )
				{
					if (g_BloodCastle.m_BridgeData[lpObj->MapNumber-MAP_INDEX_BLOODCASTLE1].m_bBC_MONSTER_KILL_COMPLETE == false )
					{
						...
						LogAdd(LOG_BLACK, "[Blood Castle] (%d) All of the Monster Terminated -> %d", lpObj->MapNumber-MAP_INDEX_BLOODCASTLE1+1,
							g_BloodCastle.m_BridgeData[lpObj->MapNumber-MAP_INDEX_BLOODCASTLE1].m_iBC_MONSTER_KILL_COUNT);
				...
				if ( g_BloodCastle.m_BridgeData[lpObj->MapNumber-MAP_INDEX_BLOODCASTLE1].m_bBC_MONSTER_KILL_COMPLETE != false )
				{
					if ( g_BloodCastle.CheckBossKillCount(lpObj->MapNumber-MAP_INDEX_BLOODCASTLE1) != false )
					{
						if ( g_BloodCastle.m_BridgeData[lpObj->MapNumber-MAP_INDEX_BLOODCASTLE1].m_bBC_BOSS_MONSTER_KILL_COMPLETE == false )
						{
							...
							LogAdd(LOG_BLACK, "[Blood Castle] (%d) All of the Boss Monster Terminated -> %d",
			...
			if (lpObj->Class == 249 || lpObj->Class == 247 )	// Guard
			{
				if ( gEvent1 )
				{
					if ( !(rand()%30) )
					{
						...
						{
							ChatSend(lpObj, gServerInfo.m_Data.szGuardTalk);
							lpObj->m_PK_Count = 0;
						}
					}
				}
...
void gObjRefillMonsterHP(LPOBJ lpMonsterObj, int iRefillHPSec)
{
	...
	LPOBJ lpObj;
		...
		{
			LogAdd(LOG_BLACK, "[KUNDUN][Status Log] HP:%d RefillHP:%d Map:%d X:%d Y:%d",
			...
			if(gObjIsConnected(lpMonsterObj->LastAttackerID))
			{
				...
				LogAdd(LOG_BLACK, "[KUNDUN][Attacker Info] [%s][%s] HP:%d MP:%d X:%d Y:%d",
						  ...
						  BOOL bMustHaveAdditionalOption)
{
	...
	for (int i=0;i<iCount;i++)
	{
		...
		if ( item_drop )
		{
			...
			if ( exitem_drop )
			{
				option1rand = gServerInfo.m_Data.iDropItemMonsterRate[1];
				option2rand = gServerInfo.m_Data.iDropItemMonsterRate[2];
			...
			{
				option1rand = gServerInfo.m_Data.iDropItemMonsterRate[3];
				option2rand = gServerInfo.m_Data.iDropItemMonsterRate[4];
				option3rand = rand()%100;
				optionc = rand()%3;
				NOption = 0;
			}
...
void gObjMonsterDieGiveItem(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	if ( lpObj->Class == 295 )
	{
		...
		for ( int i=0; i<iMaxCount ; i++ )
		{
			...
			LogAdd(LOG_BLACK, "[Castle HuntZone] Boss Monster ItemDrop MaxHitUser [%d][%s][%s]",
	...
	{
		LogAdd(LOG_BLACK, "[★★Kundun EVENT] Kundun die, Killer [%s][%s]",
		...
		for ( int i=0;i<iMaxNumOfRewardItem;i++)
		{
			...
			if ( OBJMAX_RANGE(MaxHitUser) )
			{
				...
				LogAdd(LOG_BLACK, "[★Kundun EVENT] In KALIMA(7), ItemDrop MaxHitUser [%d][%s][%s]",
			...
			if ( lpObj->Class == 275 ) // Kundun	// useless if
			{
				if ( (rand()%10000) < 2500 )
				{
					...
					LogAdd(LOG_BLACK, "[○Kundun EVENT] Drop SetItem ");
			...
			if ( OBJMAX_RANGE(MaxHitUser ))
			{
				LogAdd(LOG_BLACK, "[○Kundun EVENT] Drop Item [%d][%s][%s]",
					i, gObj[MaxHitUser].AccountID, gObj[MaxHitUser].Name);
	...
	if(gServerInfo.m_Data.iDropItemMonsterRate[0] == 0)
	{
		ExtDropPer = 0;
	}
	else
	{
		ExtDropPer = rand()%gServerInfo.m_Data.iDropItemMonsterRate[0];
	}

	int ItemDropPer = gVip.GetDropRate(lpObj->m_Vip);
	ItemDropPer += gVip.GetDropRate(lpObj->m_Vip) * lpTargetObj->SetOpImproveItemDropRate / 100;
	...
	else if ( (rand()%moneyrate) < 10 )
	{
		...
		double money = lpObj->Money;
		float percent = 100;
		money *= gServerInfo.m_Data.fRateZen;
		...
		else
		{
			if ( lpObj->Level >= gSetItemInCastleHuntZoneDropLevel )
			{
				if ( (rand()%10000) < gSetItemInCastleHuntZoneDropRate )
				{
					...
					{
						LogAdd(LOG_BLACK, "[Castle HuntZone] Drop SetItem [%s][%s] ",
...
#endif
	if(lpObj->MapNumber == MAP_INDEX_AIDA)
	{
...
void CQeustNpcTeleport::Run(int aIndex)
{
	...
	if ( this->TimeCount > gQuestNPCTeleportTime )
	{
		...
		LogAdd(LOG_BLACK, "NPC %d로 이동", tableindex);	// #warning Translation Deathway
	}
}