...
//	GS-CS	1.00.18	JPN	0x004B5E50	-	Completed
...
BOOL CObjBaseAttack::CheckAttackArea(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	if ( lpObj->Type == OBJ_USER )
	{
		...
		if ( iRet != 0 )
		{
			LogAdd(LOG_BLACK, "[%s][%s] Try Attack In Not Attack Area (%s,%d,%d) errortype = %d",
...
BOOL CObjBaseAttack::MissCheck(LPOBJ lpObj, LPOBJ lpTargetObj, int skill, int skillSuccess, int magicsend, BOOL& bAllMiss)
{
	__try
	{
		...
		if ( bAllMiss != FALSE )	// When All Miss
		{
			if ( (rand()%100) >= 5 )
			{
				GCDamageSend(lpObj->m_Index, lpTargetObj->m_Index, 0, 0, iMSBDamage);
	...
	{
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(2, 12)), lpObj->Name);
		return FALSE;
	}
