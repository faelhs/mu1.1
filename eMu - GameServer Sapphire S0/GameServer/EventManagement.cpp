...
//	GS-CS	1.00.18	JPN	0x00477D60	-	Completed
...
void CEventManagement::Run()
{
	...
	if ( this->m_wToday_Year != sysTime.wYear || this->m_wToday_Month != sysTime.wMonth || this->m_wToday_Day != sysTime.wDay )
	{
		...
		LogAdd(LOG_BLACK, "○●[Event Management] 날짜 변경됨. %02d %02d %02d", this->m_wToday_Year, this->m_wToday_Month, this->m_wToday_Day);	// Deathway Need Translation
	...
	for ( it = this->m_vtEventTime.begin() ; it != this->m_vtEventTime.end() ; it++ )
	{
		...
		{
			LogAdd(LOG_BLACK, "○●[Event Management] 이벤트시작함. (%d) %02d %02d (state=%d)",
...
bool CEventManagement::Load(LPSTR lpszFileName)
{
	...
	fclose(SMDFile);
	LogAdd(LOG_BLACK, "[DOTH] %s file load!", lpszFileName);
