...
//	GS-CS	1.00.18	JPN	0x00470700	-	Completed
...
void DataSendEventChip(PCHAR pMsg, int size)
{
	if ( IsEventChipServerConnected == FALSE && ::EventChipServerConnect != FALSE )
	{
		...
		if ( GMEventChipServerConnect(::gEventChipServerIp, WM_GM_EVENTCHIP_CLIENT_MSG_PROC) == FALSE )
		{
			...
			LogAdd(LOG_BLACK, "Can not connect EventChip Server");
...
void EledoradoBoxOpenEven(LPOBJ lpObj, int boxtype,int addlevel,int money)
{
	...
	if ( EledoradoBox->GetBagCount() > 0 )
	{
		if ( (rand() % 100) < ItemDropRate )
		{
			...
			LogAdd(LOG_BLACK, "[%s][%s][Lucky Box Item Drop][Eledorado Box] : (%d)(X:%d/Y:%d) Item:%s(%d) Level:%d Op1:%d Op2:%d Op3:%d",
...
void EventChipOpenEven(LPOBJ lpObj)
{
	...
	if ( (rand()%100) < ::g_EventChipDropRateForBoxOfGold )
	{
		...
		LogAdd(LOG_BLACK, "EventChip Event ItemDrop Rena");
	...
	if ( LuckboxItemBag->GetBagCount() > 0 )
	{
		if ( (rand()%20) < ::g_ItemDropRateForBoxOfGold )
		{
			...
			LogAdd(LOG_BLACK, "[%s][%s][Lucky Box Item Drop][Event Chip] : (%d)(X:%d/Y:%d) Item:%s(%d) Level:%d Op1:%d Op2:%d Op3:%d",
				lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y, NewItem.GetName(),
				type, level, Option1, Option2, Option3);
...
void GoldMedalOpenEven(LPOBJ lpObj)
{
	...
	if ( GoldMedalItemBag->GetBagCount() > 0 )
	{
		if ( (rand()%20) < g_ItemDropRateForGoldMedal )
		{
			...
			LogAdd(LOG_BLACK, "[%s][%s][Lucky Box Item Drop][Gold Medal] : (%d)(X:%d/Y:%d) Item:%s(%d) Level:%d Op1:%d Op2:%d Op3:%d",
...
void SilverMedalOpenEven(LPOBJ lpObj)
{
	...
	if ( SilverMedalItemBag->GetBagCount() > 0 )
	{
		if ( (rand()%20) < g_ItemDropRateForSilverMedal )
		{
			...
			LogAdd(LOG_BLACK, "[%s][%s][Lucky Box Item Drop][Silver Medal] : (%d)(X:%d/Y:%d) Item:%s(%d) Level:%d Op1:%d Op2:%d Op3:%d",
				lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y, EventItem.GetName(),
				type, level, Option1, Option2, Option3);
...
void HeartOfLoveOpenEven(LPOBJ lpObj)
{
	...
	if ( HeartOfLoveItemBag->GetBagCount() > 0 )
	{
		if ( (rand()%20) < g_ItemDropRateForgHeartOfLove )
		{
			...
			LogAdd(LOG_BLACK, "[%s][%s][Lucky Box Item Drop][Heart Of Love] : (%d)(X:%d/Y:%d) Item:%s(%d) Level:%d Op1:%d Op2:%d Op3:%d",
				lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y, EventItem.GetName(),
				type, level, Option1, Option2, Option3);
...
void FireCrackerOpenEven(LPOBJ lpObj)
{
	...
	if ( FireCrackerItemBag->GetBagCount() > 0 )
	{
		if ( (rand()%10) < g_ItemDropRateForgFireCracker )
		{
			...
			LogAdd(LOG_BLACK, "[%s][%s][Lucky Box Item Drop][Fire Cracker] : (%d)(X:%d/Y:%d) Item:%s(%d) Level:%d Op1:%d Op2:%d Op3:%d",
				lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y, EventItem.GetName(),
				type, level, Option1, Option2, Option3);
...
void RingEventItemBoxOpen(LPOBJ lpObj)
{
	...
	if ( RingEventItemBag->GetBagCount() > 0 )
	{
		if ( (rand()%10) < 9 )
		{
			...
			LogAdd(LOG_BLACK, "[%s][%s][Ring Event](%d,%d,%d) ItemDrop : Item:%s %d Level:%d op1:%d op2:%d op3:%d",
				lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y, EventItem.GetName(),
				type, level, Option1, Option2, Option3);
...
void FriendShipItemBoxOpen(LPOBJ lpObj)
{
	...
	if ( RingEventItemBag->GetBagCount() > 0 )	// #error Chagne to FriendShipItemBag
	{
		if ( true )
		{
			...
			LogAdd(LOG_BLACK, "[%s][%s][FriendShip Ring Event](%d,%d,%d) ItemDrop : Item:%s %d Level:%d op1:%d op2:%d op3:%d",
				lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y, EventItem.GetName(),
				type, level, Option1, Option2, Option3);
...
void DarkLordHeartItemBoxOpen(LPOBJ lpObj)
{
	...
	if ( DarkLordHeartItemBag->GetBagCount() > 0 )
	{
		...
		if ( (rand()%100) < DropItemRate && lpObj->Level <= 100 )
		{
			...
			LogAdd(LOG_BLACK, "[%s][%s][Lucky Box Item Drop][Darklord Heart] : (%d)(X:%d/Y:%d) Item:%s(%d) Level:%d Op1:%d Op2:%d Op3:%d",
				lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y, EventItem.GetName(),
				type, level, Option1, Option2, Option3);
...
void HiddenTreasureBoxItemBoxOpen(LPOBJ lpObj)
{
	...
	{
		LogAdd(LOG_BLACK, "[Hidden TreasureBox Event] [%s][%s] Item Pick Failed - Data Error",
	...
	if ( (rand()%100) < iItemSuccessRate )
	{
		...
		LogAdd(LOG_BLACK, "[Hidden TreasureBox Event] [%s][%s] Event ItemDrop : (%d)(%d/%d) Item:(%s)%d Level:%d op1:%d op2:%d op3:%d exop:[%d,%d,%d,%d,%d,%d]",
...
void LuckyBoxOpenEven(LPOBJ lpObj)
{
	...
	if ( LuckboxItemBag->GetBagCount() > 0 )
	{
		if ( (rand()%10)<2 )
		{
			...
			LogAdd(LOG_BLACK, "[%s][%s][Lucky Box Item Drop] : (%d)(X:%d/Y:%d) Item:%s(%d) Level:%d Op1:%d Op2:%d Op3:%d",
				lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y, EventItem.GetName(),
				type, level, Option1, Option2, Option3);
...
BOOL AttackEvent53BagOpen(LPOBJ lpObj)
{
	...
	if ( Mon53->GetBagCount() > 0 )
	{
		...
		LogAdd(LOG_BLACK, "Event ItemDrop(53) : Item:%d Level:%d op1:%d op2:%d op3:%d",
			type, level, Option1, Option2, Option3);
...
BOOL AttackEvent55BagOpen(LPOBJ lpObj)
{
	...
	if ( Mon55->GetBagCount() > 0 )
	{
		...
		LogAdd(LOG_BLACK, "Event ItemDrop(55) : Item:%d Level:%d op1:%d op2:%d op3:%d",
			type, level, Option1, Option2, Option3);
...
void EGResultRegEventChip(PMSG_ANS_REGISTER_EVENTCHIP * aRecv)
{
	...
	{
		Result.ChipCount = -1;
		LogAdd(LOG_BLACK, "[EventChip] [%s][%s] RegEventServer Fail (RegEventchip) %d",
	...
	else
	{
		...
		LogAdd(LOG_BLACK, "[EventChip] [%s][%s] Delete EventChip (%d)",
			lpObj->AccountID, lpObj->Name, aRecv->Pos);
	}
...
void EGRecvRegMutoNum( PMSG_ANS_REGISTER_MUTONUM* aRecv)
{
	...
	LogAdd(LOG_BLACK, "[EventChip] [%s][%s] Make MutoNumber %d,%d,%d",
		lpObj->AccountID, lpObj->Name, 
...
void EGRecvChangeRena( PMSG_ANS_RESET_EVENTCHIP* aRecv)
{
	...
	if ( aRecv->bSUCCESS != FALSE )
	{
		...
		LogAdd(LOG_BLACK, "[EventChip] [%s][%s] ChangeRena AddMoney(%d)",
			lpObj->AccountID, lpObj->Name, lpObj->EventChipCount * 3000);
	}
	else
	{
		LogAdd(LOG_BLACK, "[EventChip] [%s][%s] ChangeRena Fail",
			lpObj->AccountID, lpObj->Name);
	}
...
void EGRecvRegStone( PMSG_ANS_REGISTER_STONES* aRecv)
{
	...
	if ( aRecv->bSUCCESS != FALSE )
	{
		...
		LogAdd(LOG_BLACK, "[Stone] [%s][%s] Delete Stones",
			lpObj->AccountID, lpObj->Name);
	}
	else
	{
		...
		LogAdd(LOG_BLACK, "[Stone] [%s][%s] RegEventServer Fail (Stones : %d)",
...
void EGRecvChangeStones( PMSG_ANS_RESET_EVENTCHIP* aRecv)
{
	...
	if ( aRecv->bSUCCESS != FALSE )
	{
		...
		LogAdd(LOG_BLACK, "[Stones] [%s][%s] ChangeRena AddMoney(%d)",
			lpObj->AccountID, lpObj->Name, lpObj->iStoneCount*3000);
	}
	else
	{
		LogAdd(LOG_BLACK, "[Stones] [%s][%s] ChangeRena Fail",
			lpObj->AccountID, lpObj->Name);
	}
...
void EGRecv2AnvRegSerial( PMSG_ANS_2ANIV_SERIAL* aRecv)
{
	...
	{
		LogAdd(LOG_BLACK, "[Mu_2Anv_Event] Error : Index is out of bound [%d]", aRecv->iINDEX);
		return;
	}
	...
	if ( aRecv->btIsRegistered == FALSE )
	{
		...
		{
			LogAdd(LOG_BLACK, "[Mu_2Anv_Event] Error : Gift Index is out of bound [%d]", aRecv->iGiftNumber);
		...
		switch ( aRecv->iGiftNumber )
		{
			...
			case 50:
				...
				break;
			default:
				LogAdd(LOG_BLACK, "[Mu_2Anv_Event] Error : iGiftNumber is Out of Boud [%d]", aRecv->iGiftNumber);
				break;
		}
	}
	...
	else
	{
		...
		LogAdd(LOG_BLACK, "[Mu_2Anv_Event] Error : Result Value is Wrong [%d]", aRecv->btIsRegistered);
	}

	LogAdd(LOG_BLACK, "[Mu_2Anv_Event] Register Serial Result : %d [%s][%s]",
...
void EGRecvRegRingGift( PMSG_ANS_REG_RINGGIFT* aRecv)
{
	...
	if ( aRecv->btIsRegistered == 1 )
	{
		if ( CHECK_LIMIT(aRecv->btGiftKind-1, 4) )
		{
			...
			LogAdd(LOG_BLACK, "[Ring Event] [%s][%s] Register Succeeded Result:%d, Gift:%d",
				gObj[aRecv->iINDEX].AccountID, gObj[aRecv->iINDEX].Name,
				aRecv->btIsRegistered, aRecv->btGiftKind);
		}
		else
		{
			LogAdd(LOG_BLACK, "[Ring Event] [%s][%s] Register Failed Result:%d, Gift:%d (out of bound, 1~4)",
				gObj[aRecv->iINDEX].AccountID, gObj[aRecv->iINDEX].Name,
				aRecv->btIsRegistered, aRecv->btGiftKind);
		}
	...
	LogAdd(LOG_BLACK, "[Ring Event] [%s][%s] Register Failed Result : %d",
		gObj[aRecv->iINDEX].AccountID, gObj[aRecv->iINDEX].Name,
...
void EGAnsRegCCOfflineGift( PMSG_ANS_REG_CC_OFFLINE_GIFT* lpMsg)
{
	...
	LogAdd(LOG_BLACK, "[Chaos Castle] [%s][%s] Success to Register OffLine Gift (GIFT:%s)",
...
void EGAnsRegDLOfflineGift( PMSG_ANS_REG_DL_OFFLINE_GIFT* lpMsg)
{
	...
	LogAdd(LOG_BLACK, "[DarkLord Heart Event] [%s][%s] Success to Register OffLine Gift (GIFT:%s)",
		szAccountID, szName, szGIFT_NAME);
}
...
void EGAnsRegHTOfflineGift( PMSG_ANS_REG_HT_OFFLINE_GIFT* lpMsg)
{
	...
	LogAdd(LOG_BLACK, "[Hidden TreasureBox Event] [%s][%s] Success to Register OffLine Gift (GIFT:%s)",
		szAccountID, szName, szGIFT_NAME);
}
...
void Japan1StAnivBoxOpen(LPOBJ lpObj, int iBoxLevel)
{
	...
	if ( (rand()%100) < iItemDropRate )
	{
		if ( Japan1StAnivBox->GetBagCount() > 0 )
		{
			...
			LogAdd(LOG_BLACK, "[Japan1StAnivBox] BoxLevel-%d Event ItemDrop : Item:%d Level:%d op1:%d op2:%d op3:%d ex:%d",
