...
//	GS-CS	1.00.18	JPN	0x0050BB90	-	Completed
...
int CSetItemOption::IsSetItem(int itemnum)	
{
	...
	{
		LogAdd(LOG_BLACK, "IsSetItem Check error: (itemnum:%d)", itemnum);
		return 0;
	}
...
int CSetItemOption::GetOptionTableIndex(int itemnum, int setoption)	
{
	if (itemnum<0 || itemnum > MAX_ITEMS-1)
	{
		LogAdd(LOG_BLACK, "GetOptionTableIndex Check error: (itemnum:%d)", itemnum);
		return -1;
	}

	if (setoption != 1 && setoption != 2)
	{
		LogAdd(LOG_BLACK, "GetOptionTableIndex setoption Check error: (itemnum:%d)", itemnum);
		return -1;
	}
...
char * CSetItemOption::GetSetOptionName(int optionindex)
{
	...
	{
		LogAdd(LOG_BLACK, "GetSetOptionName Check error: (itemnum:%d)", optionindex);
		return "";
	}
...
LPSTR CSetItemOption::GetSetOptionName(int itemnum, int settype)
{
	...
	if (itemnum<0 || itemnum > MAX_ITEMS-1)
	{
		LogAdd(LOG_BLACK, "GetSetOptionName Check error: (itemnum:%d)", itemnum);
		return "";
	}
...
int CSetItemOption::GetSetOption(int optionindex, int optablenum, int& op1, int& op2, int& opvalue1,int& opvalue2, int Class, int ChangeUp)
{
	...
	if (optionindex <0 || optionindex > MAX_SETITEM_OPTION-1)
	{
		LogAdd(LOG_BLACK, "GetSetOption Check error: (itemnum:%d)", optionindex);
		return 0;
	}
...
int CSetItemOption::GetMaxSetOptionCount(int optionindex)
{
	if (optionindex <0 || optionindex > MAX_SETITEM_OPTION-1)
	{
		LogAdd(LOG_BLACK, "GetMaxOptionCount Check error: (itemnum:%d)", optionindex);
		return -1;
	}
...
int CSetItemOption::CheckMixContition(int itemnum, int mixitemlevel)
{
	...
	if ( itemnum <0 || itemnum > MAX_ITEMS-1)
	{
		LogAdd(LOG_BLACK, "MakeSetOption Check error: (itemnum:%d)", itemnum);
		return 0;
	}
...
int CSetItemOption::GenSetOption(int itemnum)
{
	if ( itemnum < 0 || itemnum > MAX_ITEMS-1)
	{
		LogAdd(LOG_BLACK, "GenSetType Check error: (itemnum:%d)", itemnum);
		return FALSE;
	}
