...
//	GS-CS	1.00.18	JPN	0x00484870	-	Completed
...
int CMagicInf::Set(BYTE aType, BYTE aIndex, BYTE aLevel)
{
	if ( this->m_Skill != 0xFF )
	{
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 203)), __FILE__, __LINE__);
		return -1;
	}
...
int CMagicInf::GetDamage()
{
	...
	__except(subd=1,1)
	{
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(0, 111)), __FILE__, __LINE__);
	}
