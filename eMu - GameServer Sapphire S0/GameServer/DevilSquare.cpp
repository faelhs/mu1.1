...
// GS-CS	1.00.18	JPN	0x00462B60	-	Completed
#include "stdafx.h"
...
void CDevilSquare::SetClose()
{
	if ( LogAdd != NULL )	// LoL
	{
		LogAdd(LOG_BLACK, "[DevilSquare] Close");
	}
...
void CDevilSquare::CheckSync()
{
	...
	LogAdd(LOG_BLACK, "[DevilSquare] Sync Open Time. [%d]min remain", this->m_iRemainTime);
}
	...
	if ( this->m_iremainTimeSec != -1 )
	{
		...
		if ( lc2 != 0 )
		{
			...
			if (this->m_iremainTimeSec < 1 )
			{
				...
				LogAdd(LOG_BLACK, pNotice.Notice);
	...
	else if ( this->m_iTime < GetTickCount() )
	{
		...
		if ( this->m_iRemainTime <= 15 )
		{
			if ( (this->m_iRemainTime%5)== 0 )
			{
				...
				LogAdd(LOG_BLACK, (char*)pNotice.Notice);
			}
		}
...
void CDevilSquare::SetMonster()
{
	...
	for ( int n=0;n<gMSetBase.m_Count;n++)
	{
		if ( DS_MAP_RANGE(gMSetBase.m_Mp[n].m_MapNumber) != FALSE )
		{
			...
			if ( btDSIndex == 0xFF )
			{
				LogAdd(LOG_BLACK, "[DevilSquare] [%d] Invalid MonterType", wMonIndex);
				continue;
			}
...
void CDevilSquare::gDevilSquareMonsterRegen(LPOBJ lpObj)
{
	...
	{
		LogAdd(LOG_BLACK, "[DevilSquare] Invalid DevilSquareIndex [%d]", devilsquareindex);
		return;
	}
	...
	{
		LogAdd(LOG_BLACK, "[DevilSquare] [%d] Invalid MonterType", monstertype);
		return;
	}
	...
	LogAdd(LOG_BLACK, "[DevilSquare] Monter Regen [%d][%d][%d,%d]",
...
void CDevilSquare::SendEventStartMsg()
{
	...
	LogAdd(LOG_BLACK, "[DevilSquare] Start Event");
}
...
void CDevilSquare::DieProcDevilSquare(LPOBJ lpObj)
{
	...
	LogAdd(LOG_BLACK, "[DevilSquare] Dead [%s][%s][%d][%d]",
	...
	for ( int n=OBJ_STARTUSERINDEX;n<OBJMAX;n++)
	{
		if ( gObj[n].Connected == PLAYER_PLAYING )
		{
			if ( DS_MAP_RANGE(gObj[n].MapNumber) != FALSE )
			{
				...
				{
					LogAdd(LOG_RED,  "[DevilSquare] [%s][%s] Found user in DevilSquare [State:Close]",
