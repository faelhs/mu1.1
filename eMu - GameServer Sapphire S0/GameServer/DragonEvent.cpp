...
// GS-CS	1.00.18	JPN	0x0046A1F0	-	Completed
#include "stdafx.h"
...
void CDragonEvent::Run()
{
	...
	if ( this->EventState == 1 )
	{
		if ( ( GetTickCount() - this->EventStartTime ) > 3000 )
		{
			...
			LogAdd(LOG_BLACK, "Event Start 1");
			this->DragonActive();
			this->EventStartTime = GetTickCount();
			return;
		}
...
void CDragonEvent::Start_Menual()
{
	...
	LogAdd(LOG_BLACK, "¡Û¡Ü[Event Management] [Start] Dragon Event!");
	this->EventState=0;
	this->Start();
}
