...
void CCastleDeepEvent::StartEvent()
{
	...
	{
		LogAdd(LOG_BLACK, "[CastleDeep Event] Start Fail - No Assult Data");
	...
	if(bDoStart == FALSE)
	{
		LogAdd(LOG_BLACK, "[CastleDeep Event] Start Fail - bDoStart == FALSE");
		SetState(CD_STATE_NONE);
		return;
	}
...
void CCastleDeepEvent::CheckSync()
{
	...
	{
		LogAdd(LOG_RED, "[CastleDeep Event] No Schedule Time Data");
		this->SetState(CD_STATE_NONE);
		return;
	}
	...
	LogAdd(LOG_BLACK, "[CastleDeep Event] Sync Start Time. [%d] min remain (START HOUR:%d, MIN:%d)",
	...
	if(iTICK_MSEC >= 1000)
	{
		...
		if(m_bDoEvent != 0)
		{
				...
				&& m_iTIME_MSEC_REMAIN / 60000 != m_iTIME_NOTIFY_COUNT)
			{
				...
				{
					char szTemp[256];
					wsprintf(szTemp,lMsg.Get(MSGGET(12,152)));
					g_CastleSiege.SendMapServerGroupMsg(szTemp);
				}
			}
		}

		LogAdd(LOG_GREEN, "[CastleDeep Event] ProcState_Closed()	LEFT-MSEC:%d",m_iTIME_MSEC_REMAIN);
	}

	if(m_iTIME_MSEC_REMAIN <= 0)
	{
		char szTemp[256];
		wsprintf(szTemp,lMsg.Get(MSGGET(12,153)));
		g_CastleSiege.SendMapServerGroupMsg(szTemp);
		...
		LogAdd(LOG_BLACK, "[CastleDeep Event] - Event Started");
	}
}
...
using namespace std;
	...
	if(iTICK_MSEC >= 1000)
	{
		...
		LogAdd(LOG_GREEN, "[CastleDeep Event] ProcState_Playing()	LEFT-MSEC:%d",m_iTIME_MSEC_REMAIN);
	}
	...
	if(m_mapRunCheck.size() <= 0)
	{
		...
		else
		{
			if(m_uCheckEndTick + 600000 < GetTickCount())
			{
				...
				wsprintf(szTemp,lMsg.Get(MSGGET(12,154)));
		...
		LogAdd(LOG_BLACK, "[CastleDeep Event] - Event Ended");
	}
}
