		...
		else
		{
			...
			if ( mana > lpObj->Mana )
			{
				...
				while ( true )
				{
					...
					{
						LogAdd(LOG_BLACK, "★[CHECK_LOG_INFINITY] gObjIsConnected() error %s %d", __FILE__, __LINE__);
						break;
					}

					if ( count > 100 )
					{
						LogAdd(LOG_BLACK, "★[CHECK_LOG_INFINITY] ( _count > 100 ) error %s %d", __FILE__, __LINE__);
						break;
					}
				...
				if(lpObj->PathCount != 0 && lpObj->m_SkillHarden == 0 && lpObj->m_iSkillStunTime == 0)
				{
					...
					if( (GetTickCount() - lpObj->PathTime) > MoveTime && lpObj->PathCur < 14)
					{
						...
						if( lpObj->Type == OBJ_USER &&
							( (attr & 4) == 4 || (attr & 8) == 8) )
						{
							LogAdd(LOG_BLACK, "[ CHECK POSITION ] MoveMosterProc [%s][%s] Map[%d]-(%d,%d) User(%d,%d) Can not Move Position Attr[%d]",
...
void gObjCharZeroSet(int aIndex)
{
	...
	lpObj->m_Vip = 0;
	lpObj->m_Resets = 0;
	lpObj->m_MasterResets = 0;
	lpObj->m_Cash = 0;
	lpObj->m_Gold = 0;

	lpObj->m_DelayPost = 0;
	lpObj->m_DelayWare = 0;
	lpObj->m_DelayOpenWare = 0;

	lpObj->bReconnect = false;

	lpObj->m_WareNumber = 0;
...
int GetMapMoveLevel(LPOBJ lpObj, int mapnumber, int max_over)
{
	...
	{
		mapnumber = 0;
		LogAdd(LOG_BLACK, "error-L3 : map number not %s %d", __FILE__, __LINE__);
		return 0;
	}
...
void DbItemSetInByte(LPOBJ lpObj, struct SDHP_DBCHAR_INFORESULT* lpMsg, int ItemDbByte, bool* bAllItemExist)
{
	...
	for (n = 0; n < INVENTORY_SIZE; n++)
	{
		...
		{
			if(lpMsg->dbInventory[n*ItemDbByte] == 0xFF && (lpMsg->dbInventory[n*ItemDbByte+DBI_NOPTION_DATA] & 0x80) == 0x80)
			{
				continue;
			}
		}
		...
		if(itype != -1)
		{
			...
			{
				item.Convert(itype,item.m_Option1,item.m_Option2,item.m_Option3,DBI_GET_NOPTION(lpMsg->dbInventory[n*ItemDbByte+DBI_NOPTION_DATA]),lpMsg->dbInventory[n*ItemDbByte+DBI_SOPTION_DATA], 0, 1);
			}
			else
			{
				item.Convert(itype,item.m_Option1,item.m_Option2,item.m_Option3,DBI_GET_NOPTION(lpMsg->dbInventory[n*ItemDbByte+DBI_NOPTION_DATA]),lpMsg->dbInventory[n*ItemDbByte+DBI_SOPTION_DATA], 0, 3);
			}
			// ---
			...
			if(bIsItemExist == false)
			{
				...
				// ---
				LogAdd(LOG_BLACK, "error-L2 : Unknown Item found [%s][%s] (Type:%d, LEV:%d, DUR:%d OP1:%d, OP2:%d, OP3:%d, NEWOP:%d, SET:%d)",
...
void gObjSetBP(int aIndex)
{
	...
	{
		case 0: gObj[aIndex].MaxBP = (Strength * 0.2) + (Dexterity * 0.4) + (Vitality * 0.3) + (Energy * 0.2); break;
		case 1: gObj[aIndex].MaxBP = (Strength * 0.15) + (Dexterity * 0.2) + (Vitality * 0.3) + (Energy * 1.0); break;
		case 2: gObj[aIndex].MaxBP = (Strength * 0.3) + (Dexterity * 0.2) + (Vitality * 0.3) + (Energy * 0.2); break;
		case 3: gObj[aIndex].MaxBP = (Strength * 0.2) + (Dexterity * 0.25) + (Vitality * 0.3) + (Energy * 0.15); break;
		case 4: gObj[aIndex].MaxBP = (Strength * 0.3) + (Dexterity * 0.2) + (Vitality * 0.1) + (Energy * 0.15) + (Leadership * 0.3); break;
	}
}
...
BOOL gObjSetCharacter(LPBYTE lpdata, int aIndex)
{
	...
	{
		LogAdd(LOG_RED,  lMsg.Get(MSGGET(1, 242)), gObj[aIndex].AccountID, gObj[aIndex].Name);
		return FALSE;
	}
	// ---
	...
	{
		LogAdd(LOG_BLACK, "error-L1 : ChaosBox Init error %s %d", __FILE__, __LINE__);
	}
	// ---
	lpObj->m_PosNum		= 0;
	...
	lpObj->ChangeUP		= lpMsg->Class & 0x07;
	// ---
	...
	if(lpObj->Level < 1)
	{
		LogAdd(LOG_RED,  "error : %s Level Zero", lpMsg->Name);
		return FALSE;
	}
	// ---
	lpObj->X			= lpMsg->MapX;
	...
	{
		LogAdd(LOG_BLACK, "error : Map Number max over. %s %d", __FILE__, __LINE__);
		// ---
		lpObj->MapNumber = MAP_INDEX_LORENCIA;
		// ---
	...
	{
		lpObj->MapNumber = gServerInfo.m_Data.iStartMap[lpObj->Class];
		// ---
		MapC[lpObj->MapNumber].GetMapPos(lpObj->MapNumber, lpObj->X, lpObj->Y);
	}
	// ---
	...
	if ( lpObj->m_bMapSvrMoveReq == true )
	{
		...
		{
			LogAdd(LOG_RED,  "[MapServerMng] Dest Map Server doesn't have Map [%s][%s] : %d",
	...
	{
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 243)));
		MapC[lpObj->MapNumber].GetMapPos(lpObj->MapNumber, lpObj->X, lpObj->Y);
	}
	...
	if ( gt > 0 )
	{
		...
		{
			LogAdd(LOG_BLACK, "error : %d (%s %d)", result, __FILE__, __LINE__);
		}
	}
	...
	if ( ::g_QuestInfo.GetQuestState(lpObj, 2) == 2 )	// Ring of Honor
	{
		if ( lpObj->Level < QUEST_MINLEVEL_PLUSSTAT )
		{
			...
			LogAdd(LOG_BLACK, "[%s][%s] Find Invalid QuestInfo (%d)",
		...
		if ( IsGetInitinityArrowSkill == 0 && lpObj->Class == 2)
		{
			if(lpObj->Level >= g_SkillAdditionInfo.GetInfinityArrowUseLevel())
			{
				if(lpObj->ChangeUP == 1)
				{
					...
					{
						LogAdd(LOG_BLACK, "[%s][%s] Add Infinity Arrow Skill (Character Level : %d)(ChangeUp: %d)",
	...
	{
		lpObj->Authority = 2;
		LogAdd(LOG_RED,  lMsg.Get(MSGGET(1, 245)), lpObj->AccountID, lpObj->Name);
	...
	if ( (lpMsg->CtlCode & 0x20 ) == 0x20 )
	{
		...
		LogAdd(LOG_RED,  "(%s)(%s) Set Event GM", lpObj->AccountID, lpObj->Name);
	}
	...
	{
		lpObj->Penalty |= 4;
		LogAdd(LOG_BLACK, "Penalty : Item Don't touch  %s %s", lpObj->AccountID, lpObj->Name);
	}

	::gObjAuthorityCodeSet(lpObj);
	LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 246)), lpObj->m_Index, lpObj->AccountID, lpObj->Name);
	...
	{
		PMSG_NOTICE pNotice;
		TNotice::MakeNoticeMsgEx((TNotice *)&pNotice, 1, "Item not found !!!");
	...
	if((lpObj->Authority & 2) == 2 || (lpObj->Authority & 8) == 8 || (lpObj->Authority & 32) == 32)
	{
...
void gObjMagicTextSave(LPOBJ lpObj)
{
	...
	for ( int n=0;n<MAGIC_SIZE;n++)
	{
		...
		{
			LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 247)), lpObj->AccountID, lpObj->Name, n, lpObj->Magic[n].m_Skill);
			total++;
		}
	}
}
...
{
	LogAdd(LOG_BLACK, "[%s][%s] CharInfoSave : Class=%d Level=%d LVPoint=%d Exp=%u Str=%d Dex=%d Vit=%d Energy=%d Leadership:%d Map=%d Pk=%d",
	...
	for (int n=0;n<INVENTORY_SIZE;n++)
	{
		if ( lpObj->pInventory[n].IsItem() == TRUE )
		{
			if ( lpObj->pInventory[n].m_serial != 0 )
			{
				...
				LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 248)), lpObj->AccountID, lpObj->Name, n, lpObj->pInventory[n].GetName(),
					...
					lpObj->pInventory[n].m_SetOption, 0, 0, 0);
			}
		}
	}

	LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 249)), lpObj->AccountID, lpObj->Name, lpObj->Money);
}
	...
	for (int n=0;n<WAREHOUSE_SIZE;n++)
	{
		if ( lpObj->pWarehouse[n].IsItem() == TRUE )
		{
			if ( lpObj->pWarehouse[n].m_serial != 0 )
			{
				...
				LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 250)), lpObj->AccountID, lpObj->Name, n, lpObj->pWarehouse[n].GetName(),
					...
					NewOption[0], NewOption[1], NewOption[2], NewOption[3], NewOption[4], NewOption[5], NewOption[6],
					lpObj->pWarehouse[n].m_SetOption, 0, 0, 0);
			}
		}
	}

	LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 251)), lpObj->AccountID, lpObj->Name, lpObj->WarehouseMoney);
}
...
BOOL gObjSetPosMonster(int aIndex, int PosTableNum)
{
	...
	{
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(1, 252)), __FILE__, __LINE__);
		LogAdd(LOG_BLACK, "error-L2 : %d class : %d", PosTableNum, lpObj->Class);
		return FALSE;
	}
...
BOOL gObjSetMonster(int aIndex, int MonsterClass)
{
	...
	if ( MonsterClass == 77 )
	{
		...
		if ( iSL >= 0 )
		{
			...
			LogAdd(LOG_BLACK, "SkyLand Boss Monster Setting!!");
		}
		else
		{
			MsgBox("천공보스몹 설정 실패");
	...
	if ( MonsterClass == 76 )
	{
		LogAdd(LOG_BLACK, "SkyLand Boss Shield Setting!!");
	}
	...
	if (lpm->m_Level == 0)
	{
		LogAdd(LOG_BLACK, lMsg.Get( MSGGET(1, 254)), lpObj->Class, __FILE__, __LINE__);
	}
...
short gObjAdd(SOCKET aSocket, char* ip, int aIndex)
{
	...
	LogAdd(LOG_BLACK, "connect : [%d][%s]", aIndex, ip);
	gObjCount++;
...
short gObjAddMonster(int iMapNumber)
{
	...
	while ( true )
	{
		...
		{
			LogAdd(LOG_BLACK, lMsg.Get( MSGGET ( 1, 255 )), __FILE__, __LINE__ );
			return -1;
		}
	}
	return -1;
...
void gObjAllLogOut()
{
	...
	if ( bAllLogOut == 0 )
	{
		...
		gObjAllDisconnect();
		Sleep(1000);
		LogAdd(LOG_RED,  lMsg.Get( MSGGET(2, 0)));
	}
}
...
void gObjTradeSave(LPOBJ lpObj, int index)
{
	if ( lpObj->m_IfState.use != 0 && lpObj->m_IfState.type == 1 )
	{
		...
		if ( tObjNum > 0 )
		{
			...
			LogAdd(LOG_BLACK, lMsg.Get(MSGGET(2, 1)), gObj[tObjNum].AccountID, gObj[tObjNum].Name);
		}

		gObjTradeCancel( index );
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(2,2)), lpObj->AccountID, lpObj->Name);
	}
}
...
void gObjBillRequest(LPOBJ lpObj)
{
	...
	else if ( m_ObjBill[lpObj->m_Index].GetPayCode() == 3)
	{
		...
		LogAdd(LOG_BLACK, "[%s][%s] BillType : (Time) RemainPoint : (%d)",
	...
	else if ( m_ObjBill[lpObj->m_Index].GetPayCode() == 4)
	{
		...
		LogAdd(LOG_BLACK, "[%s][%s] BillType : (Date) RemainDate : (%s-%s-%s)",
	...
	else if ( m_ObjBill[lpObj->m_Index].GetPayCode() == 5)		// FREE
	{
		...
		LogAdd(LOG_BLACK, "[%s][%s] BillType : (NoCharge)", lpObj->AccountID, lpObj->Name);
	}
...
short gObjMemFree(int index)
{
	...
	{
		LogAdd(LOG_BLACK, "(%s)(%d) = index over error (%d)", __FILE__, __LINE__, index);
		return false;
	}
...
BOOL gObjGameClose(int aIndex)
{
	...
	{
		LogAdd(LOG_BLACK, "(%s)(%d) = index over error (%d)", __FILE__, __LINE__, aIndex);
	...
	if ( lpObj->Type != OBJ_USER )
	{
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(2, 3)), lpObj->AccountID, lpObj->Name);
		return FALSE;
	}
	...
	gObjViewportClose(lpObj);
	LogAdd(LOG_BLACK, lMsg.Get(MSGGET(2, 4)), lpObj->m_Index, lpObj->AccountID, lpObj->Name);
...
short gObjDel(int index)
{
	...
	if ( index < 0 || index > OBJMAX )
	{
		LogAdd(LOG_RED, "(%s)(%d) = index over error (%d)", __FILE__, __LINE__, index);
		return 0;
	}
	...
	if ( lpObj->Type == OBJ_USER )
	{
		...
		if ( UserConnectState >= PLAYER_LOGGED )
		{
			...
			{
				GJPUserClose(lpObj);
				LogAdd(LOG_BLACK, "(%d)logout-send : %s [%s]", index, lpObj->AccountID, lpObj->Ip_addr);
			}
			else
			{
				LogAdd(LOG_BLACK, "[MapServerMng] User Quit without JoinServer logout-send [%s][%s]",
		...
		LogAdd(LOG_BLACK, "(%d)logout : %s [%s]", index, lpObj->AccountID, lpObj->Ip_addr);
...
BOOL gObjCheckXYMapTile(LPOBJ lpObj, int iDbgName)
{
	...
	if ( lpObj->Class == 2 && lpObj->Level < 10 )
	{
		...
		{
			LogAdd(LOG_BLACK, lMsg.Get(MSGGET(2, 5)), lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y);
			PMSG_TELEPORT pMsg;
			pMsg.MoveNumber = 27;
	...
	if ( (attr&4) == 4 || (attr&8) == 8 )
	{
		...
		{
			LogAdd(LOG_BLACK, "[ CHECK POSITION ] DbgName[%d] [%s][%s] Map[%d]-(%d,%d) Invalid location causes to force to move",iDbgName,lpObj->AccountID, lpObj->Name, lpObj->MapNumber, lpObj->X, lpObj->Y);
			PMSG_TELEPORT pMsg;
			pMsg.MoveNumber = 17;
			gObjMoveGate(lpObj->m_Index, pMsg.MoveNumber);
...
BOOL gObjSetAccountLogin(int aIndex, char * szId, int aUserNumber, int aDBNumber, char* JoominNumber)
{
	...
	if ( gObj[aIndex].Connected != PLAYER_CONNECTED )
	{
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(2, 6)), aIndex, szId, aDBNumber, __FILE__, __LINE__);
		return 0;
	}

	if ( strcmp(gObj[aIndex].AccountID, szId) != 0 )
	{
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(2, 7)), szId);
		return 0;
	}
	...
	// ---
	if(gObj[aIndex].bReconnect)
	{
		gReconnect.SendGame(aIndex);
	}
	// ---
	return TRUE;
...
BOOL gObjUserIdConnectCheck(char* szId,int index)
{
	...
	{
		LogAdd(LOG_BLACK, "error : index error !! %s %d",__FILE__, __LINE__);
		return 0;
	}
...
{
	LogAdd(LOG_BLACK, "Experience : Map[%d]-(%d,%d) [%s][%s](%d) %u %d MonsterIndex : %d, EventType : %d",
	...
	{
		lpObj->LevelUpPoint += gServerInfo.m_Data.iLevelUp[0];
	}
	// ---
	else if(lpObj->Class == CLASS_KNIGHT)
	{
		lpObj->LevelUpPoint += gServerInfo.m_Data.iLevelUp[1];
	}
	// ---
	else if(lpObj->Class == CLASS_ELF)
	{
		lpObj->LevelUpPoint += gServerInfo.m_Data.iLevelUp[2];
	}
	// ---
	else if(lpObj->Class == CLASS_MAGUMSA)
	{
		lpObj->LevelUpPoint += gServerInfo.m_Data.iLevelUp[3];
	}
	// ---
	else if(lpObj->Class == CLASS_DARKLORD)
	{
		lpObj->LevelUpPoint += gServerInfo.m_Data.iLevelUp[4];
	}

	if ( lpObj->PlusStatQuestClear != false )
	{
		lpObj->LevelUpPoint += gServerInfo.m_Data.iLevelUp[5];
		LogAdd(LOG_BLACK, "[%s][%s] LevelUp PlusStatQuest Clear AddStat %d",
	...
	LogAdd(LOG_BLACK, lMsg.Get(MSGGET(2, 8)), lpObj->AccountID, lpObj->Name, lpObj->Level);
	...
	if(lpObj->Class == CLASS_ELF)
	{
		if(lpObj->Level >= g_SkillAdditionInfo.GetInfinityArrowUseLevel() && lpObj->ChangeUP == 1)
		{
			...
			{
				LogAdd(LOG_BLACK, "[%s][%s] Fail - Add Infinity Arrow Skill (Character Level : %d)(ChangeUp: %d)",
					lpObj->AccountID,lpObj->Name,lpObj->Level,lpObj->ChangeUP);
			}
		}
	}
	...
	if((GetTickCount()-lpObj->AutoSaveTime) > 30000)
	{
...
BOOL gObjLevelUpPointAdd(BYTE type, LPOBJ lpObj)
{
	...
	{
		LogAdd(LOG_RED,  lMsg.Get(MSGGET(2, 9)), lpObj->AccountID, lpObj->Name, __FILE__, __LINE__);
		return false;
	}
	// ---
	...
	{
		case 0:
			if(lpObj->Strength >= gServerInfo.m_Data.iMaxStatus)
			{
				return false;
			}
			else
			...
			// ---
		case 1:
			if(lpObj->Dexterity >= gServerInfo.m_Data.iMaxStatus)
			{
				return false;
			}
			else
			...
			// ---
		case 2:
			if(lpObj->Vitality >= gServerInfo.m_Data.iMaxStatus)
			{
				return false;
			}
			else
			...
			// ---
		case 3:
			if(lpObj->Energy >= gServerInfo.m_Data.iMaxStatus)
			{
				return false;
			}
			else
			...
			// ---
		case 4:
			if(lpObj->Leadership >= gServerInfo.m_Data.iMaxStatus)
			{
				return false;
			}
			else
...
void gObjPlayerKiller(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	if ( gObjGetRelationShip(lpObj, lpTargetObj) == 2 )
	{
		...
		{
			LogAdd(LOG_BLACK, "[U.System][Rival][Player Kill] ( Killer [%s][%s] / %s / U:%d / R:%d ) vs ( Victim [%s][%s] / %s / U:%d / R:%d )",
		...
		{
			LogAdd(LOG_BLACK, "[U.System][Rival][Player Kill][★ERROR : Can't find GuildInfo] (  [%s][%s] ) vs ( [%s][%s] )",
	...
	LogAdd(LOG_BLACK, "[%s][%s] PK Info Set (Killed Player) Origin(PkLevel:%d, PkCount:%d) Changed(PkLevel:%d, PkCount:%d)",
...
void gObjUserDie(LPOBJ lpObj, LPOBJ lpTargetObj)
{
	...
	if(DS_MAP_RANGE(lpObj->MapNumber))
	{
		if(lpTargetObj->Type == OBJ_MONSTER)
		{
			...
			if(mAttr != 0)
			{
				LogAdd(LOG_BLACK, "[DevilSquare] Dead In DevilSquare [%s][%s][%s]",lpObj->AccountID,lpObj->Name,mAttr->m_Name);
			}
		}
	...
	if(BC_MAP_RANGE(lpObj->MapNumber))
	{
		...
		{
			LogAdd(LOG_BLACK, "[Blood Castle] (%d) Try to drop Ultimate Weapon [%s][%s]",lpObj->MapNumber - (MAP_INDEX_BLOODCASTLE1-1),lpObj->AccountID,lpObj->Name);
			g_BloodCastle.SearchUserDropQuestItem(lpObj->m_Index);
		}
		else
		{
			LogAdd(LOG_BLACK, "[Blood Castle] (%d) Try to delete Ultimate Weapon [%s][%s]",lpObj->MapNumber - (MAP_INDEX_BLOODCASTLE1-1),lpObj->AccountID,lpObj->Name);
		...
		if(lpTargetObj->Type == OBJ_MONSTER)
		{
			...
			if(mAttr != 0)
			{
				LogAdd(LOG_BLACK, "[Blood Castle] (%d) Dead In Blood Castle, Killed by Monster [%s][%s][%s]",lpObj->MapNumber - (MAP_INDEX_BLOODCASTLE1-1),lpObj->AccountID,lpObj->Name,mAttr->m_Name);
				g_BloodCastle.SearchUserDropQuestItem(lpObj->m_Index);
			}
		}
		else if(lpTargetObj->Type == OBJ_USER)
		{
			LogAdd(LOG_BLACK, "[Blood Castle] (%d) Dead In Blood Castle, Killed by Other User [%s][%s][%s]",lpObj->MapNumber - (MAP_INDEX_BLOODCASTLE1-1),lpObj->AccountID,lpObj->Name,lpTargetObj->Name);
			g_BloodCastle.SearchUserDropQuestItem(lpObj->m_Index);
		}
		return;
	}
	...
	if(lpTargetObj->Type == OBJ_MONSTER)
	{
		LogAdd(LOG_BLACK, "[%s][%s] Killed [%s][%s] Pos:(%d,%d,%d)","Monster",lpTargetObj->Name,lpObj->AccountID,lpObj->Name,lpObj->MapNumber,lpObj->X,lpObj->Y);
	}
	else
	{
		LogAdd(LOG_BLACK, "[%s][%s] (PkLevel = %d) Killed [%s][%s] (PkLevel = %d) Pos:(%d,%d,%d)",lpTargetObj->AccountID,lpTargetObj->Name,lpTargetObj->m_PK_Level,lpObj->AccountID,lpObj->Name,lpObj->m_PK_Level,lpObj->MapNumber,lpObj->X,lpObj->Y);
		LogAdd(LOG_BLACK, "Killed by User State Victim:[%s][%s], Murderer:[%s][%s], Dead State (Duel:%d, GuildWar:%d, SeldDefense:%d)",lpObj->AccountID,lpObj->Name,lpTargetObj->AccountID,lpTargetObj->Name,!!IsOnDuel(lpTargetObj->m_Index,lpObj->m_Index),!!gObjTargetGuildWarCheck(lpObj,lpTargetObj),!!gObjIsSelfDefense(lpTargetObj,lpObj->m_Index));
	}
		...
		if(dropresult == 0)
		{
			...
			while(count-- != 0)
			{
				...
				if(lpObj->pInventory[number].IsItem()==1)
				{
					...
					{
						dropresult = 1;
						LogAdd(LOG_BLACK, "[%s][%s] User Die Item Drop, ItemName:%s",lpObj->AccountID,lpObj->Name,lpObj->pInventory[number].GetName());
						break;
					}
				}
			}
	...
	LogAdd(LOG_BLACK, "[%s][%s] Death reduces Experience %u - %d",lpObj->AccountID,lpObj->Name,subexp,decexp);
	...
	if(lpObj->Money > 0)
	{
		...
		LogAdd(LOG_BLACK, lMsg.Get(574),lpObj->AccountID,lpObj->Name,lpObj->Money,submoney);
	}
	...
	if(lpObj->WarehouseMoney > 0)
	{
		...
		LogAdd(LOG_BLACK, lMsg.Get(575),lpObj->AccountID,lpObj->Name,lpObj->WarehouseMoney,submoney);
	}
...
BOOL gObjWingSprite(LPOBJ lpObj)
{
	...
	if ( (Wing->m_Type >= ITEMGET(12,0) && Wing->m_Type <= ITEMGET(12,6)) || Wing->m_Type <= ITEMGET(13,30) || NEWWINGS(Wing->m_Type))
	{
...
void gObjSpriteDamage(LPOBJ lpObj, int damage)
{
	...
	else if ( sprite->m_Type == ITEMGET(13,4) )
	{
		...
		if ( sprite->m_DurabilitySmall > 1500 )
		{
			...
			if ( sprite->m_Durability < 1.0f )
			{
				...
				{
					LogAdd(LOG_BLACK, "[%s][%s][PetItemExpDown] [%s] Level:[%d]Exp:[%d]DecExp[%d]",
				...
				LogAdd(LOG_BLACK, "[%s][%s] 다크호스의뿔 Item is Broken because durability is exhausted [%d]",
	...
	if ( send_dur != FALSE )
	{
		...
		if ( sprite->m_Durability < 1.0f )
		{
			...
			LogAdd(LOG_BLACK, lMsg.Get(MSGGET(2, 10)), lpObj->AccountID, lpObj->Name,
...
void gObjChangeDurProc(LPOBJ lpObj)
{
	if(lpObj->pInventory[10].IsItem() == 1 && lpObj->pInventory[10].m_Type == ITEMGET(13,10))
	{
		...
		if(m_Durability > (int)lpObj->pInventory[10].m_Durability)
		{
			...
			if(dur == 0)
			{
				...
				LogAdd(LOG_BLACK, lMsg.Get(544),lpObj->AccountID,lpObj->Name,lpObj->pInventory[10].GetName(),lpObj->pInventory[10].m_Level,dur);
	...
	else if(lpObj->pInventory[11].IsItem() == 1 && lpObj->pInventory[11].m_Type == ITEMGET(13,10))
	{
		...
		if(m_Durability > (int)lpObj->pInventory[11].m_Durability)
		{
			...
			{
				gObjUseSkill.SkillChangeUse(lpObj->m_Index);
				LogAdd(LOG_BLACK, lMsg.Get(544),lpObj->AccountID,lpObj->Name,lpObj->pInventory[10].GetName(),lpObj->pInventory[11].m_Level,dur);
...
void gObjCheckSelfDefense(LPOBJ lpObj, int aTargetIndex)
{
	...
	LogAdd(LOG_BLACK, "[%s][%s] Set SelfDefence [%s][%s]",lpObj->AccountID,lpObj->Name,gObj[aTargetIndex].AccountID,gObj[aTargetIndex].Name);
}
	...
	for ( int n=0;n<MAX_SELF_DEFENSE;n++)
	{
		if ( lpObj->SelfDefense[n] >= 0 )
		{
			if ( GetTickCount() > lpObj->SelfDefenseTime[n] )
			{
				...
				LogAdd(LOG_BLACK, "[%s][%s] ReSet SelfDefence [%s][%s]",
					lpObj->AccountID, lpObj->Name,
...
int gObjMonsterExpSingle(LPOBJ lpObj, LPOBJ lpTargetObj, int dmg, int tot_dmg, bool& bSendExp)
{
	...
	exp = int(exp * gVip.GetExperience(lpObj->m_Vip));
...
void gObjExpParty(LPOBJ lpObj , LPOBJ lpTargetObj, int AttackDamage, int MSBFlag )
{
	...
	if(viewplayer > 1)
	{
		if(bApplaySetParty != 0)
		{
			if(viewplayer == 3)
			{
				viewpercent = gServerInfo.m_Data.iExpParty[0];
			}
			else if(viewplayer == 4)
			{
				viewpercent = gServerInfo.m_Data.iExpParty[1];
			}
			else if(viewplayer >= 5)
			{
				viewpercent = gServerInfo.m_Data.iExpParty[2];
			}
			else
			{
				viewpercent = gServerInfo.m_Data.iExpParty[3];
			}
		}
		else
		{
			if(viewplayer == 2)
			{
				viewpercent = gServerInfo.m_Data.iExpParty[4];
			}
			else if(viewplayer == 3)
			{
				viewpercent = gServerInfo.m_Data.iExpParty[5];
			}
			else if(viewplayer == 4)
			{
				viewpercent = gServerInfo.m_Data.iExpParty[6];
			}
			else if(viewplayer >= 5)
			{
				viewpercent = gServerInfo.m_Data.iExpParty[7];
			}
			else
			{
				viewpercent = gServerInfo.m_Data.iExpParty[8];
			}
		}
...
void gObjMonsterDieLifePlus(LPOBJ lpObj, LPOBJ lpTartObj)
{
	...
	GCReFillSend(lpObj->m_Index,lpObj->Life,255,0);
...
void gObjLifeCheck(LPOBJ lpTargetObj, LPOBJ lpObj, int AttackDamage, int DamageSendType, int MSBFlag, int MSBDamage, BYTE Skill)
{
	...
	if(lpTargetObj->Life <= 0 && lpTargetObj->Live != 0)
	{
				...
				case 303:
					{
						...
						if(gObjIsConnected(lpObj->m_Index))
						{
							...
							LogAdd(LOG_BLACK, "[CastleDeep Event] [%s][%s] Boss Monster Killed : %s",lpObj->AccountID,lpObj->Name,lpTargetObj->Name);
						}
						else
						{
							wsprintf(szMsg,lMsg.Get(MSGGET(12, 156)),lpTargetObj->Name);
							LogAdd(LOG_BLACK, "[CastleDeep Event] Boss Monster Killed : %s",lpTargetObj->Name);
						}
			...
			if(BC_MAP_RANGE(lpTargetObj->MapNumber))
			{
				if(lpTargetObj->Class == 131)
				{
					...
					if(TopHitUser != -1)
					{
						...
						LogAdd(LOG_BLACK, "[Blood Castle] (%d) Door Terminated -> %d [%s][%s]",lpObj->MapNumber - (MAP_INDEX_BLOODCASTLE1-1),g_BloodCastle.m_BridgeData[lpObj->MapNumber-MAP_INDEX_BLOODCASTLE1].m_iBC_BOSS_MONSTER_KILL_COUNT,gObj[TopHitUser].AccountID,gObj[TopHitUser].Name);
					}
					else
					{
						g_BloodCastle.SendNoticeMessage(lpObj->MapNumber - MAP_INDEX_BLOODCASTLE1,lMsg.Get(1169));
						LogAdd(LOG_BLACK, "[Blood Castle] (%d) Door Terminated -> %d)",lpObj->MapNumber - (MAP_INDEX_BLOODCASTLE1-1),g_BloodCastle.m_BridgeData[lpObj->MapNumber-MAP_INDEX_BLOODCASTLE1].m_iBC_BOSS_MONSTER_KILL_COUNT);
					}
				}
				...
				if(BC_STATUE_RANGE(lpTargetObj->Class - 132))
				{
					...
					if(TopHitUser != -1)
					{
						...
						LogAdd(LOG_BLACK, "[Blood Castle] (%d) a stone statue Of Saint Terminated -> %d [%s][%s]",lpObj->MapNumber - (MAP_INDEX_BLOODCASTLE1 - 1),g_BloodCastle.m_BridgeData[lpObj->MapNumber-MAP_INDEX_BLOODCASTLE1].m_iBC_BOSS_MONSTER_KILL_COUNT,gObj[TopHitUser].AccountID,gObj[TopHitUser].Name);
					}
					else
					{
						g_BloodCastle.SendNoticeMessage(lpObj->MapNumber - MAP_INDEX_BLOODCASTLE1,lMsg.Get(1170));
						LogAdd(LOG_BLACK, "[Blood Castle] (%d) a stone statue Of Saint Terminated -> %d",lpObj->MapNumber - (MAP_INDEX_BLOODCASTLE1-1),g_BloodCastle.m_BridgeData[lpObj->MapNumber-MAP_INDEX_BLOODCASTLE1].m_iBC_BOSS_MONSTER_KILL_COUNT);
					}
				}
		...
		else if(lpTargetObj->Type == OBJ_USER)
		{
			...
			if(CC_MAP_RANGE(lpTargetObj->MapNumber))
			{
				...
				LogAdd(LOG_BLACK, "[Chaos Castle] (%d) [%s][%s] User Dead In Chaos Castle : Killed by [%s][%s]",lpTargetObj->MapNumber - (MAP_INDEX_CHAOSCASTLE1 - 1),lpTargetObj->AccountID,lpTargetObj->Name,lpObj->AccountID,lpObj->Name);
			}
		...
		if(gObjDuelCheck(lpCallObj,lpTargetObj))
		{
			...
			if(lpObj->m_btDuelScore >= 10)
			{
				...
				LogAdd(LOG_BLACK, "[Duel] [%s][%s] Win Duel, Loser [%s][%s]",lpObj->AccountID,lpObj->Name,lpTargetObj->AccountID,lpTargetObj->Name);
			}
		}
	...
	else
	{
		...
		{
			GCDamageSend(lpCallObj->m_Index,lpTargetObj->m_Index,AttackDamage,MSBFlag,5);
		}
	}
}
...
BOOL gObjInventoryTrans(int aIndex)
{
	...
	{
		LogAdd(LOG_BLACK, "[%s][%s] error-L3 : pTransaction(%d) status error %s %d",
	...
	LogAdd(LOG_BLACK, "Trade Transaction (%s)", gObj[aIndex].Name);
...
int gObjInventoryCommit(int aIndex)
{
	if(!OBJMAX_RANGE(aIndex))
	{
		LogAdd(LOG_BLACK, "error : gObjInventoryCommit() - aIndex out of bound %s %d",__FILE__,__LINE__);
		return false;
	}

	if(gObj[aIndex].pTransaction != 1)
	{
		LogAdd(LOG_BLACK, "[%s][%s] error-L3 : pTransaction(%d) status2",gObj[aIndex].AccountID,gObj[aIndex].Name,gObj[aIndex].pTransaction);
		return false;
	}
	...
	LogAdd(LOG_BLACK, "Trade Commit (%s)",gObj[aIndex].Name);
...
int gObjInventoryRollback(int aIndex)
{
	...
	LogAdd(LOG_BLACK, "Trade Rollback (%s)",gObj[aIndex].Name);
...
void gObjInventoryItemSet(int aIndex, int itempos, BYTE set_byte)
{
	...
	{
		LogAdd(LOG_BLACK, lMsg.Get(527),__FILE__,__LINE__);
		return;
	}
...
bool gObjFixInventoryPointer(int aIndex)
{
	...
	{
		LogAdd(LOG_BLACK, "[Fix Inv.Ptr] [%s][%s] - disconnected",gObj[aIndex].AccountID,gObj[aIndex].Name);
		return false;
	}
	...
	if(gObj[aIndex].pInventory == gObj[aIndex].Inventory2)
	{
		if(gObj[aIndex].pTransaction == 1)
		{
			LogAdd(LOG_BLACK, "[Fix Inv.Ptr] [%s][%s] - Transaction == 1, Do not change Pointer",gObj[aIndex].AccountID,gObj[aIndex].Name);
			return false;
		}
		else
		{
			LogAdd(LOG_BLACK, "[Fix Inv.Ptr] [%s][%s] - Inventory Pointer was 2",gObj[aIndex].AccountID,gObj[aIndex].Name);
	...
	{
		LogAdd(LOG_BLACK, "[Fix Inv.Ptr] [%s][%s] - Inventory Pointer was Wrong",gObj[aIndex].AccountID,gObj[aIndex].Name);
	}
...
BYTE gObjInventoryInsertItem(int aIndex, CMapItem * item)
{
	...
	if(item->GetSize((int &)iwidth,(int &)iheight)==0)
	{
		LogAdd(LOG_BLACK, "error : Item doesn't exist %s %d",__FILE__,__LINE__);
		return -1;
	}
		...
		for(w = 0; w < 8; w++)
		{
			if(*(BYTE*)(gObj[aIndex].pInventoryMap+h*8+w) == 255)
			{
				...
				if(blank != 255)
				{
					...
					copyitem.Convert(item->m_Type,item->m_Option1,item->m_Option2,item->m_Option3,item->m_NewOption,item->m_SetOption, 0, CURRENT_DB_VERSION);
...
BOOL gObjSearchItemMinus(LPOBJ lpObj, int pos, int m_dur)
{
	...
	{
		LogAdd(LOG_BLACK, "error-L4 : %s %d",__FILE__,__LINE__);
		return 1;
	}
...
void gObjWarehouseItemBoxSet(int aIndex, int itempos, int xl, int yl, BYTE set_byte)
{
	...
	for(int y = 0; y < yl; y ++)
	{
		...
		for(int x = 0; x < xl; x++)
		{
			...
			{
				LogAdd(LOG_BLACK, "error : %d/%d  %s %d",xx,yy,__FILE__,__LINE__);
				return;
			}
		}
	}
...
BOOL gObjInventorySearchSerialNumber(LPOBJ lpObj, DWORD serial)
{
	...
	for(int n = 0; n < INVENTORY_SIZE; n++)
	{
		...
		if(s_num != 0 && s_num == serial)
		{
			LogAdd(LOG_BLACK, lMsg.Get(529),lpObj->AccountID,lpObj->Name,lpObj->pInventory[n].GetName(),n,s_num);
...
int s_num;
	...
	for(int n = 0; n < WAREHOUSE_SIZE; n++)
	{
		...
		if(s_num != 0 && s_num == sirial)
		{
			LogAdd(LOG_BLACK, lMsg.Get(530),lpObj->AccountID,lpObj->Name,lpObj->pWarehouse[n].GetName(),n,s_num);
			GCServerMsgStringSend(lMsg.Get(1102),lpObj->m_Index,1);
			gObjUserKill(lpObj->m_Index);
		}
	}
...
BYTE gObjInventoryMoveItem(int aIndex, unsigned char source, unsigned char target, int& durSsend, int& durTsend, unsigned char sFlag, unsigned char tFlag, unsigned char* siteminfo)
{
	...
	__try
	{
		if(sFlag == 2 || tFlag == 2)
		{
			...
			{
				LogAdd(LOG_BLACK, lMsg.Get(531),lpObj->AccountID,lpObj->Name);
				return -1;
			}
		...
		case 0:
			...
			{
				LogAdd(LOG_BLACK, "error-L1 : %s %d",__FILE__,__LINE__);
				return -1;
			}

			if(lpObj->pInventory[source].IsItem() == 0)
			{
				LogAdd(LOG_BLACK, lMsg.Get(532),lpObj->AccountID,lpObj->Name,__LINE__);
				return -1;
			}
		...
		case 4:
			...
			{
				LogAdd(LOG_BLACK, "[PShop] [%s][%s] PShop Item Move Request Failed : Already Trade With Other",gObj[aIndex].AccountID,gObj[aIndex].Name);
		...
		{
			LogAdd(LOG_RED, lMsg.Get(533),type,level,op1,op2,op3,sitem->m_Type,sitem->m_Level,sitem->m_Option1,sitem->m_Option2,sitem->m_Option3);
			return -1;
		}
		...
		if(gObjCheckSerial0ItemList(sitem))
		{
			...
			LogAdd(LOG_BLACK, "[ANTI-HACK][Serial 0 Item] [MoveItem] (%s)(%s) Item(%s) Pos(%d)",lpObj->AccountID,lpObj->Name,sitem->GetName(),source);
			return -1;
		}
		...
		if(sFlag == 0 && source < INVETORY_WEAR_SIZE)
		{
			if(lpObj->pInventory[source].IsItem() == 1)
			{
				...
				{
					gObjUseSkill.SkillChangeUse(aIndex);
					LogAdd(LOG_BLACK, lMsg.Get(534),gObj[aIndex].Name,lpObj->pInventory[source].m_Level);
				}
		...
		if(tFlag == 0 && target < INVETORY_WEAR_SIZE)
		{
			...
			{
				gObjUseSkill.SkillChangeUse(aIndex);
				LogAdd(LOG_BLACK, lMsg.Get(534),gObj[aIndex].Name,lpObj->pInventory[target].m_Level);
			}
...
BYTE gObjTradeInventoryMove(LPOBJ lpObj, BYTE source, BYTE target)
{
	...
	{
		MsgOutput(lpObj->m_Index,lMsg.Get(3354));
		LogAdd(LOG_BLACK, "[ANTI-HACK][Serial 0 Item] [Trade] (%s)(%s) Item(%s) Pos(%d)",lpObj->AccountID,lpObj->Name,lpObj->Trade[source].GetName(),source);
		return -1;
	}
	...
	if(target < INVETORY_WEAR_SIZE)
	{
		if(lpObj->pInventory[target].IsItem() == 1)
		{
			...
			{
				gObjUseSkill.SkillChangeUse(lpObj->m_Index);
				LogAdd(LOG_BLACK, lMsg.Get(534),lpObj->Name,lpObj->pInventory[target].m_Level);
			}
			if(lpObj->pInventory[target].m_Type == ITEMGET(13,39))
			{
				gObjUseSkill.SkillChangeUse(lpObj->m_Index);
...
BYTE gObjInventoryTradeMove(LPOBJ lpObj, BYTE source, BYTE target)
{
	...
	{
		MsgOutput(lpObj->m_Index,lMsg.Get(3354));
		LogAdd(LOG_BLACK, "[ANTI-HACK][Serial 0 Item] [Trade] (%s)(%s) Item(%s) Pos(%d)",lpObj->AccountID,lpObj->Name,lpObj->pInventory[source].GetName(),source);
		return -1;
	}
...
void gObjTradeCancel(int aIndex)
{
	...
	{
		LogAdd(LOG_BLACK, lMsg.Get(535),gObj[aIndex].AccountID,gObj[aIndex].Name);
		return;
	}
	if(gObj[aIndex].m_IfState.use != 1)
	{
		LogAdd(LOG_BLACK, lMsg.Get(536),gObj[aIndex].AccountID,gObj[aIndex].Name);
		return;
	}
	if(gObj[aIndex].m_IfState.type != 1)
	{
		LogAdd(LOG_BLACK, lMsg.Get(537),gObj[aIndex].AccountID,gObj[aIndex].Name);
		return;
	}
	gObjInventoryRollback(aIndex);
	...
	LogAdd(LOG_BLACK, lMsg.Get(514),gObj[aIndex].AccountID,gObj[aIndex].Name);
}
...
BYTE gObjTempInventoryInsertItem(LPOBJ lpObj, CItem item, BYTE * TempMap)
{
	...
	if(item.GetSize(iwidth,iheight) == 0)
	{
		LogAdd(LOG_BLACK, lMsg.Get(532),__FILE__,__LINE__,item.m_Type);
		return -1;
	}
	for(h = 0; h < 8; h++)
	{
...
BOOL TradeitemInventoryPut(int aIndex)
{
	...
	for(int n = 0; n < TRADE_BOX_SIZE; n++)
	{
		if(gObj[number].Trade[n].IsItem() == 1)
		{
			...
			{
				LogAdd(LOG_BLACK, "error : 이건 큰일나는일!!");
				return false;
			}
		}
	}
...
void gObjTradeOkButton(int aIndex)
{
	...
	{
		LogAdd(LOG_BLACK, lMsg.Get(539),gObj[aIndex].AccountID,gObj[aIndex].Name,gObj[number].TargetNumber,gObj[number].TargetNumber);
		return;
	}
	...
	if(result == 1)
	{
		...
		LogAdd(LOG_BLACK, lMsg.Get(540),gObj[aIndex].AccountID,gObj[aIndex].Name,gObj[number].AccountID,gObj[number].Name,gObj[aIndex].TradeMoney,gObj[number].TradeMoney);
	...
	if(gItemBlock.IsEnableToTrade(&gObj[number]) == 0)
	{
		MsgUser(aIndex, 1, "Item Bloqueado no Trade");
		MsgUser(number, 1, "Item Bloqueado no Trade");
		// ---
		result = 0;
		// ---
		gObjTradeCancel(aIndex);
...
void gObjMakePreviewCharSet(int aIndex)
{
	if(!OBJMAX_RANGE(aIndex))
	{
		LogAdd(LOG_RED, "[MAKE PREVIEW CHARSET] Error : %s %d", __FILE__, __LINE__);
		return;
	}
	// ---
	BYTE index;
	...
	int iLevelIndex;
	// ---
	iLevelIndex = CS_SET_SMALLLEVEL_RH(LevelSmallConvert(aIndex, 0));
	iLevelIndex |= CS_SET_SMALLLEVEL_LH(LevelSmallConvert(aIndex, 1));
	iLevelIndex |= CS_SET_SMALLLEVEL_HELMET(LevelSmallConvert(aIndex, 2));
	iLevelIndex |= CS_SET_SMALLLEVEL_ARMOR(LevelSmallConvert(aIndex, 3));
	iLevelIndex |= CS_SET_SMALLLEVEL_PANTS(LevelSmallConvert(aIndex, 4));
	iLevelIndex |= CS_SET_SMALLLEVEL_GLOVES(LevelSmallConvert(aIndex, 5));
	iLevelIndex |= CS_SET_SMALLLEVEL_BOOTS(LevelSmallConvert(aIndex, 6));
	// ---
	lpObj->CharSet[6] = CS_SET_SMALLLEVEL1(iLevelIndex);
	lpObj->CharSet[7] = CS_SET_SMALLLEVEL2(iLevelIndex);
	lpObj->CharSet[8] = CS_SET_SMALLLEVEL3(iLevelIndex);
	// ---
	if((lpObj->pInventory[7].m_Type >= ITEMGET(12,3) && lpObj->pInventory[7].m_Type <= ITEMGET(12,6)) || lpObj->pInventory[7].m_Type == ITEMGET(13,30))
	{
	...
	// ---
	if((lpObj->pInventory[7].m_Type >= ITEMGET(12, 100) && lpObj->pInventory[7].m_Type <= ITEMGET(12, 120)))
	{
		lpObj->CharSet[5] |= CS_SET_WING1(3);
		lpObj->CharSet[17] |= ((lpObj->pInventory[7].m_Type - 100) + 1 << 2);
	}
	// ---
	lpObj->CharSet[10] = 0;
	// ---
	if(lpObj->pInventory[2].IsExtItem() != 0)
	{
		lpObj->CharSet[10] = (char)0x80;
	}
	// ---
	if(lpObj->pInventory[3].IsExtItem() != 0)
	{
		lpObj->CharSet[10] |= 0x40;
	}
	// ---
	if(lpObj->pInventory[4].IsExtItem() != 0)
	{
		lpObj->CharSet[10] |= 0x20;
	}
	// ---
	if(lpObj->pInventory[5].IsExtItem() != 0)
	{
		lpObj->CharSet[10] |= 0x10;
	}
	// ---
	if(lpObj->pInventory[6].IsExtItem() != 0)
	{
		lpObj->CharSet[10] |= 0x8;
	}
	// ---
	if(lpObj->pInventory[0].IsExtItem() != 0)
	{
		lpObj->CharSet[10] |= 0x04;
	}
	// ---
	if(lpObj->pInventory[1].IsExtItem() != 0)
	{
	...
	lpObj->CharSet[11] = 0;
	// ---
	if(lpObj->pInventory[2].IsSetItem() != 0)
	{
		lpObj->CharSet[11] = (char)0x80;
	}
	// ---
	if(lpObj->pInventory[3].IsSetItem() != 0)
	{
		lpObj->CharSet[11] |= 0x40;
	}
	// ---
	if(lpObj->pInventory[4].IsSetItem() != 0)
	{
		lpObj->CharSet[11] |= 0x20;
	}
	// ---
	if(lpObj->pInventory[5].IsSetItem() != 0)
	{
		lpObj->CharSet[11] |= 0x10;
	}
	// ---
	if(lpObj->pInventory[6].IsSetItem() != 0)
	{
		lpObj->CharSet[11] |= 0x8;
	}
	// ---
	if(lpObj->pInventory[0].IsSetItem() != 0)
	{
		lpObj->CharSet[11] |= 0x04;
	}
	// ---
	if(lpObj->pInventory[1].IsSetItem() != 0)
	{
...
void PShop_ViewportListRegenarate(short aIndex)
{
	...
	for(int n = 0; n < MAX_VIEWPORT; n++)
	{
		if(lpObj->VpPlayer[n].state == 2 && lpObj->VpPlayer[n].type == OBJ_USER && gObj[lpObj->VpPlayer[n].number].m_bPShopOpen == 1)
		{
			...
			{
				LogAdd(LOG_BLACK, "[PShop] ERROR : lpObj->m_wVpPShopPlayerCount is OUT of BOUND : %d",lpObj->m_wVpPShopPlayerCount);
			}
		}
	}
	...
	{
		LogAdd(LOG_BLACK, "[PShop] ERROR : iVpOpenCount is OUT of BOUND: %d",iVpOpenCount);
	}
	...
	{
		LogAdd(LOG_BLACK, "[PShop] ERROR : iVpAddCount is OUT of BOUND: %d",iVpAddCount);
	}
			...
			if(lpObj->DieRegen == 2)
			{
				...
				if(BC_MAP_RANGE(lpObj->MapNumber) && lpObj->Type >= OBJ_MONSTER)
				{
					...
					if(BC_STATUE_RANGE(lpObj->Class - 132) && BC_MAP_RANGE(lpObj->MapNumber))
					{
						...
						LogAdd(LOG_BLACK, "[Blood Castle] (%d) Ultimate Weapon Dropped -> (%s)",
				...
				if(State != 0)
				{
					...
					if(lpObj->Type == OBJ_USER)
					{
						...
						if(m_Result != gGameServerCode )
						{
							if(m_Result == -1)
							{
								LogAdd(LOG_RED, "[MapServerMng] Map Server Move Fail : CheckMoveMapSvr() == -1 [%s][%s] (%d)",lpObj->AccountID,lpObj->Name,lpObj->m_Index);
								continue;
							}
							...
							LogAdd(LOG_BLACK, "[MapServerMng] Request to Move Map Server : (%d) - [%s][%s] (%d)",m_Result,lpObj->AccountID,lpObj->Name,lpObj->m_Index);
							continue;
						}
			...
			if(lpObj->Type == OBJ_USER)
			{
				if(lpObj->Connected >= PLAYER_LOGGED)
				{
					if(GetTickCount() - lpObj->ConnectCheckTime > 60000)
					{
						...
						LogAdd(LOG_BLACK, lMsg.Get(542),lpObj->m_Index,lpObj->AccountID,lpObj->Name,lpObj->Ip_addr);
					}
				}
				else
				{
					...
					{
						ResponErrorCloseClient(n);
						LogAdd(LOG_BLACK, lMsg.Get(543),lpObj->m_Index,lpObj->AccountID,lpObj->Name,lpObj->Ip_addr);
					}
				}
			}
		}
	...
	if( gTamaJJangEvent != 0)
	{
		...
		if(gTamaJJangTime <= 0)
		{
			if(gAppearTamaJJang==0)
			{
				...
				LogAdd(LOG_BLACK, "Appear TamaJJang : Keep Time (%d)",gTamaJJangTime);
			}
			else
			{
				gAppearTamaJJang = 0;
				...
				LogAdd(LOG_BLACK, "DisAppear TamaJJang : Keep Time (%d)",gTamaJJangTime);
			}
...
void gObjViewportListProtocol(short aIndex) //Identical
{
	if (gObj[aIndex].Connected < PLAYER_PLAYING)
	{
		return;
						...
						case 1:
...
#if(GS_CASTLE==1)
								//CastleSiege ViewPort of ViewSkillState
								if (g_CastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE)
								{
									...
									default:
										{
											...
											pViewportCreateChange.ViewSkillState &= 0xFF3FFFFF; //HermeX Fix
											lpTargetObj->m_ViewSkillState &= 0xFF3FFFFF; //HermeX Fix
										}
										break;
									}
								}
							...
							else
							{
								...
								if (g_CastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE)
								{
									...
									{
									case 1:
										pViewportCreate.ViewSkillState |= 0x40000; //HermeX Fix
										lpTargetObj->m_ViewSkillState |= 0x40000;
										break;
									case 2:
										{
											...
											pViewportCreate.ViewSkillState |= iViewSkillState; //HermeX Fix
											lpTargetObj->m_ViewSkillState |= iViewSkillState;
										}
										break;
									default:
										{
											pViewportCreate.ViewSkillState &= 0xFFF3FFFF; //HermeX Fix
											lpTargetObj->m_ViewSkillState &= 0xFFF3FFFF;
											pViewportCreate.ViewSkillState &= 0xFF3FFFFF; //HermeX Fix
											lpTargetObj->m_ViewSkillState &= 0xFF3FFFFF; //HermeX Fix
										}
										break;
									}
							...
							if (lpObj->Type == OBJ_USER)
							{
								...
								else
								{
...
#if(GS_CASTLE==1)
									if (lpTargetObj->Class == 278) //LifeStone ViewPort of ViewSkillState
									{
										if (g_CastleSiege.GetCastleState() == CASTLESIEGE_STATE_STARTSIEGE)
										{
											if (lpObj->m_btCsJoinSide == lpTargetObj->m_btCsJoinSide) //HermeX Fix
											{
												pMonsterViewportCreate.Type_HI |= 0x80; //HermeX Fix
											}
										}
										if (lpObj->lpGuild && lpTargetObj->lpGuild) //HermeX Fix
										{
											if (lpObj->lpGuild == lpTargetObj->lpGuild) //HermeX Fix
											{
												pMonsterViewportCreate.Type_HI |= 0x80; //HermeX Fix
										...
										pMonsterViewportCreate.Type_HI |= (lpTargetObj->m_btCreationState << 4 & 0x70); //HermeX Fix
									}
#endif
									...
									if (lpTargetObj->Class == 277)
									{
										...
										{
											pMonsterViewportCreate.ViewState &= 0xFFFDFFFF;
										}
										else
										{
...
void gObjSkillBeAttackProc(LPOBJ lpObj)
{
	...
	if(lpObj->m_PoisonBeattackCount > 0)
	{
		...
		if(lpObj->m_PoisonBeattackCount % 3 == 0)
		{
			...
			{
				gObjLifeCheck(lpObj,lpObj->lpAttackObj,ad,2,0,0,1);
			}
...
BOOL gObjMoveGate(int aIndex, int gt)
{
	...
	{
		LogAdd(LOG_BLACK, "Regen OK %s %d",__FILE__,__LINE__);
		x = lpObj->X;
		y = lpObj->Y;
		mapNumber = lpObj->MapNumber;
		dir = lpObj->Dir;
	...
	if(gGateC.IsGate(gt) == false)
	{
		...
		LogAdd(LOG_BLACK, lMsg.Get(545),gt,__FILE__,__LINE__);
	...
	if(gt >= 58 && gt <= 61)
	{
		...
		{
			LogAdd(LOG_BLACK, "[DevilSquare] [%s][%s] Not Used DevilSquare Invitation In DevilSquare",gObj[aIndex].AccountID,gObj[aIndex].Name);
			return false;
		}
		else
		{
	...
	if(lpObj->m_bPShopOpen == 1)
	{
		...
		if(bMove == 0)
		{
			LogAdd(LOG_BLACK, "[PSHOP ITEM CLONNING] Move Failed : %d (%s %s)",result,lpObj->AccountID,lpObj->Name);
	...
	if(!KALIMA_MAP_RANGE(mapNumber) && KALIMA_MAP_RANGE(lpObj->MapNumber))
	{
		...
		{
			LogAdd(LOG_BLACK, "[Kalima] [%s][%s] Kalima Gate Vanished - User MoveGate (SummonIndex:%d, EnterCount:%d)",lpObj->AccountID,lpObj->Name,iKalimaGateIndex,gObj[iKalimaGateIndex].m_cKalimaGateEnterCount);
		}
	}
		...
		if(gGateC.GetGate(gt,(short &)x,(short &)y,(BYTE &)mapNumber,(BYTE &)dir,(short &)level) != -1)
		{
			if(gObj[aIndex].MapNumber != mapNumber)
			{
				...
				if(sSvrCode != gGameServerCode)
				{
					...
					LogAdd(LOG_BLACK, "[MapServerMng] Request to Move Map Server : (%d) - [%s][%s] (%d)",sSvrCode,lpObj->AccountID,lpObj->Name,lpObj->m_Index);
					return false;
				}
			}
		}

		LogAdd(LOG_BLACK, "[MoveMap][%s][%s] : (Before:Map:%d X:%d Y:%d) -> (New Map:%d X:%d Y:%d)",lpObj->AccountID,lpObj->Name,lpObj->MapNumber,lpObj->X,lpObj->Y,mapNumber,x,y);
...
class CMagicInf* gObjGetMagic(LPOBJ lpObj, int mIndex)
{
	...
	{
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(2, 34)), __FILE__, __LINE__);
		return NULL;
	}
				...
				if(gObj[tObjNum].Live)
				{
					...
					if(dis <= MagicDistance)
					{
						if(gObjAttack(lpObj,&gObj[tObjNum],lpMagic,0,0,0,0) == 1)
						{
							...
							if(lOfs > 250)
							{
								LogAdd(LOG_BLACK, lMsg.Get(547),__FILE__,__LINE__);
								return false;
							}
						}
					}
...
int gObjMagicAdd(LPOBJ lpObj, BYTE Type, BYTE Index, BYTE Level, BYTE & SkillNumber)
{
	...
	if(reqleadership < 0)
	{
		LogAdd(LOG_BLACK, "error-L2: Skill Leadership not enough");
		return -1;
	}
	...
	for(n = 0; n < MAGIC_SIZE; n++)
	{
		if(lpObj->Magic[n].IsMagic() == 1)
		{
			...
			if(skill < 0)
			{
				LogAdd(LOG_BLACK, lMsg.Get(551),__FILE__,__LINE__);
				return -1;
			}

			if(lpObj->Magic[n].m_Skill == skill)
			{
				LogAdd(LOG_BLACK, lMsg.Get(552));
				return -1;
			}
		}
	}
	...
	for(n = 0; n < MAGIC_SIZE; n++)
	{
		if(lpObj->Magic[n].IsMagic() == 0)
		{
			...
			if(skill < 0)
			{
				LogAdd(LOG_BLACK, lMsg.Get(553),__FILE__,__LINE__);
				return -1;
			}
			else
			{
	...
	LogAdd(LOG_BLACK, lMsg.Get(554),__FILE__,__LINE__);
	return -1;
}
...
int  gObjMagicAdd(LPOBJ lpObj, BYTE aSkill, BYTE Level)
{
	...
	for(n = 0; n < MAGIC_SIZE; n++)
	{
		if(lpObj->Magic[n].IsMagic() == 1)
		{
			if(lpObj->Magic[n].m_Skill == aSkill)
			{
				LogAdd(LOG_BLACK, lMsg.Get(555),lpObj->Magic[n].m_Skill,aSkill);
				return -1;
			}
		}
	}
...
int gObjMonsterMagicAdd(LPOBJ lpObj, BYTE aSkill, BYTE Level)
{
	...
	for (int n=0; n<MAGIC_SIZE ; n++)
	{
		if ( lpObj->Magic[n].IsMagic() == FALSE )
		{
			...
			if ( skill < 0 )
			{
				LogAdd(LOG_BLACK, lMsg.Get( MSGGET(2, 41)),__FILE__, __LINE__);
				return -1;
			}
	...
	LogAdd(LOG_BLACK, lMsg.Get( MSGGET(2, 42)), __FILE__, __LINE__);
	return -1;
}
...
BOOL gObjMagicEnergyCheck(LPOBJ lpObj , BYTE aSkill)
{
	...
	if(gServerInfo.m_Data.iSkillNotLevel == 0)
	{
...
BOOL gObjItemLevelUp(LPOBJ lpObj, int source, int target)
{
	...
	LogAdd(LOG_BLACK, lMsg.Get(556),lpObj->AccountID,lpObj->Name,lpObj->pInventory[target].GetName(),lpObj->pInventory[target].m_Number,lpObj->pInventory[target].m_Level);
	...
	lpObj->pInventory[target].Convert(
			...
			lpObj->pInventory[target].m_SetOption, 0, CURRENT_DB_VERSION);

	LogAdd(LOG_BLACK, lMsg.Get(557),lpObj->AccountID,lpObj->Name,lpObj->pInventory[source].m_Number,
...
BOOL gObjItemRandomLevelUp(LPOBJ lpObj, int source, int target)
{
	...
	LogAdd(LOG_BLACK, lMsg.Get(559),lpObj->AccountID,lpObj->Name,lpObj->pInventory[source].m_Number,
			lpObj->pInventory[target].GetName(),lpObj->pInventory[target].m_Number,lpObj->pInventory[target].m_Level);
...
//fix +28 limitador
	...
	else
	{
		...
		{
			//2lvl wings (random opt)
			if (
				(lpObj->pInventory[target].m_Type >= ITEMGET(12,3) && lpObj->pInventory[target].m_Type <= ITEMGET(12,6)) || 
	...
	LogAdd(LOG_BLACK, "[%s][%s] After using Jewel of Life -> target:%d item:%s serial:%d opt:%d", lpObj->AccountID, lpObj->Name, lpObj->pInventory[source].m_Number, 
  lpObj->pInventory[target].GetName(),lpObj->pInventory[target].m_Number,lpObj->pInventory[target].m_Option3);
	...
	LogAdd(LOG_BLACK, "[%s][%s] JewelofLife Result [%d,%s,%d,%d,%d,%d]serial:[%d] dur:[%d] Ex:[%d,%d,%d,%d,%d,%d,%d]",
...
int gObjGuildWarItemGive(_GUILD_INFO_STRUCT * lpWinGuild, _GUILD_INFO_STRUCT * lpLoseGuild)
{
	...
	if(r_usercount < 1)
	{
		LogAdd(LOG_BLACK, lMsg.Get(560),__FILE__,__LINE__);
		return false;
	}
...
BOOL gObjMonsterCall(int aIndex, int MonsterType, int x, int y)
{
	...
	if(result >= 0)
	{
		...
		LogAdd(LOG_BLACK, "[Summon Monster] [%s][%s] Try to Summon Monster - Succeed (SummonIndex:%d)",gObj[aIndex].AccountID,gObj[aIndex].Name,result);
		return true;
	}
	return false;
}
...
{
	LogAdd(LOG_BLACK, "[Summon Monster] [%s][%s] Try to Kill Summoned Monster",gObj[aIndex].AccountID,gObj[aIndex].Name);
	...
	if(gObj[aIndex].m_RecallMon >= 0)
	{
		...
		{
			LogAdd(LOG_BLACK, "[Summon Monster] [%s][%s] Try to Kill Summoned Monster - Found Summoned Monster (SummonIndex:%d)",gObj[aIndex].AccountID,gObj[aIndex].Name,gObj[aIndex].m_RecallMon);
	...
	LogAdd(LOG_BLACK, "[Summon Monster] [%s][%s] Try to Kill Summoned Monster - Finished to Kill Summoned Monster (SummonIndex:%d)",gObj[aIndex].AccountID,gObj[aIndex].Name,callmon);
}
	...
	for(int n = 0; n < MAIN_INVENTORY_SIZE;n++)
	{
		if(lpObj->pInventory[n].IsItem() == 1)
		{
			if(lpObj->pInventory[n].m_Type == ITEMGET(14,21))
			{
				...
				LogAdd(LOG_BLACK, "[EventChip] [%s][%s] Delete Rena (%d)",lpObj->AccountID,lpObj->Name,n);
				count++;
	...
	for(int n = 0; n < MAIN_INVENTORY_SIZE; n++)
	{
		if(lpObj->pInventory[n].IsItem() == 1)
		{
			if(lpObj->pInventory[n].m_Type == itemtype * 512 + itemindex)
			{
				gObjInventoryDeleteItem(aIndex,n);
				GCInventoryItemDeleteSend(aIndex,n,1);
				LogAdd(LOG_BLACK, "[DeleteItem] [%s][%s] Delete (%d,%d)(%d)",lpObj->AccountID,lpObj->Name,itemtype,itemindex,n);
				delcount++;
...
void gObjUseCircle(int aIndex, int pos)
{
	...
	LogAdd(LOG_BLACK, "[StatUp] [%s][%s] Use StatUpItem Level:%d Pos:%d serial:%d",gObj[aIndex].AccountID,gObj[aIndex].Name,level,pos,iSerial);
...
//+28???
		pResult.result |= level << 4; //pResult.result |= level * 16; //old
		...
		LogAdd(LOG_BLACK, "[StatUp] [%s][%s] Success [%d][%d] %d",gObj[aIndex].AccountID,gObj[aIndex].Name,level,incvalue,iSerial);
	}
	else
	{
		pResult.result |= 0x40;
		...
		DataSend(aIndex,(unsigned char *)&pResult,pResult.h.size);
		LogAdd(LOG_BLACK, "[StatUp] [%s][%s] Fail [%d] %d",gObj[aIndex].AccountID,gObj[aIndex].Name,level,iSerial);
		return;
	}
	...
	case 1:
		...
		GCReFillSend(gObj[aIndex].m_Index,gObj[aIndex].MaxLife + gObj[aIndex].AddLife,0xFE,0);
		break;
	case 0:
...
void gObjUsePlusStatFruit(int aIndex,int pos)
{
	...
	LogAdd(LOG_BLACK, "[StatUp] [%s][%s] Use StatUpItem Level:%d Pos:%d serial:%d",
	...
	if(bSuccess != 0)
	{
		...
		LogAdd(LOG_BLACK, "[StatUp] [%s][%s] Success [%d][%d] %d",
	...
	else
	{
		...
		LogAdd(LOG_BLACK, "[StatUp] [%s][%s] Fail [%d] %d",
...
void gObjUseMinusStatFruit(int aIndex, int pos)
{
	...
	LogAdd(LOG_BLACK, "[StatDown] [%s][%s] Use StatDownItem Level:%d Pos:%d serial:%d",
		gObj[aIndex].AccountID,gObj[aIndex].Name,iItemLevel,pos,iItemSerial);
	...
	if(bSuccess != 0)
	{
		...
		DataSend(aIndex,(unsigned char *)&pResult,pResult.h.size);
		LogAdd(LOG_BLACK, "[StatDown] [%s][%s] Success [%d][%d] %d",
	...
	else
	{
		...
		pResult.btStatValue = 0;
		DataSend(aIndex,(unsigned char *)&pResult,pResult.h.size);
		LogAdd(LOG_BLACK, "[StatDown] [%s][%s] Fail [%d] %d",
			gObj[aIndex].AccountID,gObj[aIndex].Name,iItemLevel,iItemSerial);
		return;
	}
	...
	LogAdd(LOG_BLACK, "[StatDown] [%s][%s] MinusStat[%d] -> LevelUpPoint Old(%d)/New(%d)  FruitPoint Old(%d)/New(%d)",
...
/*void gObjNProtectGGCheckSum(int aIndex)
{
	...
	if(lpObj->m_bSentGGAuth == 0)
	{
		if((GetTickCount() - lpObj->m_NPggCheckSumSendTime) > 300000)
		{
			...
			if(dwGGErrCode != 0)
			{
				LogAdd(LOG_BLACK, "[NPgg] Failed Send Checksum %x [%s][%s] errorcode: %d",dwGGErrCode,lpObj->AccountID,lpObj->Name,dwGGErrCode);
				lpObj->m_bSentGGAuth = 0;
				gObjCloseSet(aIndex,0);
			}
			else
			{
				LogAdd(LOG_BLACK, "[NPgg] Send Checksum %x [%s][%s]",dwGGErrCode,lpObj->AccountID,lpObj->Name);
	...
	else
	{
		...
		{
			LogAdd(LOG_BLACK, "[NPgg] Checksum Time Out [%s][%s]",lpObj->AccountID,lpObj->Name);
...
void MakeRandomSetItem(int aIndex)
{
	...
	LogAdd(LOG_BLACK, "[Chaos Castle] [%s][%s] Winner Set Item itemnum:[%d] skill:[%d] luck:[%d] option:[%d] SetOption:[%d]",gObj[aIndex].AccountID,gObj[aIndex].Name,
...
void MakeRewardSetItem(int aIndex, BYTE cDropX, BYTE cDropY, int iRewardType, int iMapnumber)
{
	...
	if(iRewardType == 1)
	{
		LogAdd(LOG_BLACK, "[★☆Reward][KUNDUN] [%s][%s] Set Item itemnum:[%d] skill:[%d] luck:[%d] option:[%d] SetOption:[%d]",
			gObj[aIndex].AccountID,gObj[aIndex].Name,itemnum,Option1,Option2,Option3,SetOption);
	}
	else
	{
		LogAdd(LOG_BLACK, "[Reward][Etc] [%s][%s] Set Item itemnum:[%d] skill:[%d] luck:[%d] option:[%d] SetOption:[%d]",
			gObj[aIndex].AccountID,gObj[aIndex].Name,itemnum,Option1,Option2,Option3,SetOption);
	}		
}
...
void gObjSetExpPetItem(int aIndex, int exp)
{
	...
	if(lpObj->pInventory[8].m_Type == ITEMGET(13,4) && lpObj->pInventory[1].m_Type == ITEMGET(13,5))
	{
		...
		if(lpObj->pInventory[1].AddPetItemExp(addexp))
		{
			...
			{
				LogAdd(LOG_BLACK, "[%s][%s][PetItemLevelUp] [%s] Level:[%d]Exp:[%d]AddExp:[%d]",
...
int i;
	...
	for(i = 0; i < INVENTORY_SIZE; i++)
	{
		...
		if(iItemSerial == 0)
		{
			LogAdd(LOG_BLACK, "[ANTI-HACK][Serial 0 Item] (%s)(%s) Item(%s) Pos(%d)",
...
BOOL gCheckSkillDistance(int aIndex, int aTargetIndex, int iSkillNum)
{
	...
	{
		LogAdd(LOG_BLACK, "[SKILL DISTANCE CHECK] [%s][%s] Invalid Skill attacker(%d, %d), defender(%d, %d), skillnum:%d skilldistance:%d",
			lpObj->AccountID, lpObj->Name,
			lpObj->X, lpObj->Y, 
...
void gObjSaveChaosBoxItemList(LPOBJ lpObj)
{
	...
	for(int n = 0; n < CHAOS_BOX_SIZE;n++)
	{
		if(lpObj->pChaosBox[n].IsItem() == 1)
		{
			...
			LogAdd(LOG_BLACK, "[ChaosBoxItemList][Lost ItemList] [%s][%s] [%d,%s,%d,%d,%d,%d] Serial:[%d][%d] Ex:[%d,%d,%d,%d,%d,%d,%d] Set:[%d]",
