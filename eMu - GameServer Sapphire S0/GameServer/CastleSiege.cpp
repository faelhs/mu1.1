...
int CCastleSiege::DelGemOfDefend(int iIndex, int iNeedGemOfDefend)
{
	...
	for (int x = 0; x < INVENTORY_SIZE; x++ )
	{
		if ( gObj[iIndex].pInventory[x].IsItem() == TRUE )
		{
			if ( gObj[iIndex].pInventory[x].m_Type == ITEMGET(14,31) )
			{
				LogAdd(LOG_BLACK, "[CastleSiege] [%s][%s] Deleting Gem of Defend (GUILD:%s) - Serial:%u", gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].GuildName, gObj[iIndex].pInventory[x].m_Number);
...
void CCastleSiege::SaveCsTotalGuildInfo()
{
	...
	for( int iSize = this->m_mapCsTotalGuildInfo.size(); it2 != this->m_mapCsTotalGuildInfo.end(); it2++ )
	{
		...
		LogAdd(LOG_BLACK, "[CastleSiege] Siege Guild List Save - GNAME:%s, CSGUID:%d, INVOLVED:%d", (char*)&szGuildName[0], GuildData.m_iCsGuildID, GuildData.m_bGuildInvolved);
	}
...
void CCastleSiege::SetCsTotalGuildInfo(CSP_CSLOADTOTALGUILDINFO* lpMsg, int iCOUNT)
{
	...
	for( ; it2 != this->m_mapCsTotalGuildInfo.end(); it2++ )
	{
		...
		LogAdd(LOG_BLACK, "[CastleSiege] Siege Guild List Load - GNAME:%s, CSGUID:%d, INVOLVED:%d", &GuildName[0], GuildData.m_iCsGuildID, GuildData.m_bGuildInvolved);
	}
...
int CCastleSiege::CheckCastleSiegeResult()
{
	...
	LogAdd(LOG_BLACK, "[CastleSiege] Castle Final Winner Guild : %s",(strcmp(this->m_szCastleOwnerGuild,"") == FALSE) ? "NONE" : this->m_szCastleOwnerGuild);
		...
		case 8:
			{
				...
				else
				{
					...
					if ( iResult == 1 )
					{
						...
						{
							LogAdd(LOG_BLACK, "[CastleSiege] Castle Winner Has been Changed - GM (GUILD:%s)", szGuildName);
