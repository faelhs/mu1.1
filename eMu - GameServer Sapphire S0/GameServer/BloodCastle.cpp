...
BOOL CBloodCastle::Load(LPSTR filename)
{
	Tokenizer          token;
	TokenizerGroup     group;
	TokenizerSection   section;
	int BridgeCount;
	signed int iBridgeNum;
	int null;
	...
	if(group.GetSection(0, section))
	{
		...
		{
			this->m_iBC_TIME_MIN_OPEN = section.Rows[i].GetInt(0);
			this->m_iBC_TIME_MIN_PLAY = section.Rows[i].GetInt(1);
			this->m_iBC_TIME_MIN_REST = section.Rows[i].GetInt(2);
		}
	}
	if(group.GetSection(1, section))
	{
		for(i = 0; i < section.RowCount; i++)
		{
			this->m_iBC_NORMAL_ITEM_DROP = section.Rows[i].GetInt(0);
			this->m_iBC_EXCEL_ITEM_DROP  = section.Rows[i].GetInt(1);
			this->m_iBC_MONSTER_REGEN    = section.Rows[i].GetInt(2);
		}
	}
	if(group.GetSection(2, section))
	{
		for(i = 0; i < section.RowCount; i++)
		{
			iBridgeNum = section.Rows[i].GetInt(0);

			if ( BC_BRIDGE_RANGE(iBridgeNum) != FALSE )
			{
				this->m_BridgeData[iBridgeNum].m_iCastleStatueHealth = section.Rows[i].GetInt(1);
				this->m_BridgeData[iBridgeNum].m_iCastleDoorHealth   = section.Rows[i].GetInt(2);
				iBridgeNum++;
			}
			else
			{
				null = section.Rows[i].GetInt(1);
				null = section.Rows[i].GetInt(2);
			}
		}
	}

	LogAdd(LOG_BLACK, "%s file load!", filename);
...
void CBloodCastle::CheckSync(int iBridgeIndex)
{
	...
	LogAdd(LOG_BLACK, "[Blood Castle] (%d) Sync Open Time. [%d] min remain",
...
void CBloodCastle::ProcState_Playing(int iBridgeIndex)
{
	...
	if ( iTICK_MSEC >= 1000 )
	{
		...
		if ( this->m_BridgeData[iBridgeIndex].m_bBC_MONSTER_KILL_COMPLETE != false	)
		{
			if ( this->m_BridgeData[iBridgeIndex].m_dwBC_TICK_DOOR_OPEN != -1 )
			{
				if ( GetTickCount() > this->m_BridgeData[iBridgeIndex].m_dwBC_TICK_DOOR_OPEN )
				{
					...
					LogAdd(LOG_BLACK, "[Blood Castle] (%d) Bridge Change Bridge Attribute -> Open", iBridgeIndex+1);
		...
		if ( this->m_BridgeData[iBridgeIndex].m_iBC_REMAIN_MSEC <= ((this->m_iBC_TIME_MIN_PLAY*60-60)*1000) && this->m_BridgeData[iBridgeIndex].m_bBC_PLAY_START == false )
		{
			...
			LogAdd(LOG_BLACK, "[Blood Castle] (%d) Blood Castle Quest Start", iBridgeIndex+1);
		}
		...
		if ( this->CheckEveryUserDie(iBridgeIndex) != false )
		{
			...
			LogAdd(LOG_BLACK, "[Blood Castle] (%d) Blood Castle Quest Fail Result -> Destroy Castle Door [%s][%s]",
			...
			LogAdd(LOG_BLACK, "[Blood Castle] (%d) Blood Castle Quest Fail Result -> Destroy Saint Status [%s][%s]",
				iBridgeIndex+1, this->m_BridgeData[iBridgeIndex].m_szKill_Door_AccountID,
				this->m_BridgeData[iBridgeIndex].m_szKill_Status_CharName);
			this->GiveReward_Fail(iBridgeIndex);
			this->SetState(iBridgeIndex, BC_STATE_CLOSED);
			LogAdd(LOG_BLACK, "[Blood Castle] (%d) Blood Castle Quest Failed -> Every User Out", iBridgeIndex+1);
		}
		else
		{
		...
		if ( this->CheckWinnerExist(iBridgeIndex) == true )
		{
			if ( this->CheckWinnerValid(iBridgeIndex) == true )
			{
				if ( this->CheckWinnerPartyComplete(iBridgeIndex) == true )
				{
					...
					LogAdd(LOG_BLACK, "[Blood Castle] (%d) CheckWinnerPartyComplete(iBridgeIndex) == true",
						iBridgeIndex+1);
			...
			else
			{
				...
				LogAdd(LOG_BLACK, "[Blood Castle] (%d) CheckWinnerValid(iBridgeIndex) == false",
					iBridgeIndex+1);
	...
	if ( this->m_BridgeData[iBridgeIndex].m_iBC_REMAIN_MSEC <= 0 )
	{
		if ( this->m_BridgeData[iBridgeIndex].m_bBC_REWARDED == false )
		{
			...
			LogAdd(LOG_BLACK, "[Blood Castle] (%d) Blood Castle Quest Failed -> Time Out", iBridgeIndex+1);
...
void CBloodCastle::SetState_Closed(int iBridgeIndex)
{
	...
	LogAdd(LOG_BLACK, "[Blood Castle] (%d) SetState CLOSED", iBridgeIndex+1);
}
...
void CBloodCastle::SetState_Playing(int iBridgeIndex)
{
	...
	LogAdd(LOG_BLACK, "[Blood Castle] (%d) SetState PLAYING", iBridgeIndex+1);
}
...
void CBloodCastle::SetState_PlayEnd(int iBridgeIndex)
{
	...
	LogAdd(LOG_BLACK, "[Blood Castle] (%d) SetState PLAYEND", iBridgeIndex+1);
...
bool CBloodCastle::BloodCastleChaosMix(int iIndex, int iLEVEL)
{
	...
	LogAdd(LOG_BLACK, "[Blood Castle] Mix Chaos Mix Start (Account:%s, Name:%s, Level:%d)",
		gObj[iIndex].AccountID, gObj[iIndex].Name, iLEVEL);
	LogChaosItem(&gObj[iIndex], "Mix");
	...
	if ( iMIX_SUCCESS_RATE < 0 || iMIX_SUCCESS_RATE > 100 )
	{
		...
		LogAdd(LOG_BLACK, "[Blood Castle]Mix Chaos Mix Failed - MixRate Out of Bound (Account:%s, Name:%s, Level:%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, iLEVEL);
	...
	{
		DataSend(iIndex, (UCHAR*)&pMsg, pMsg.h.size);
		LogAdd(LOG_BLACK, "[Blood Castle] Åõ¸í¸ÁÅä Mix Chaos Mix Failed - MixMoney < 0 (Account:%s, Name:%s, Level:%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, iLEVEL);
	...
	if ( (gObj[iIndex].Money - iMIX_NEED_MONEY) < 0 )	// Not enoght zen
	{
		...
		DataSend(iIndex, (UCHAR*)&pMsg, pMsg.h.size);
		LogAdd(LOG_BLACK, "[Blood Castle] Åõ¸í¸ÁÅä Mix Chaos Mix Failed - Not Enough Money (Account:%s, Name:%s, Level:%d)",
			gObj[iIndex].AccountID, gObj[iIndex].Name, iLEVEL);
	...
	if ( (rand()%100) < iMIX_SUCCESS_RATE )	// Siccess
	{
		...
		LogAdd(LOG_BLACK, "[Mix] [%s][%s] CBMix Success %d Money : %d-%d",
	...
	else	// Failure
	{
		...
		DataSend(iIndex, (UCHAR*)&pMsg, pMsg.h.size);
		LogAdd(LOG_BLACK, "[Mix] [%s][%s] CBMix Fail %d Money : %d-%d",
			gObj[iIndex].AccountID, gObj[iIndex].Name, iMIX_SUCCESS_RATE, gObj[iIndex].Money, iMIX_NEED_MONEY);
...
void CBloodCastle::SetSaintStatue(int iBridgeIndex)
{
	...
	if ( nSaintStatuePosNum != -1 )
	{
		...
		{
			LogAdd(LOG_BLACK, "[Blood Castle][Bug Tracer] (%d) It's not Saint Statue( Map:%d, BloodCastleIndex:%d, Type:%d )",
...
int  CBloodCastle::LevelUp(int iIndex, int iAddExp, int iEventType)
{
	...
	else
	{
		...
		LogAdd(LOG_BLACK, lMsg.Get(MSGGET(2, 8)), gObj[iIndex].AccountID, gObj[iIndex].Name, gObj[iIndex].Level);
	}
...
void CBloodCastle::ReleaseCastleBridge(int iBridgeIndex)
{
	...
	for ( int i= ::g_btCastleBridgeMapXY[iBridgeIndex].btStartX;i<= ::g_btCastleBridgeMapXY[iBridgeIndex].btEndX;i++)
	{
		for ( int j= ::g_btCastleBridgeMapXY[iBridgeIndex].btStartY;j<= ::g_btCastleBridgeMapXY[iBridgeIndex].btEndY;j++)
		{
			...
			LogAdd(LOG_BLACK, "[Blood Castle][Bug Tracer] ReleaseCastleBridge-> %d", iBridgeIndex+1);
		}
	}
}
	...
	for ( int i=OBJ_MAXMONSTER;i<OBJMAX;i++)
	{
		if ( gObj[i].MapNumber == (iBridgeIndex + MAP_INDEX_BLOODCASTLE1) )
		{
			if ( gObj[i].Connected > PLAYER_LOGGED )
			{
				...
				LogAdd(LOG_BLACK, "[Blood Castle][Bug Tracer] SendCastleBridgeBlockInfo-> Bridge:%d/Live:%d, (%d,%d,%d,%d)",
		...
		if ( gObj[iIndex].pInventory[x].IsItem() == TRUE )
		{
			if ( gObj[iIndex].pInventory[x].m_Type == ITEMGET(13,19) )
			{
				...
				if ( BC_WEAPON_LEVEL_RANGE(iLEVEL) != FALSE )
				{
					...
					LogAdd(LOG_BLACK, "[Blood Castle] Force (Account:%s, Name:%s) to Delete Angel King's Weapon (%d) [Serial:%d]",
			...
			if ( gObj[iIndex].pInventory[x].m_Type == ITEMGET(13,19) )
			{
				...
				if ( BC_WEAPON_LEVEL_RANGE(iLEVEL) != FALSE )
				{
					...
					if ( BC_MAP_RANGE(gObj[iIndex].MapNumber) != FALSE )
					{
						...
						LogAdd(LOG_BLACK, "[Blood Castle] Force (Account:%s, Name:%s) to Drop Angel King's Weapon (%d) [Serial:%d-%d]",
					...
					{
						LogAdd(LOG_BLACK, "[Blood Castle] Force (Account:%s, Name:%s) to Drop Angel King's Weapon (%d) [Serial:%d]",
							gObj[iIndex].AccountID, gObj[iIndex].Name, iLEVEL,
							gObj[iIndex].pInventory[x].m_Number);
					}
				}
...
void CBloodCastle::GiveReward_Win(int iIndex, int iBridgeIndex)
{
	...
	LogAdd(LOG_BLACK, "[Blood Castle] (%d) Winner Party Point - Alive Party Member : %d, Alive Party Point : %d",
	...
	if ( this->m_BridgeData[iBridgeIndex].m_iMISSION_SUCCESS != -1 )
	{
		...
		LogAdd(LOG_BLACK, "[Blood Castle] (%d) Quest Complete - All Alive User's Total EXP:%d",
		...
		for ( int i=0;i<MAX_BLOOD_CASTLE_SUB_BRIDGE;i++)
		{
			...
			switch ( this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iUserState  )
			{
				case 0:
					...
					LogAdd(LOG_BLACK, "[Blood Castle] (%d) Quest Complete Rewarded (Account:%s, Name:%s [State:%d], Exp:%d, ExtExp:%d, Zen:%d, Score:%d, LeftTime:%d, MapNumber:%d)",
...
void CBloodCastle::GiveReward_Fail(int iBridgeIndex)
{
	...
	if ( iUserWhoGotUltimateWeapon != -1 )
	{
		...
		{
			LogAdd(LOG_BLACK, "[Blood Castle] (%d) (Account:%s, Name:%s) got Ultimate Weapon (%d) but Failed - Time Out",
	...
	for ( int i=0;i<MAX_BLOOD_CASTLE_SUB_BRIDGE;i++)
	{
		...
		LogAdd(LOG_BLACK, "[Blood Castle] (%d) Quest Fail Rewarded (Account:%s, Name:%s, Score:%d, ExtEXP:%d, LeftTime:%d)",
			iBridgeIndex+1, gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].AccountID,
			gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].Name,
		...
		if ( gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].m_cBloodCastleIndex != -1 && gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].m_cBloodCastleSubIndex != -1 )
		{
			...
			LogAdd(LOG_BLACK, "[Blood Castle] (%d) Send User Quest Fail Message (Account:%s, Name:%s [State:%d], MapNumber:%d)",
				iBridgeIndex+1, gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].AccountID,
				gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].Name, 
...
void CBloodCastle::SendRewardScore(int iIndex, int iSCORE, int iLeftTime, int iAlivePartyCount)
{
	...
	if( ::IsDevilSquareEventConnected == FALSE && ::DevilSquareEventConnect == FALSE )
	{
		...
		if ( GMRankingServerConnect(gDevilSquareEventServerIp, WM_GM_RANKING_CLIENT_MSG_PROC) == FALSE )
		{
			...
			LogAdd(LOG_BLACK, "Can not connect Ranking Server");
...
void CBloodCastle::SetMonsterKillCount(int iBridgeIndex)
{
	...
	LogAdd(LOG_BLACK, "[Blood Castle] (%d) Monster Kill Count Set - Monster:%d, Current Monster:%d, USER TOT:%d, LIVE:%d, DEAD:%d",
...
bool CBloodCastle::CheckMonsterKillCount(int iBridgeIndex)
{
	...
	{
		LogAdd(LOG_BLACK, "[Blood Castle][Bug Tracer] (%d) All Monster Kill Success(Max Kill Count:%d / Kill Count:%d)",
			iBridgeIndex+1, this->m_BridgeData[iBridgeIndex].m_iBC_MONSTER_MAX_COUNT,
			this->m_BridgeData[iBridgeIndex].m_iBC_MONSTER_KILL_COUNT);
...
bool CBloodCastle::CheckBossKillCount(int iBridgeIndex)
{
	...
	{
		LogAdd(LOG_BLACK, "[Blood Castle][Bug Tracer] (%d) All Boss Monster Kill Success(Max Boss Kill Count:%d / Boss Kill Count:%d)",
...
bool CBloodCastle::CheckEveryUserDie(int iBridgeIndex)
{
	...
	for ( int i=0;i<MAX_BLOOD_CASTLE_SUB_BRIDGE;i++)
	{
		if ( this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex != -1 )
		{
			...
			{
				LogAdd(LOG_BLACK, "error-L3 : [Blood Castle] (%d) User Out of Bound UserIndex:%d, SubIndex:%d",
			...
			if ( gObj[this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex].MapNumber == (iBridgeIndex + MAP_INDEX_BLOODCASTLE1) )
			{
				...
				{
					LogAdd(LOG_BLACK, "error-L3 : [Blood Castle] (%d) Connection Closed UserIndex:%d, SubIndex:%d",
						iBridgeIndex+1, this->m_BridgeData[iBridgeIndex].m_UserData[i].m_iIndex, i);
				}
			}
			else
...
bool CBloodCastle::CheckAngelKingExist(int iBridgeIndex)
{
	...
	{
		LogAdd(LOG_BLACK, "[Blood Castle] (%d) Angel King's Monster Position Doesn't Exist",
			iBridgeIndex+1);
	...
	if ( bRET_VAL == false )
	{
		...
		{
			LogAdd(LOG_BLACK, "[Blood Castle] (%d) Angel King - gObjAdd() failed (no space to add monster, result:%d)",
...
BOOL CBloodCastle::DropItemDirectly(int iBridgeIndex, int iIndex, int iItemType, int iItemPos)
{
	...
	{
		LogAdd(LOG_BLACK, "[Blood Castle] [%s][%s] DropItemDirectly() failed (iBridgeIndex wrong - %d)",
	...
	if ( MapC[map_num].ItemDrop(type, level, dur, gObj[iIndex].X, gObj[iIndex].Y,Option1, Option2, Option3, NOption, SOption, item_number, aAntiLootIndex, PetLevel, PetExp, 0) == TRUE )
	{
...
bool CBloodCastle::CheckWinnerValid(int iBridgeIndex)
{
	if ( !BC_BRIDGE_RANGE(iBridgeIndex))
	{
		LogAdd(LOG_BLACK, "[Blood Castle] (%d) CBloodCastle::CheckWinnerValid() - !CHECK_LIMIT(iBridgeIndex, MAX_BLOODCASTLE_BRIDGE_COUNT)",
			iBridgeIndex+1);
	...
	if ( this->m_BridgeData[iBridgeIndex].m_iBC_COMPLETE_USER_INDEX == -1 )
	{
		LogAdd(LOG_BLACK, "[Blood Castle] (%d) CBloodCastle::CheckWinnerValid() - m_BridgeData[iBridgeIndex].m_iBC_COMPLETE_USER_INDEX == -1",
			iBridgeIndex+1);
	...
	{
		LogAdd(LOG_BLACK, "[Blood Castle] (%d) CBloodCastle::CheckWinnerValid() - !gObjIsConnected(m_BridgeData[iBridgeIndex].m_iBC_COMPLETE_USER_INDEX)",
			iBridgeIndex+1);
	...
	{
		LogAdd(LOG_BLACK, "[Blood Castle] (%d) CBloodCastle::CheckWinnerValid() - (gObj[m_BridgeData[iBridgeIndex].m_iBC_COMPLETE_USER_INDEX].m_cBloodCastleIndex == -1) ...",
			iBridgeIndex+1);
	...
	{
		LogAdd(LOG_BLACK, "[Blood Castle] (%d) CBloodCastle::CheckWinnerValid() - !CHECK_BLOODCASTLE(gObj[m_BridgeData[iBridgeIndex].m_iBC_COMPLETE_USER_INDEX].MapNumber)",
			iBridgeIndex+1);
...
void CBloodCastle::FixUsersPlayStateWin(int iBridgeIndex)
{
	...
	if ( this->m_BridgeData[iBridgeIndex].m_iMISSION_SUCCESS != -1 )
	{
		for ( int i=0;i<MAX_BLOOD_CASTLE_SUB_BRIDGE;i++)
		{
			...
			LogAdd(LOG_BLACK, "[Blood Castle] (%d) [%s][%s] FixUsersPlayStateWin() - State : %d",
...
void CBloodCastle::FixUsersPlayStateFail(int iBridgeIndex)
{
	...
	for ( int i=0;i<MAX_BLOOD_CASTLE_SUB_BRIDGE;i++)
	{
		...
		LogAdd(LOG_BLACK, "[Blood Castle] (%d) [%s][%s] FixUsersPlayStateFail() - State : %d",
