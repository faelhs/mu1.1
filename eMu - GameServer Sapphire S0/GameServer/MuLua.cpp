...
bool MULua::Generic_Call(const char* func, const char* sig, ...)
{
	...
	while (*sig)
	{
		...
		default:
		{
			LogAdd(LOG_RED, "luacall_Generic_Call invalid option (%c)", *(sig - 1));
	...
	{
		LogAdd(LOG_RED, "luacall_Generic_Call error running function '%s': '%s'", func, lua_tolstring(this->m_luaState, -1, 0));
